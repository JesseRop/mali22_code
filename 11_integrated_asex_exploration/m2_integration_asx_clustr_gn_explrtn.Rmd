...
```{r, warning=FALSE, message=FALSE}
v3_m2_asex_cr_filt_norm_harmony_jnd_fine_clustrs <- map(v3_m2_asex_cr_filt_norm_harmony_jnd, ~FindClusters(.x, resolution = 0.9, cluster.name = "harmony_clusters_fine_erings"))
...
#SURF1.2 gexp02
map(v3_m2_asex_cr_filt_norm_harmony_jnd_fine_clustrs, ~FeaturePlot(.x, reduction = "umap.harmony_PC_man",  label = T, features = c("PF3D7-0113600", "PF3D7-1102500"), order = T, dims = c(1,3)))
```
...
er_clstr_set <- list(c("0","5","12"), c("2","11","13"))
...
```{r, warning=FALSE, message=FALSE}
# gene_count
map(v3_early_asex, ~VlnPlot(.x, features = "nFeature_RNA", group.by = "harmony_PC_man_clusters") + geom_hline(yintercept = 600))
# map(v3_early_asex, ~VlnPlot(.x, features = "nCount_RNA") + geom_hline(yintercept = 2000))
map(v3_early_asex, ~VlnPlot(.x, features = "nCount_RNA", group.by = "harmony_PC_man_clusters") + geom_hline(yintercept = 5000))

```

```{r, warning=FALSE, message=FALSE}
# gene_count
map(v3_early_asex, ~VlnPlot(.x, features = "nFeature_RNA") + geom_hline(yintercept = 600))
# map(v3_early_asex, ~VlnPlot(.x, features = "nCount_RNA") + geom_hline(yintercept = 2000))
map(v3_early_asex, ~VlnPlot(.x, features = "nCount_RNA") + geom_hline(yintercept = 5000))
...
```{r, warning=FALSE, message=FALSE}
# gene_count
v3_early_asex <- map(v3_early_asex, ~subset(.x, subset = nFeature_RNA < 600))
# v3_early_asex <- map(v3_early_asex, ~subset(.x, subset = nCount_RNA < 2500))
v3_early_asex <- map(v3_early_asex, ~subset(.x, subset = nCount_RNA < 5000))
...
```{r, message=F, warning=F}
##Read BPcells matrices
bp_CbendrCr_prelim_stg_sct_m68C <- readRDS("data/processed/Pf/MSC68C/seu_obj/bp_CbendrCr3_prelim_stg_sct.RDS")
bp_CbendrCr_prelim_stg_sct_m68 <- readRDS("data/processed/Pf/MSC68/seu_obj/bp_CbendrCr3_prelim_stg_sct.RDS")
