...
##Read edrops results matrices
bcranks_cutoff_plot_fn <-  function(dset, hline_droplets = "umi_counts_cutoff_total_droplets_", hline_cells = "umi_counts_cutoff_cell_estimate_", hline_cells_lab = "umi_cutoff_cells:", hline_droplets_lab = "umi_cutoff_droplets:", line_part = "knee") {
    ...
    ggplot(., aes(x = rank, y = total)) +
      ...
      geom_text(data = label_data,
                ...
                vjust = -0.2) +
      facet_wrap2(vars(donor, species, jumpcode), ncol = 6) +
...
map(don_names, ~{
  ...
  map(c("knee", "inflection"), ~{
    ...
    bcranks_cutoff_plot_fn(dset = all_bcrank_mrg_sset, hline_droplets = "n_cells_total_droplets_estimated_ndroplets", hline_droplets_lab = "umi_cutoff_droplets:", hline_cells = "n_cells_cell_estimate_estimated_ncells", hline_cells_lab = "umi_cutoff_cells:", line_part = .x)
...
map(don_names, ~{
  don = .x
  map(c("knee", "inflection"), possibly(~{
    all_bcrank_mrg_sset <- all_bcrank_mrg %>% filter(donor == don)
    
    print(dim(all_bcrank_mrg_sset))
    
    label_data = distinct(all_bcrank_mrg_sset, donor, species, jumpcode, !!rlang::sym(paste0("umi_counts_cutoff_cell_estimate_", .x)), !!rlang::sym(paste0("umi_counts_cutoff_total_droplets_", .x)))
    ...
    ggplot(., aes(x = rank, y = total)) +
        ...
        scale_x_log10() +
        scale_y_log10() +
      geom_hline(aes(yintercept = !!rlang::sym(paste0("umi_counts_cutoff_cell_estimate_", .x)))) +
      geom_hline(aes(yintercept = !!rlang::sym(paste0("umi_counts_cutoff_total_droplets_", .x)))) +
      geom_text(data = label_data,
                aes(x = 500, 
                    y = !!rlang::sym(paste0("umi_counts_cutoff_cell_estimate_", .x)), 
                    label = paste("umi_cutoff_cells:", !!rlang::sym(paste0("umi_counts_cutoff_cell_estimate_", .x)))),
                vjust = -0.2) +
      geom_text(data = label_data,
                aes(x = 500, 
                    y = !!rlang::sym(paste0("umi_counts_cutoff_total_droplets_", .x)), 
                    label = paste("umi_cutoff_droplets:", !!rlang::sym(paste0("umi_counts_cutoff_total_droplets_", .x)))),
                vjust = -0.2) +
      facet_wrap2(vars(donor, species, jumpcode), ncol = 6) +
      labs(title = paste0(.x)) 
  
  
  }), 'k=1 so no groups to compare which is necessary for the heatmap')
})
...
map(c("knee", "inflection"), ~{
  ...
  label_data = distinct(all_bcrank_mrg, donor, species, jumpcode, !!rlang::sym(paste0("umi_counts_cutoff_cell_estimate_", .x)), !!rlang::sym(paste0("umi_counts_cutoff_total_droplets_", .x)))
  ...
  ggplot(., aes(x = rank, y = total)) +
    ...
    geom_text(data = label_data,
              aes(x = 500, 
                  y = !!rlang::sym(paste0("umi_counts_cutoff_cell_estimate_", .x)), 
                  label = paste("cell_estimate:", !!rlang::sym(paste0("umi_counts_cutoff_cell_estimate_", .x))))) +
    geom_text(data = label_data,
              aes(x = 500, 
                  y = !!rlang::sym(paste0("umi_counts_cutoff_total_droplets_", .x)), 
                  label = paste("total_droplets:", !!rlang::sym(paste0("umi_counts_cutoff_total_droplets_", .x))))) +
    facet_wrap2(vars(donor, species, jumpcode)) +
    labs(title = paste0(.x)) 
...
```{r, message=F, warning=F, eval=T}
##Read edrops results matrices
all_bcrank_umi_estimates <- map(c("knee", "inflection"), ~{

  hlines_df = data.frame(lims = c(yascp_cbender_cutoffs_all_ncells_ndrops %>% filter(donor_cutoff == paste0(.y, "_cell_estimate")) %>% filter(method == knee_or_inlctn) %>% pull(umi_counts_cutoff),
                                  yascp_cbender_cutoffs_all_ncells_ndrops %>% filter(donor_cutoff == paste0(.y, "_total_droplets")) %>% filter(method == knee_or_inlctn)  %>% pull(umi_counts_cutoff)),
  ...
  ggplot(., aes(x = rank, y = total)) +
    ...
    pivot_longer()
    labs(title = paste0(knee_or_inlctn)) +
...
##Read edrops results matrices
imap(all_bcrank, possibly(~{
  hlines_df = data.frame(lims = c(yascp_cbender_cutoffs_all_ncells_ndrops %>% filter(donor_cutoff == paste0(.y, "_wHsPf_cell_estimate")) %>% filter(method == "estimated_ncells") %>% pull(n_cells),
                                  yascp_cbender_cutoffs_all_ncells_ndrops %>% filter(donor_cutoff == paste0(.y, "_wHsPf_total_droplets")) %>% filter(method == "estimated_ndroplets") %>% pull(n_cells)),
                         labls = c("cell_estimate", "total_droplets"),
                         pos = rep(5, 2))
  # print(hlines_df)
...
```{r, message=F, warning=F, eval=T, fig.height= 4, fig.width=10}
##Read edrops results matrices
bcranks_ncells_ndrops_plot_fn <-  function(dset, don_nm) {
    ...
    dset %>%
      ...
      geom_text(data = label_data,
                aes(y = 5, 
                    ...
                    label = "total_droplets"),
                vjust = -0.2, angle = 90) +
      facet_wrap2(vars(donor, species, jumpcode), ncol = 6, scales = "free_x") +
      theme_classic()
...
```{r}
all_msc_bp_raw_noHsGns_mdata <- readRDS("data/processed/Pf/m2_all/all_msc_bp_raw_noHsGns_mdata.RDS")
```

```{r, message=F, warning=F, eval=T}
##Read edrops results matrices
imap(all_msc_bp_raw_noHsGns_mdata, ~.x %>%
    data.frame() %>%
  # filter(total < 500) %>% 
  filter(!(duplicated(rank))) %>%
  # rename("total_umi" = detected) %>%
...
all_msc_bp_raw_noHsGns_mdata_mrg <- map2(all_msc_bp_raw_noHsGns_mdata, man_thresh[names(all_msc_bp_raw_noHsGns_mdata)], ~{
  .x %>% 
...
kneeplt_facet_fn(ncount_tbl = all_msc_bp_raw_noHsGns_mdata_mrg %>% filter(!(duplicated(pf_rank)), .by = "donor") , cell_call_methd = "donor", donor_group = "donor", numbr_of_cols = 5, rank_var = "pf_rank", cell_var = "pf_umi_count", cut_offs = c(40,50))

```

```{r, fig.height= 10, fig.width= 10, eval=T}
kneeplt_facet_fn(ncount_tbl = all_msc_bp_raw_noHsGns_mdata_mrg %>% filter(!(duplicated(pf_rank)), .by = "donor") , cell_call_methd = "donor", donor_group = "donor", numbr_of_cols = 5, rank_var = "hs_rank", cell_var = "hs_umi_count")
...
```{r, fig.height= 10, fig.width= 10, eval=T}
kneeplt_facet_fn(ncount_tbl = all_msc_bp_raw_noHsGns_mdata_mrg %>% filter(!(duplicated(pf_rank)), .by = "donor") , cell_call_methd = "donor", donor_group = "donor", numbr_of_cols = 5, rank_var = "pf_rank", cell_var = "nCount_RNA")

```


```{r, fig.height= 10, fig.width= 10, eval=T}
kneeplt_facet_fn(ncount_tbl = all_msc_bp_raw_noHsGns_mdata_mrg %>% filter(!(duplicated(pf_rank)), .by = "donor") , cell_call_methd = "donor", donor_group = "donor", numbr_of_cols = 5, rank_var = "hs_rank", cell_var = "nCount_RNA")

```


```{r, fig.height= 10, fig.width= 10, eval=T}
all_msc_bp_raw_noHsGns_mdata_mrg %>% 
  filter(nCount_RNA > 5) %>%
