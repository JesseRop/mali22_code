...
cbender_mdata_std_yascp_syst <- map(dirs, ~list.files(.x, pattern = "cb_mdata.csv$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/cbender_custom2|_gns|/cb_mdata.*|/cb|/'))))  %>% unlist(., recursive = F) %>% 
  ...
  .[c(sample_hsLRloEhi, sample_hsLRloElo, sample_hsLRhiEhi, sample_hsLRhiElo, sample_pfLRloEhi, sample_pfLRloElo, sample_pfLRhiEhi, sample_pfLRhiElo)] %>%
  .[!is.na(.)] %>%
  map(read_csv) 
```
...
cr_cbender_mdata_hmnz <- map2(cr_cbender_mdata_hmnz_syst_snky_pfumi, cranger_bcodes[names(cr_cbender_mdata_hmnz_syst_snky_pfumi)], ~.x %>%
    mutate(#is_cell_cr = case_when(barcode %in% .y$bcode ~ "Cr"),
           is_cell_cb_hs = case_when(cell_probability_hsLRhiElo > 0.5  ~ "Cbh", cell_probability_hsLRhiElo <= 0.5  ~ "nonCell"),
           is_cell_cb_pf = case_when(cell_probability_pfLRhiElo > 0.5  ~ "Cbp", cell_probability_pfLRhiElo <= 0.5  ~ "nonCell"),
           is_cell_cb_yhs = case_when(cell_probability_hsLRloEhi > 0.5  ~ "Cbyh", cell_probability_hsLRloEhi <= 0.5  ~ "nonCell"),
           is_cell_cb_ypf = case_when(cell_probability_pfLRloEhi > 0.5  ~ "Cbyp", cell_probability_pfLRloEhi <= 0.5  ~ "nonCell")
           ) #%>%
      #full_join(., .y, by = c("barcode"="bcode"))
    )
...
cr_cbender_mdata_uni <- map(cr_cbender_mdata_hmnz, ~.x %>%
    # filter() %>%
    mutate(mthds_union = paste0(is_cell_cr, is_cell_cb_pf, is_cell_cb_yhs, is_cell_cb_ypf),
...
cr_cbender_mdata_uni <- map(cr_cbender_mdata_uni, ~.x %>%
    ...
    mutate(across(mthds_union, ~factor(str_replace_all(., c("^Cb" = "", "Cb" = "_")), levels = c_combos)))
)

```

```{r}
map(cr_cbender_mdata_uni[1:2], ~.x %>% count(is_cell_cr,is_cell_cr, is_cell_cb_pf, is_cell_cb_yhs, is_cell_cb_ypf, mthds_union))
