...
```{r, message=F, warning=F, eval=T}
dirs <- Sys.glob("data/processed/Pf/MSC*/cbender_custom_*/cb*")
class(dirs)
...
cbender_mdata_std_yascp_syst <- map(dirs, ~list.files(.x, pattern = "cb_mdata.csv$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/cbender_custom|_gns|/cb_mdata.*|/cb|/'))))  %>% unlist(., recursive = F) %>% 
