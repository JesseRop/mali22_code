...
# MSC Preprocessing - filter out different poor qC cells
...
### POORLY ANNOTATED VISUALIZATIONS
...
```{r, fig.height = 12, fig.width = 12}
map(all_seu_prelim_stg_norm, ~.@meta.data) %>% 
  bind_rows(., .id = 'donor')  %>%
...
plts_seu_hsumi50 <- imap(all_seu_prelim_stg_norm, possibly(~DimPlot(.x, label = T, cells.highlight =  colnames(.x[, (.x@meta.data$hs_gn_count > 40 & .x@meta.data$mthds_union_slim3 == "mHs")]), sizes.highlight = 0.05) +       theme_void() + theme(legend.position = "none", panel.border = element_rect(color = "black", fill = NA, size = 1))  +
                                     labs(title = .y), "no cells"))
...
## 2.4 CELLBENDER METRICS CLUSTER VISUALIZATIONS - NOT USED FOR FILTRATION
