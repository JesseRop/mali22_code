...
# all_seu_prelim_stg_norm <- list.files("data/processed/Pf", pattern = "bpseu_(cr|cbendr|edrops)_prelim_stg_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
all_seu_prelim_stg_norm <- list.files("data/processed/Pf", pattern = "bp_CbendrCr_prelim_stg_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|bp.*'))) %>% 
...
```{r, message=FALSE}
cr_cbender_pf100_mdata_slct_uni <- readRDS("data/processed/Pf/m2_all/cr_cbender_pf100_mdata_slct_uni.RDS")
...
all_seu_prelim_stg_norm <- list(all_seu_prelim_stg_norm, cr_cbender_pf100_mdata_slct_uni[names(all_seu_prelim_stg_norm)]) %>%
  pmap(~ {
    ..2 %>%
      select(mthds_union_slim, barcode) %>%
...
#     .x@meta.data %>%
#     ggplot(aes(x = mthds_union_slim, y = background_fraction_yPf)) +
#     geom_boxplot()
#   })
...
```{r, fig.height = 12, fig.width = 12}
all_seu_prelim_stg_norm_mdata_mrg %>%
  ggplot(aes(x = background_fraction_yHs, y = background_fraction_yPf, colour = mthds_union_slim)) +
...
```{r, fig.height = 12, fig.width = 12}
all_seu_prelim_stg_norm_mdata_mrg %>%
  ggplot(aes(x = cell_probability_yHs, y = cell_probability_yPf, colour = mthds_union_slim)) +
  geom_point(size = 0.01) +
...
```{r, fig.height = 12, fig.width = 12}
all_seu_prelim_stg_norm_mdata_mrg %>%
  ggplot(aes(x = log10(hs_umi_count), y = log10(pf_umi_count), colour = mthds_union_slim)) +
    geom_point(size = 0.01) +
  geom_abline() +
  facet_wrap(donor ~.) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
```{r, warning=FALSE, message=FALSE, fig.width=4, fig.height=4.5}
## in cellbender but not in edrops
plts_seu_cb5 <- imap(all_seu_prelim_stg_norm, possibly(~DimPlot(.x, label = T, cells.highlight =  colnames(.x[,.x@meta.data$cell_probability_yPf < 0.9 & .x@meta.data$cell_probability_yHs < 0.9]), sizes.highlight = 0.05) + 
                                     theme_void() + theme(legend.position = "none", panel.border = element_rect(color = "black", fill = NA, size = 1))  +
                                     labs(title = .y), "no cells"))
...
my21_qc_vln_fn(clust_var = "sR_fine_stg", clust_cols = sp_col_new, ci_sufx = "_stg", row_numbers = 1 ) 
...
map(all_seu_filtOlMtCb, ~.x@meta.data %>% count(cell_probability_yPf >= 0.9))
```
...
### gn_edrops50
# imap(all_ed, ~saveRDS(.x, paste0("data/processed/Pf/",str_remove(.y, "_edrops50"), "/seu_obj/bpseu_raw_ed_pref.RDS")))
