...
```{r, message=F, warning=F}
##Read BPcells matrices
all_msc_cb_no_dbt_sct_anotd <- list.files("data/processed/Pf", pattern = "all_msc_cb_no_dbt_sct_prelim_anotd.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|all_msc_cb_no_dbt_sct_prelim_anotd.RDS'))) %>% 
# all_msc_cb_no_dbt_sct_multi_ref_anotd <- list.files("data/processed/Pf", pattern = "all_msc_cb_no_dbt_sct_multi_ref_anotd.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
#   set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|all_msc_cb_no_dbt_sct_multi_ref_anotd.RDS'))) %>% 
  # .[sample_name_all] %>%
  .[sample_name] %>%
  map(readRDS)
```
...
multi_ref_stg_tbl1 <- map(all_msc_cb_no_dbt_sct_anotd, ~{
  obj <- .x
...
all_msc_cb_no_dbt_sct <- list(multi_ref_stg_tbl2, all_msc_cb_no_dbt_sct) %>%
  pmap(~ ..1 %>%
...
```{r}
all_msc_cb_no_dbt_sct %>% map(., ~.x@meta.data %>% count(stage_final2, combined_labs, sR_fine_stg) #%>% 
...
```{r}
all_msc_cb_no_dbt_sct %>% map(., ~.x@meta.data %>% 
...
```{r}
## Adding stage annotations to query cells
imap(all_msc_cb_no_dbt_sct, ~DimPlot(.x, group.by = "stage_fld") + labs(title = .y)) 
```

```{r}
## Adding stage annotations to query cells
imap(all_msc_cb_no_dbt_sct, ~DimPlot(.x, group.by = "stage_final2") + labs(title = .y) + scale_color_manual(values = sp_col_new)) 
```


```{r, warning=FALSE, message=FALSE, fig.width=4, fig.height=4.5}
## in both edrops and cellbender
qcd_umaps <- imap(all_msc_cb_no_dbt_sct, ~DimPlot(.x, group.by = "stage_final2", pt.size = 0.01) + 
                    labs(title = .y) + 
...
```{r, warning=FALSE, message=FALSE, fig.height= 10, fig.width= 10, eval=T}
plot_grid(plotlist = qcd_umaps[!str_detect(names(qcd_umaps), "12")], ncol = 5)
...
```{r}
## pfs16
imap(all_msc_cb_no_dbt_sct, ~FeaturePlot(.x, label = T, features = "PF3D7-0406200") + labs(title = .y)) 
```

```{r}
## dnaj
imap(all_msc_cb_no_dbt_sct, ~FeaturePlot(.x, label = T, features = "PF3D7-1473200", order = T) + labs(title = .y)) 
```

```{r}
## hda1
imap(all_msc_cb_no_dbt_sct, ~FeaturePlot(.x, label = T, features = "PF3D7-1467600", order = T) + labs(title = .y)) 
```

```{r}
## gexp02
imap(all_msc_cb_no_dbt_sct, ~FeaturePlot(.x, label = T, features = "PF3D7-1102500", order = T) + labs(title = .y)) 
```
...
```{r}
map(all_msc_cb_no_dbt_sct, ~.x@meta.data %>% count(stage_fld) %>% arrange(n) )

```


```{r, warning=FALSE, message=FALSE, fig.width=16, fig.height=4}
imap(all_msc_cb_no_dbt_sct, ~DimPlot(.x, label = T, group.by = c("sR_fine_stg", "scf_class_rand", "seurat_clusters"), ncol = 4) + 
       labs(title = .y) )

```

```{r, warning=FALSE, message=FALSE, fig.width=16, fig.height=4}
imap(all_msc_cb_no_dbt_sct, ~FeaturePlot(.x, label = T, features = c("nCount_RNA", "nFeature_RNA", "cell_probability", "background_fraction"), ncol = 4) + 
       labs(title = .y) )
...
#################################### START compare stage annotations - should be in stage assingment rmd ####################################


```{r, message=FALSE, eval=F}
all_stg_anots <- map(all_msc_cb_no_dbt_sct, ~{
  .x@meta.data %>% 
    mutate(lbls = as.character(str_remove_all(sR_fine_stg, "\\(|\\)")),
           lbls_scmap = as.character(StagePrelim),
           across(c(lbls, lbls_scmap), ~replace_na(., "missing")))
  })
```


pheatmap
```{r , fig.width = 10, fig.height = 5, warning=FALSE, eval=F}
...
phmap_lab_transfr <- function(dset_lab, col_labs, row_labs, row_labs_c, col_labs_c, strn_order, ttl){
  ...
  hm_mtx <- dset_lab %>% dplyr::count({{ col_labs }}, {{ row_labs }}) %>%
    ...
    mutate(across(c({{ col_labs }}, {{ row_labs }}), )) %>%
...
```{r , fig.width = 10, fig.height = 5, warning=FALSE, eval=F}
#
imap(all_stg_anots, ~phmap_lab_transfr(.x, col_labs= `lbls_scmap`, row_labs=`lbls`, row_labs_c ='lbls', col_labs_c= 'lbls_scmap', ttl = .y))

```
#################################### END compare stage annotations ####################################

```{r, eval=F}
mthds <- unique(str_remove(names(all_msc_cb_no_dbt_sct), "M.*_"))

```

```{r, eval=FALSE}
## Making list of objects and renaming before integrating
m2_seu_raw_edcb <- map(mthds, ~{
  obj <- merge(x = all_msc_cb_no_dbt_sct[str_detect(names(all_msc_cb_no_dbt_sct), .x)][[1]], 
               y = all_msc_cb_no_dbt_sct[str_detect(names(all_msc_cb_no_dbt_sct), .x)][2:length(all_msc_cb_no_dbt_sct[str_detect(names(all_msc_cb_no_dbt_sct), .x)])],
               add.cell.ids = str_remove_all(names(all_msc_cb_no_dbt_sct[str_detect(names(all_msc_cb_no_dbt_sct), .x)]), paste0("SC|_", .x)))
  
  obj$donor <- str_remove_all(obj$orig.ident, "SC|\\.mrna")
  return(obj)
  }) %>% set_names(mthds)

# m2_seu_raw_cb$donor <- str_remove_all(m2_seu_raw_cb$orig.ident, "SC|\\.mrna")
# m2_seu_raw_cb$stage_c <- factor(m2_seu_raw_cb$stage_c, levels = c("Asexual", "Female", "Female LE", "Male", "Male LE"))
```

```{r}
## Making list of objects and renaming before integrating

# m2_seu_raw_edcb <-map(m2_seu_raw_edcb, ~{
#     obj <- .x
#     colnames(obj) <- str_remove(colnames(obj), "cb_")
#     return(obj)
#   })
...
# m2_seu_raw_cb_jnd <- JoinLayers(m2_seu_raw_cb)
# m2_seu_raw_cb_noM12 <- m2_seu_raw_cb[,m2_seu_raw_cb@meta.data$donor != "M12"]

```

Write out in .h5ad 
```{r, message=FALSE, warning=FALSE, eval=F}
imap(m2_seu_raw_edcb, ~{
  ##Change factor metadata to character as these are not handled properly when converting to h5ad
  m2_seu_raw_cb_v3 <- .x
  
  ## Join layers before conversion
  m2_seu_raw_cb_v3 <- JoinLayers(m2_seu_raw_cb_v3, layers = "counts")
  ...
  DefaultAssay(m2_seu_raw_cb_v3) <- "RNA"
  m2_seu_raw_cb_v3[["RNA"]] <- CreateAssayObject(counts = m2_seu_raw_cb_v3[["RNA"]]$counts) #Convert Assay v5 to Assay
  m2_seu_raw_cb_v3[["SCT"]] <- NULL
  
  SaveH5Seurat(m2_seu_raw_cb_v3, filename =  paste0("data/processed/Pf/m2_all/m2_seu_raw_", .y,".h5Seurat"), overwrite = T)
  
  Convert(paste0("data/processed/Pf/m2_all/m2_seu_raw_", .y,".h5Seurat"), dest = "h5ad", overwrite = T)
...
## Join layers before conversion
DefaultAssay(all_msc_cb_no_dbt_mrg) <- "RNA"
all_msc_cb_no_dbt_mrg
```

```{r, eval=F}
## Join layers before conversion
saveRDS(all_msc_cb_no_dbt_mrg, "data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg.RDS")
```


```{r, eval=F}
all_msc_cb_no_dbt_sct_noM12 <- all_msc_cb_no_dbt_sct[!names(all_msc_cb_no_dbt_sct) %in% c("MSC12")]
...
```{r, eval=F}
## Making list of objects and renaming before integrating
all_msc_cb_no_dbt_mrg_noM12 <- merge(x = all_msc_cb_no_dbt_sct_noM12[[1]],
                             y = all_msc_cb_no_dbt_sct_noM12[2:length(all_msc_cb_no_dbt_sct_noM12)],
                             add.cell.ids = str_remove(names(all_msc_cb_no_dbt_sct_noM12), "SC"))
  
## Join layers before conversion
DefaultAssay(all_msc_cb_no_dbt_mrg_noM12) <- "RNA"
all_msc_cb_no_dbt_mrg_noM12
```
...
```{r, eval=F}
## Remove poor QC
all_msc_cb_no_dbt_mrg_noM12 <- subset(all_msc_cb_no_dbt_mrg_noM12, subset = sR_fine_stg %in% c("weak_score", "discrepantFM"), invert = T)

```

```{r, eval=F}
## Join layers before conversion
saveRDS(all_msc_cb_no_dbt_mrg_noM12, "data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_noM12.RDS")
```
...
all_msc_cb_no_dbt_mrg_noM12_v3 <- all_msc_cb_no_dbt_mrg_noM12
  
DefaultAssay(all_msc_cb_no_dbt_mrg_noM12_v3) <- "RNA"

## Join layers before conversion
all_msc_cb_no_dbt_mrg_noM12_v3 <- JoinLayers(all_msc_cb_no_dbt_mrg_noM12_v3, assay = "RNA", layers = "counts")

# ## !!!NB - NEEDS SORTING IN FUTURE UPGRADING TO SEURAT 5 RESULTED IN ERROR AS DESCRIBED https://github.com/mojaveazure/seurat-disk/issues/147#top

all_msc_cb_no_dbt_mrg_noM12_v3[["RNA"]] <- CreateAssayObject(counts = all_msc_cb_no_dbt_mrg_noM12_v3[["RNA"]]$counts) #Convert Assay v5 to Assay
all_msc_cb_no_dbt_mrg_noM12_v3[["SCT"]] <- NULL
# all_msc_cb_no_dbt_mrg_noM12_v3 <- CreateAssayObject(counts = all_msc_cb_no_dbt_mrg_noM12_v3[["RNA"]]$counts) #Convert Assay v5 to Assay
# all_msc_cb_no_dbt_mrg_noM12_v3@meta.data <- all_msc_cb_no_dbt_mrg_noM12@meta.data
  
SaveH5Seurat(all_msc_cb_no_dbt_mrg_noM12_v3, filename =  paste0("data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_noM12_v3.h5Seurat"), overwrite = T)
  
Convert(paste0("data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_noM12_v3.h5Seurat"), dest = "h5ad", overwrite = T)
  
rm(all_msc_cb_no_dbt_mrg_noM12_v3)

```

```{r, eval=T}
## Making list of objects and renaming before integrating
all_msc_cb_no_dbt_mrg_noM12_asex <- subset(all_msc_cb_no_dbt_mrg_noM12, subset = sR_fine_stg %in% c("Early ring", "Late ring", "Early trophozoite", "Late trophozoite"))
...
```{r}
## Join layers before conversion
# all_msc_cb_no_dbt_mrg <- readRDS("data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg.RDS")
```
...
# saveRDS(m2_seu_raw_cb, "data/processed/Pf/m2_all/m2_seu_raw_cb.RDS")
...
```{r, message=FALSE, warning=FALSE}
## save for running stdseurat in farm
# m2_seu_raw_cb_no_M12 <- subset(m2_seu_raw_cb, subset = donor != "M12")
# m2_seu_raw_cb@meta.data %>% count(donor)
```

```{r, eval=F}
## write out seurat bpcells object files for normalization processing in farm
### gn_edrops50
imap(m2_seu_raw_edcb, ~saveRDS(.x, paste0("data/processed/Pf/m2_all/m2_seu_raw_", .y,".RDS")))
```
...
# bsub < /lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/mali22_code_lnk/bash_scripts/submsn_seu_norm_sct_integration_bsub.sh
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_stdnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg.RDS" --normlzn "norm" --umap_name "UMAP_" --hvg 700 --cluster_resoln 1 --cl_nm "seurat_clusters" --umap_redctn_nm "umap.unintegrated" --redctn "pca" --integrtd_pc30 "no" --ncores "12" --mem "150 GB" --resume

# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_sctnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_noM12.RDS" --normlzn "sct" --umap_name "UMAP_" --hvg 500 --cluster_resoln 0.1 --cl_nm "seurat_clusters_sct" --umap_redctn_nm "umap.unintegrated" --redctn "pca" --integrtd_pc30 "no" --ncores "20" --mem "120 GB" --resume
...
```{r}
m2_seu_raw_cb_norm <-  readRDS("data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_norm.RDS")
# all_msc_cb_no_dbt_mrg_noM12_sct <-  readRDS("data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_noM12_sct.RDS")
```
...
DimPlot(m2_seu_raw_cb_norm,  group.by = "sR_fine_stg", label = F, dims = c(2,3)) + scale_color_manual(values = sp_fld_colors3)
# DimPlot(all_msc_cb_no_dbt_mrg_noM12_sct,  group.by = "sR_fine_stg", label = F, dims = c(1,2)) + scale_color_manual(values = sp_fld_colors3)
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
DimPlot(m2_seu_raw_cb_norm, reduction = "umap.unintegrated", group.by = "sR_fine_stg", label = F, dims = c(1,2), pt.size = 2) + scale_color_manual(values = sp_fld_colors3)
DimPlot(m2_seu_raw_cb_norm, reduction = "umap.unintegrated", group.by = "strain_gt", label = F, dims = c(1,2), pt.size = 2) 
DimPlot(m2_seu_raw_cb_norm, reduction = "umap.unintegrated", group.by = "scf_class_norm", label = F, dims = c(1,2), pt.size = 2) 
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
# DimPlot(m2_seu_raw_cb_norm, reduction = "pca", group.by = "StagePrelim", label = F, dims = c(1,2), pt.size = 0.1, raster = F) 
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", group.by = "sR_fine_stg", label = F, dims = c(1,2), pt.size = 0.1, raster = F) 
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", group.by = "status", label = F, dims = c(1,2), pt.size = 0.1, raster = F) 
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", split.by  = "scf_class_norm", label = F, dims = c(1,2), pt.size = 0.1, raster = F) 
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
# DimPlot(m2_seu_raw_cb_norm, reduction = "pca", label = F, dims = c(1,2), pt.size = 0.1, raster = F, cells.highlight = colnames(m2_seu_raw_cb_norm[, m2_seu_raw_cb_norm$donor %in% c("M37", "M38", "M39")]), sizes.highlight = 0.2)
```

```{r, fig.width=6, fig.height=5}
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", label = F, dims = c(1,2), pt.size = 0.1, raster = F, group.by = "donor")
```

```{r, fig.width=28, fig.height=4}
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", label = F, dims = c(1,2), pt.size = 0.1, raster = F, split.by = "donor")
DimPlot(m2_seu_raw_cb_norm, reduction = "umap.unintegrated", label = F, dims = c(1,2), pt.size = 0.1, raster = F, split.by = "donor")
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", group.by = "StagePrelim", label = F, dims = c(1,3), pt.size = 0.1, raster = F) 
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", group.by = "sR_fine_stg", label = F, dims = c(1,3), pt.size = 0.1, raster = F) 
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", group.by = "status", label = F, dims = c(1,3), pt.size = 0.1, raster = F) 
DimPlot(m2_seu_raw_cb_norm, reduction = "pca", split.by  = "scf_class_norm", label = F, dims = c(1,3), pt.size = 0.1, raster = F) 
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_intgrtn_pc.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_integrtn_pc_auto.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_norm.RDS" --mthd "HarmonyIntegration" --mthd_nm "harmony" --cluster_resoln 1 --intgrtn_pc 10 --integrtd_pc_man "no" --findpc_mthd "perpendicular line" --ncores "10" --mem "100 GB" -resume

# nextflow run ../mali22_code_lnk/nf_scripts/submsn_intgrtn_pc.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_integrtn_pc_auto.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_norm.RDS" --mthd "HarmonyIntegration" --mthd_nm "harmony_PC_manl" --cluster_resoln 1 --intgrtn_pc 10 --integrtd_pc_man "yes" --findpc_mthd "perpendicular line" --ncores "16" --mem "140 GB" -resume

# nextflow run ../mali22_code_lnk/nf_scripts/submsn_intgrtn_pc.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_integrtn_pc_auto.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/m2_seu_no_M12_sct.RDS" --mthd "RPCAIntegration" --mthd_nm "dr" --cluster_resoln 1 --intgrtn_pc 10 --integrtd_pc_man "no" --findpc_mthd "perpendicular line" --ncores "20" --mem "120 GB" normalization.method = "SCT" -resume
```


```{r}
m2_seu_raw_cb_norm_harmony <-  readRDS("data/processed/Pf/m2_all/m2_seu_raw_cb_norm_harmony.RDS")
m2_seu_raw_cb_norm_harmony_elbo <-  readRDS("data/processed/Pf/m2_all/m2_seu_raw_cb_norm_harmony_elbo.RDS")
m2_seu_raw_cb_norm_harmony_elbo
```


```{r}
all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl <-  readRDS("data/processed/Pf/m2_all/all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl.RDS")
...
```{r}
DimPlot(all_msc_cb_no_dbt_mrg_noM12_sct_rpca_sct_PC10, reduction = "umap.rpca_sct_PC10", pt.size = 1, dims = c(1,2))
DimPlot(all_msc_cb_no_dbt_mrg_noM12_sct_rpca_sct_PC10, group.by = "sR_fine_stg", reduction = "umap.rpca_sct_PC10", pt.size = 0.1, dims = c(1,2), raster = F) + scale_color_manual(values = sp_col_new)
```

```{r}
DimPlot(all_msc_cb_no_dbt_mrg_noM12_sct_rpca_sct_PC10, reduction = "umap.rpca_sct_PC10", pt.size = 1, dims = c(1,3))
DimPlot(all_msc_cb_no_dbt_mrg_noM12_sct_rpca_sct_PC10, group.by = "sR_fine_stg", reduction = "umap.rpca_sct_PC10", pt.size = 0.1, dims = c(1,3), raster = F) + scale_color_manual(values = sp_col_new)
```


```{r}
# DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl, reduction = "umap.harmony_PC_manl", pt.size = 1, dims = c(1,2))
DimPlot(all_msc_cb_no_dbt_mrg_noM12_sct_rpca_sct_PC10, reduction = "umap.rpca_sct_PC10", pt.size = 1, dims = c(1,2))

# DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", pt.size = 1, dims = c(1,2))
# DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", pt.size = 1, dims = c(2,3))
```

```{r}
DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl[,all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl$orig.ident != "MSC12.mrna"], group.by = "sR_fine_stg", reduction = "umap.harmony_PC_manl", pt.size = 0.1, dims = c(1,2), raster = F) + 
  scale_color_manual(values = sp_col_new)
```

```{r, fig.width=14, fig.width=18}
DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl, group.by = "sR_fine_stg", reduction = "umap.harmony_PC_manl", pt.size = 0.1, dims = c(1,2), raster = F) + scale_color_manual(values = sp_col_new)
DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl, group.by = "orig.ident", reduction = "umap.harmony_PC_manl", pt.size = 0.1, dims = c(1,2), raster = F) 
DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl, reduction = "umap.harmony_PC_manl", pt.size = 0.1, dims = c(1,2), raster = F, label = T)
# DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", pt.size = 1, dims = c(1,2))
# DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", pt.size = 1, dims = c(2,3))
```
...
```{r, fig.width=14, fig.width=18}
DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl, cells.highlight = colnames(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl[,all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl@meta.data$sR_fine_stg %in% c("weak_score", "discrepantFM")]), reduction = "umap.harmony_PC_manl", pt.size = 0.1, dims = c(1,2), raster = F)

DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl, cells.highlight = colnames(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl[,all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl@meta.data$strain_gt %in% c("Negative")]), reduction = "umap.harmony_PC_manl", pt.size = 0.1, dims = c(1,2), raster = F)
...
```{r, eval=F}
## Remove poor QC
all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl@meta.data %>% count(seurat_clusters, sR_fine_stg) %>% filter(sR_fine_stg %in% c("Male (LE)", "discrepantFM", "weak_score"))
```

```{r}
DimPlot(all_msc_cb_no_dbt_mrg_norm_harmony_PC_manl, reduction = "integrated.harmony_PC_manl", pt.size = 1, dims = c(1,2))
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", pt.size = 1, dims = c(1,2))
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", pt.size = 1, dims = c(2,3))
```

```{r}
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", group.by = c("sR_fine_stg"), pt.size = 1, dims = c(1,2)) 
```

```{r}
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", group.by = c("sR_fine_stg"), pt.size = 1, dims = c(2,3))
```


```{r}
DimPlot(m2_seu_raw_cb_norm_harmony_PC_manl, reduction = "integrated.harmony_PC_manl", group.by = c("fine_stg"), pt.size = 0.1, dims = c(1,2), raster = F) + scale_color_manual(values = sp_fld_colors3)
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", group.by = c("fine_stg"), pt.size = 0.1, dims = c(1,2), raster = F) + scale_color_manual(values = sp_fld_colors)
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", group.by = c("fine_stg"), pt.size = 0.1, dims = c(1,3), raster = F) + scale_color_manual(values = sp_fld_colors)
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", group.by = c("fine_stg"), pt.size = 0.1, dims = c(2,3), raster = F) + scale_color_manual(values = sp_fld_colors)

```


```{r}
DimPlot(m2_seu_raw_cb_norm_harmony_PC_manl, reduction = "umap.harmony_PC_manl", group.by = c("fine_stg"), pt.size = 0.1, dims = c(1,2), raster = F) + scale_color_manual(values = sp_fld_colors3)
DimPlot(m2_seu_raw_cb_norm_harmony_PC_manl, reduction = "umap.harmony_PC_manl", group.by = c("seurat_clusters"), pt.size = 0.1, dims = c(2,3), raster = F, label = T) 

DimPlot(m2_seu_raw_cb_norm_harmony_PC_manl, reduction = "umap.harmony_PC_manl", group.by = c("fine_stg"), pt.size = 0.1, dims = c(2,3), raster = F) + scale_color_manual(values = sp_fld_colors3)
```

```{r, warning=FALSE, message=FALSE, eval=FALSE}
# Display the combined datasets as an interactive 3D plot

plot_ly(data = data.frame(m2_seu_raw_cb_norm_harmony_PC_manl@reductions[["umap.harmony_PC_manl"]]@cell.embeddings),
        x = ~umapharmonyPCmanl_1, y = ~umapharmonyPCmanl_2, z = ~umapharmonyPCmanl_3,
        color= m2_seu_raw_cb_norm_harmony_PC_manl@meta.data$fine_stg,
        # colors = c('blue', 'red'),
        opacity = 0.8,
        size = 1, 
        type = 'scatter3d'
...
```{r}
all_mrks <- FindAllMarkers(m2_seu_raw_cb_norm_harmony_PC_manl)
```


```{r}
m2_seu_raw_cb_norm_harmony_PC_manl <- JoinLayers(m2_seu_raw_cb_norm_harmony_PC_manl, layers = c("data", "counts", "scale.data"))
```

```{r}
m2_seu_raw_cb_norm_harmony_PC_manl[["RNA"]]$data <- as(object = m2_seu_raw_cb_norm_harmony_PC_manl[["RNA"]]$data, Class = "dgCMatrix")
```
...
m2_seu_cb_scvi_asex_norm <- list.files("data/processed/Pf/m2_all", pattern = "m_ad_cb_l_100_asex.*.csv$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/m2_all|/|m_ad_cb_l_100_asex_|_mdata.csv'))) %>% 
  # .[sample_name_ed_cr] %>%
  .[asex_opts] %>%
  .[!is.na(.)] %>%
  map(., ~.x %>% read_csv() %>% rename("bcode" = ...1))

```

```{r, message=FALSE, warning=FALSE}
## read in input for subsetting
# m2_seu_raw_cb <- readRDS("data/processed/Pf/m2_all/m2_seu_raw_cb.RDS")

```

```{r, message=FALSE, warning=FALSE}
## rename the barcode names with M50 to harmonize
# colnames(m2_seu_raw_cb_norm_harmony_PC_manl) <- str_replace_all(colnames(m2_seu_raw_cb_norm_harmony_PC_manl), c("M50SLO" = "M50_SLO", "M50MACS" = "M50_MACS"))
```

```{r, eval=T}
## subset asexuals
m2_seu_cb_scvi_asex <- map(m_ad_cb_l_100_asex_mdata, ~m2_seu_raw_cb[,.x$bcode])
```

```{r, eval=T}
## save for harmony
imap(m2_seu_cb_scvi_asex, ~saveRDS(.x, paste0("data/processed/Pf/m2_all/m2_seu_cb_scvi_asex_", .y,".RDS")))
```
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_stdnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/m2_seu_cb_scvi_asex_*.RDS" --normlzn "norm" --umap_name "UMAP_" --hvg 500 --cluster_resoln 1 --cl_nm "seurat_clusters" --umap_redctn_nm "umap.unintegrated" --redctn "pca" --integrtd_pc30 "no" --ncores "12" --mem "80 GB" --resume
...
```{r, message=F, warning=F, eval=T}
##Read raw objects with metadata
m2_seu_cb_scvi_asex_norm <- list.files("data/processed/Pf/m2_all", pattern = "m2_seu_cb_scvi_asex.*_norm.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/m2_all|/|m2_seu_cb_scvi_asex_|_norm.RDS'))) %>% 
  # .[sample_name_ed_cr] %>%
  .[asex_opts] %>%
  .[!is.na(.)] %>%
  map(readRDS)
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "umap.unintegrated", group.by = "StagePrelim", label = F, dims = c(1,2)) + scale_color_manual(values = sp_fld_colors3))
```

```{r, fig.width=6, fig.height=5}
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "umap.unintegrated", group.by = "fine_stg", label = F, dims = c(1,2), pt.size = 2) + scale_color_manual(values = sp_fld_colors3))
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "umap.unintegrated", group.by = "status", label = F, dims = c(1,2), pt.size = 2) )
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "umap.unintegrated", group.by = "scf_class_norm", label = F, dims = c(1,2), pt.size = 2) )
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", group.by = "StagePrelim", label = F, dims = c(1,2), pt.size = 0.1, raster = F) )
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", group.by = "fine_stg", label = F, dims = c(1,2), pt.size = 0.1, raster = F) )
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", group.by = "status", label = F, dims = c(1,2), pt.size = 0.1, raster = F) )
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", split.by  = "scf_class_norm", label = F, dims = c(1,2), pt.size = 0.1, raster = F) )
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", label = F, dims = c(1,2), pt.size = 0.1, raster = F, group.by = "donor"))
```

```{r, fig.width=28, fig.height=4}
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", label = F, dims = c(1,2), pt.size = 0.1, raster = F, split.by = "donor"))
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "umap.unintegrated", label = F, dims = c(1,2), pt.size = 0.1, raster = F, split.by = "donor"))
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", group.by = "StagePrelim", label = F, dims = c(1,3), pt.size = 0.1, raster = F) )
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", group.by = "fine_stg", label = F, dims = c(1,3), pt.size = 0.1, raster = F) )
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", group.by = "status", label = F, dims = c(1,3), pt.size = 0.1, raster = F) )
map(m2_seu_cb_scvi_asex_norm, ~DimPlot(.x, reduction = "pca", split.by  = "scf_class_norm", label = F, dims = c(1,3), pt.size = 0.1, raster = F) )
...
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
map(m2_seu_cb_scvi_asex_norm_harmony, ~DimPlot(.x, reduction = "umap.harmony", group.by = "StagePrelim", label = F, dims = c(1,2)) + scale_color_manual(values = sp_fld_colors3))
```

```{r, fig.width=6, fig.height=5}
# visualize by batch and cell type annotation
# cell type annotations were previously added by Azimuth
map(m2_seu_cb_scvi_asex_norm_harmony, ~DimPlot(.x, reduction = "umap.harmony", group.by = "fine_stg", label = F, dims = c(1,2), pt.size = 0.02) + scale_color_manual(values = sp_fld_colors3))
map(m2_seu_cb_scvi_asex_norm_harmony, ~DimPlot(.x, reduction = "umap.harmony", group.by = "status", label = F, dims = c(1,2), pt.size = 0.02) )
map(m2_seu_cb_scvi_asex_norm_harmony, ~DimPlot(.x, reduction = "umap.harmony", group.by = "scf_class_rand", label = F, dims = c(1,2), pt.size = 0.02) )

```

```{r, fig.width=6, fig.height=5}
map(m2_seu_cb_scvi_asex_norm_harmony, ~DimPlot(.x, reduction = "umap.harmony", group.by = "fine_stg", label = F, dims = c(1,3), pt.size = 0.02) + scale_color_manual(values = sp_fld_colors3))
map(m2_seu_cb_scvi_asex_norm_harmony, ~DimPlot(.x, reduction = "umap.harmony", group.by = "fine_stg", label = F, dims = c(2,3), pt.size = 0.02) + scale_color_manual(values = sp_fld_colors3))
```
```{r, eval=T}
map(m2_seu_cb_scvi_asex_norm_harmony, ~FeaturePlot(.x, features = "PF3D7-1222600", order = T, label = F, dims = c(1,2), raster = F, reduction = "umap.harmony"))
```
...
```{r}
DimPlot(m2_seu_raw_cb_norm_harmony_PC_manl, reduction = "umap.harmony_PC_manl", pt.size = 1, dims = c(1,2))
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", pt.size = 1, dims = c(1,2))
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", pt.size = 1, dims = c(2,3))
```

```{r}
DimPlot(m2_seu_raw_cb_norm_harmony_PC_manl, reduction = "integrated.harmony_PC_manl", pt.size = 1, dims = c(1,2))
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", pt.size = 1, dims = c(1,2))
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", pt.size = 1, dims = c(2,3))
```

```{r}
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "umap.harmony", group.by = c("fine_stg"), pt.size = 1, dims = c(1,2)) 
```

```{r}
DimPlot(m2_seu_raw_cb_norm_harmony, reduction = "integrated.harmony", group.by = c("fine_stg"), pt.size = 1, dims = c(2,3))
```
