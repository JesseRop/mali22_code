...
m2_asex_raw_mrg <- readRDS("data/processed/Pf/m2_all/all_msc_qcd_anotd_c_asex_mrg.RDS")
...
```{r}
m2_asex_raw_mrg@meta.data %>% count(orig.ident)
```

```{r}
## Remove donors with less than 50 cells
don50 <- m2_asex_raw_mrg@meta.data %>% filter(n() > 50 , .by = orig.ident) %>% pull(orig.ident) %>% unique()

m2_asex_raw_mrg <- subset(m2_asex_raw_mrg, subset = orig.ident %in% don50)
```

```{r}
m2_asex_raw_mrg@meta.data %>% count(stage_fld == "Unassigned", orig.ident)
```

```{r, fig.width=14, fig.height=4}
VlnPlot(m2_asex_raw_mrg, group.by = "orig.ident", split.by = "stage_fld", features = "nCount_RNA", pt.size = 0) + scale_y_log10()
```
...
```{r, warning=FALSE, message=FALSE}
m2_asex_raw_mrg@meta.data %>% count(stage_fld)
```
...
```{r, warning=FALSE, message=FALSE}
v3_m2_asex_cr_filt_all@meta.data %>% count(donor, stage_fld) %>% group_by(donor) %>% mutate(prop = round(n/sum(n), 3)) %>% filter(stage_fld == "Gametocyte (developing)")
```

```{r, warning=FALSE, message=FALSE}
v3_m2_asex_cr_filt_all@meta.data %>% add_count(stage_fld, name = "stg_n") %>% group_by() %>% dplyr::count(donor)
```

```{r, warning=FALSE, message=FALSE}
v3_m2_asex_cr_filt_all@meta.data %>% filter(stage_fld == "Gametocyte (developing)") %>% dplyr::count(donor)
```
...
plot_ly(data = data.frame(v3_m2_asex_cr_filt_norm_harmony[[1]]@reductions[["umap.harmony_PC_man"]]@cell.embeddings),
        ...
        color= v3_m2_asex_cr_filt_norm_harmony[[1]]@meta.data$stage_fld,
        # colors = c('blue', 'red'),
        opacity = 0.8,
        size = 0.1, 
        type = 'scatter3d'
