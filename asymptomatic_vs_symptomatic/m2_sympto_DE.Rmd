...
umap_meta_df <- m2_asex_harmony_tr_pt_sset@reductions$umap.harmony_pc_man@cell.embeddings %>% as.data.frame() %>%rownames_to_column("cell_bc") %>% left_join(., m2_asex_harmony_tr_pt_sset@meta.data[,c("stage", "StageFL", "pseudotime", "pseudotime_fld_only", "orig.ident", "donor", "StrainDon", "condition", "dset", "harmony_pc_man_clusters", "harmony_clusters_ss")] %>% as.data.frame() %>% rownames_to_column("cell_bc"), by = "cell_bc") %>% mutate(across(donor, ~str_remove(., "SC"))) %>% left_join(., m2_asex_harmony_tr_pt_sset@reductions$integrated.harmony@cell.embeddings %>% as.data.frame() %>%rownames_to_column("cell_bc")) #%>% 
...
```{r, fig.width=7.5, fig.height=4, eval=T}
ggplot(umap_meta_df, aes(x = umapharmonypcman_1, y = umapharmonypcman_2, col = harmony_clusters_ss)) +
  geom_point()
