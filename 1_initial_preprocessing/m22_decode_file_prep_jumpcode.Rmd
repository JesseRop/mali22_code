...
```{r, eval=T}
## Write JC-prioritized, JC+nonJC but only sample name decoding information and mixed infection status
pf_all_id_decode_minus_noJC <- pf_all_id_decode %>% filter(!str_detect(sample_nm, "noJC"))

write_csv(pf_all_id_decode_minus_noJC, "data/raw/pf_all_id_decode_minus_noJC.csv")
```

```{r, eval=T}
## Write nonJC but only sample name decoding information and mixed infection status
pf_all_id_decode_only_noJC <- pf_all_id_decode %>% filter(str_detect(sample_nm, "noJC"))

write_csv(pf_all_id_decode_only_noJC, "data/raw/pf_all_id_decode_only_noJC.csv")
```
