...
```{r}
source('mali22_code_lnk/souporcell/m2_strain_decon_all_headers/m2_strain_decon_all_resc_preQC_JC_header.R')
# source('/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/mali22_code/souporcell/m2_strain_decon_all_GE_postQC_header.R')
# source('mali22_code_lnk/souporcell/m2_strain_decon_all_headers/m2_strain_decon_all_resc_preQC_header.R')
# source('mali22_code_lnk/souporcell/m2_strain_decon_all_headers/m2_strain_decon_all_resc_ppQC_header.R')
# source('mali22_code_lnk/souporcell/m2_strain_decon_all_resc_preQC_header.R')
# source('mali22_code_lnk/souporcell/m2_strain_decon_all_preQC_header_m55_m60.R')
# source('mali22_code_lnk/souporcell/m2_strain_decon_all_preQC_header.R')
```
...
```{r}
##Transfer stage labels and other metadata from farm
# # all_ed_pref_w_mdata_cb_mdata <- list.files("data/processed/Pf", pattern = "all_ed_pref_w_mdata_cbendr_mdata.RDS", full.names = T, recursive = T, include.dirs = T) %>% 
# all_seu_prelim_stg_norm_cb_mdata <- list.files("data/processed/Pf", pattern = "all_seu_prelim_stg_norm_cb_mdata.csv", full.names = T, recursive = T, include.dirs = T) %>%
#   set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|all_seu_prelim_stg_norm_cb_mdata.csv'))) %>%
#   .[sample_name_nms] %>%
#   map(read_csv)
# 
# msc_mdata <- map(all_seu_prelim_stg_norm_cb_mdata, ~.x %>% 
#                                       rownames_to_column('bcode') %>%
#                    mutate(StagePrelim = coalesce(str_remove_all(fine_stg, "\\(|\\)")))
#                    # mutate(StagePrelim = coalesce(StagePrelim, str_remove_all(fine_stg, "\\(|\\)")))
#                    )
# 
...
```{r}
##Transfer stage labels and other metadata from farm
all_seu_prelim_stg_norm_cb_mdata <- readRDS(paste0("data/processed/", species, "/m2_all/all_seu_prelim_stg_norm_cb_mdata.RDS"))


msc_mdata <- map(all_seu_prelim_stg_norm_cb_mdata, ~.x %>% 
                   mutate(StagePrelim = sR_fine_stg)
                   # mutate(StagePrelim = coalesce(StagePrelim, str_remove_all(fine_stg, "\\(|\\)")))
                   )
```
