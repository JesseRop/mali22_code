...
// Other parameters
params.sufx = "v3_m2_asex_harmony_fld_sshot.RDS"
...
input_ch = Channel
				...
				.map { file -> tuple(file.getParent().toString().split('\\/')[13], file, file.getParent(), file.baseName+ "_" + "${params.sufx}" + ".RDS", file.baseName.split('_')[-2], file.baseName.split('_')[-1]) }
...
process SEU_NORMALIZATION {
    ...
    input:
    tuple val(sample_nm), path(seu), val(out_p), val(nm), val(s_clstr), val(e_clstr) 
	
    output:
    // tuple val(sample_nm), path("${params.o_file_name}")
	tuple val(sample_nm), path(nm)

	script:
	"""
    module load HGI/softpack/groups/team222/Pf_scRNAseq/19

    Rscript ${params.scrpt} ${seu} ${nm} ${params.umap_nm} ${params.clstr} ${s_clstr}  ${e_clstr} ${params.redctn} ${params.pca_nm}
