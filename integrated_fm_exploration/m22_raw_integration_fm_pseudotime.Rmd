...
fm_leiden_w_donors_norm_harmony_jnd <- fm_leiden_w_donors_norm_harmony_jnd[c("all.fLE", "all.m_mLE", "all.m", "w.5.f_fLE", "w.5.f", "w.7.f", "w.5.f_no_asx_dbt", "w.7.m")]
...
```{r, warning=FALSE, message=FALSE}
map(fm_leiden_w_donors_norm_harmony_jnd, ~DimPlot(.x, label = T, reduction = "umap.harmony_PC_man", dims = c(1,2), group.by = "seurat_clusters")) 
map(fm_leiden_w_donors_norm_harmony_jnd, ~DimPlot(.x, label = T, reduction = "umap.harmony_PC_man", group.by = "leiden")) 

```

```{r, warning=FALSE, message=FALSE}
## Remove the committed cluster
...
```{r, warning=FALSE, message=FALSE}
map(fm_leiden_w_donors_norm_harmony_jnd, ~DimPlot(.x, label = T, reduction = "umap.harmony_PC_man", dims = c(1,3), group.by = "leiden")) 
...
```{r, warning=FALSE, message=FALSE}
resolns <- c(0.07, rep(0.08, length(fm_leiden_w_donors_norm_harmony_jnd)-2), 0.1)
...
fm_leiden_w_donors_norm_harmony_jnd[['w_pt5_f_fLE']] <- subset(fm_leiden_w_donors_norm_harmony_jnd[['w_pt5_f_fLE']], subset = harmony_clusters_ss %in% c(0, 2))
...
start_clusters <- c(1, 0, 0, 2, 1, 0, 1, 0)
end_clusters <- c(0, 1, 1, 0, 0, 1, 0, 1)
