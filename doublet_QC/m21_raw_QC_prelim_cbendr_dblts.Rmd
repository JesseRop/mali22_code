...
all_seu_filtOlMtCb_sct <- list.files("data/processed/Pf", pattern = "all_seu_filtOlMtCb_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|all_seu_filtOlMtCb_sct.RDS'))) %>% 
  # .[sample_name_all] %>%
  .[sample_name] %>%
  map(readRDS)
```
...
Remove any left over weird clusters 

```{r, warning=FALSE, message=FALSE, fig.width=14, fig.height= 4}
# dbt_methods <- c(paste0(c("scf_class"), c("_norm", "_rand")), paste0(c("scf_class"), c("_supx_norm", "_supx_rand")), "scrub_doublet", "strain_gt")
map(all_msc_cb_filt_dbt, ~.x@meta.data %>% count(orig.ident, seurat_clusters, background_fraction_cat_poorQc_prop, sR_fine_stg_cat_poorQc_prop, asex_cat_poorQc_prop) %>% filter(if_all(c(background_fraction_cat_poorQc_prop, sR_fine_stg_cat_poorQc_prop), ~. > 0.1)))
```

```{r, warning=FALSE, message=FALSE, fig.width=4, fig.height=4.5}
## in cellbender but not in edrops
plts_poorStgHighBgProp_dbts <- imap(all_msc_cb_filt_dbt, possibly(~{
  DimPlot(.x, label = T, 
...
```{r, warning=FALSE, message=FALSE, fig.height= 14, fig.width= 10, eval=T}
plot_grid(plotlist = plts_poorStgHighBgProp_dbts, ncol = 4)
```


```{r}
##Seurat object
##Remove cluster with high proportion of cells with poor stage annotation 

all_msc_cb_filt_dbt_filtBgStg <- all_msc_cb_filt_dbt %>%
    map(. %>% subset(., subset = background_fraction_cat_poorQc_prop <= 0.1 | sR_fine_stg_cat_poorQc_prop <= 0.1 ))
    
filtBgStg <- map(all_msc_cb_filt_dbt_filtBgStg, dim) %>% as.data.frame() %>% .[2,] #%>% t()
filtBgStg 
```
...
```{r, warning=FALSE, message=FALSE, fig.width=14, fig.height= 4}
# dbt_methods <- c(paste0(c("scf_class"), c("_norm", "_rand")), paste0(c("scf_class"), c("_supx_norm", "_supx_rand")), "scrub_doublet", "strain_gt")
map(all_msc_cb_filt_dbt_filtBgStg["MSC55"], ~.x@meta.data %>% count(orig.ident, seurat_clusters, background_fraction_cat_poorQc_prop, sR_fine_stg_cat_poorQc_prop, asex_cat_poorQc_prop) %>% filter(background_fraction_cat_poorQc_prop> 0.1))
```

```{r, warning=FALSE, message=FALSE, fig.width=4, fig.height=4.5}
## in cellbender but not in edrops
plts_bg4_dbts <- imap(all_msc_cb_filt_dbt_filtBgStg, possibly(~{
  DimPlot(.x, label = T, 
          cells.highlight =  .x@meta.data %>% rownames_to_column('bcode') %>% filter(background_fraction_cat_poorQc_prop > 0.45)  %>% pull(bcode), 
          sizes.highlight = 0.05) + 
    theme_void() + theme(legend.position = "none", panel.border = element_rect(color = "black", fill = NA, size = 1))  +
    labs(title = .y)}, "no cells"))
...
```{r, warning=FALSE, message=FALSE, fig.height= 14, fig.width= 10, eval=T}
plot_grid(plotlist = plts_bg4_dbts, ncol = 4)
```

Limit removal of clusters with high background to MSC55

```{r}
##Seurat object
##Remove cluster with high proportion of cells with poor stage annotation 

all_msc_cb_filt_dbt_filtBgStg["MSC55"] <- subset(all_msc_cb_filt_dbt[["MSC55"]], subset = background_fraction_cat_poorQc_prop < 0.1 )
all_msc_cb_filt_dbt_filtBgStg["MSC55C"] <- subset(all_msc_cb_filt_dbt[["MSC55C"]], subset = background_fraction_cat_poorQc_prop < 0.1 )
all_msc_cb_filt_dbt_filtBgStg["MSC55noJC"] <- subset(all_msc_cb_filt_dbt[["MSC55noJC"]], subset = background_fraction_cat_poorQc_prop < 0.1 )
...
```{r}
rm(all_msc_cb_filt_dbt)
```


```{r, warning=FALSE, message=FALSE, fig.width=14, fig.height= 4}
# dbt_methods <- c(paste0(c("scf_class"), c("_norm", "_rand")), paste0(c("scf_class"), c("_supx_norm", "_supx_rand")), "scrub_doublet", "strain_gt")
map(all_msc_cb_filt_dbt_filtBgStg, ~.x@meta.data %>% count(orig.ident, seurat_clusters, background_fraction_cat_poorQc_prop, sR_fine_stg_cat_poorQc_prop, asex_cat_poorQc_prop) %>% filter(background_fraction_cat_poorQc_prop> 0.45))
```

```{r}
all_msc_cb_no_dbt <- all_msc_cb_filt_dbt_filtBgStg
rm(all_msc_cb_filt_dbt_filtBgStg)
```
...
imap(all_msc_cb_no_dbt[c("MSC55", "MSC55C", "MSC55noJC")], ~{
  ...
  SeuratDisk::SaveH5Seurat(obj, filename =  paste0("data/processed/Pf/", .y, "/all_msc_cb_no_dbt.h5Seurat"), overwrite = T)

  Convert(paste0("data/processed/Pf/", .y, "/all_msc_cb_no_dbt.h5Seurat"), dest = "h5ad", overwrite = T)
...
all_msc_cb_no_dbt_mrg <- merge(x = all_msc_cb_no_dbt[[1]],
                             y = all_msc_cb_no_dbt[2:length(all_msc_cb_no_dbt)],
                             add.cell.ids = str_remove(names(all_msc_cb_no_dbt), "SC"))
  
```

```{r, eval=FALSE}
rm(all_msc_cb_no_dbt)
```
