...
scf_outputs <- list.files("data/processed/Pf", pattern = "^all_seu_filtOlMtCb3_sct_scdblf.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*all_seu_filtOlMtCb.*'))) %>% 
  .[sample_name] %>%
  map(readRDS)
```
...
```{r}
imap(all_seu_filtOlMtCb_w_dblt, ~.x@meta.data  %>% 
      count(strain_gt == "Doublet") %>%
       rename(!!.y := n))
...
```{r}
#
map(all_filt_cb_no_StgDblts, ~.x@meta.data  %>% count(strain_dbt_dbt_prop < 0.40))
...
```{r, eval=T}
all_msc_cb_no_dbt <- all_msc_cb_filt_dbt
...
imap(all_msc_cb_filt_dbt, ~saveRDS(.x, paste0("data/processed/Pf/",.y,"/seu_obj/all_msc_cb_no_dbt3.RDS")))
