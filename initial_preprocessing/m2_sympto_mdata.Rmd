...
## Remove MSC1 & MSC24 since these are also poorQC and MSC25, MSC41 and MSC51 which are mixed
qcd_dons_nc <- qcd_dons_nc[!qcd_dons_nc %in% c("MSC1", "MSC24", "MSC25", "MSC41", "MSC51")] %>% unique()
```
...
donor_mdata_dtld_symptoms <- donor_mdata_dtld_symptoms %>% rename_with(.fn = ~str_to_lower(str_replace_all(str_remove(., " \\(.*"), c(" "= "_", "\\[|\\]"="", "/"="_", "º"=""))), .cols = everything())
...
```{r}
table1_cln_nms <- map(table1, possibly(~.x %>% mutate(Variable = recode(Variable, !!!table1_key)), "less than 2 categories for chisquare"))
