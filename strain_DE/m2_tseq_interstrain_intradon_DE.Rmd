...
DimPlot(v3_m2_asex_harmony_ss_res_fld_pt, reduction = "umap.harmony", group.by = "harmony_clusters_ss", pt.size = 0.5, dims = c(1,2), label = T)

```

```{r, fig.width=30, fig.height=6}
FeaturePlot(v3_m2_asex_harmony_ss_res_fld_pt, features = "PF3D7-0925700", reduction = "umap.harmony", split.by = "donor", pt.size = 2, ncol = 7)
```
...
######### Adding missing metadata ######### ######### ######### 
```{r}
donor_mdata_slct <- readRDS("data/processed/Pf/m2_all/donor_mdata_slct.RDS") %>% mutate(status = condition)
...
#   mutate(dsetC = ifelse(donor == "MSC12", "FieldC", dset)) %>% 
#   left_join(., donor_mdata_slct, by = c("donor_nm" = "short_name")) %>%
...
v3_m2_asex_harmony_ss_res_fld_pt_mxdSp <- v3_m2_asex_harmony_ss_res_fld_pt@meta.data %>% 
  rownames_to_column("bcode") %>% 
  mutate(donor_nm = str_remove_all(donor, "_SLO|_MACS")) %>% 
  mutate(dsetC = ifelse(donor == "MSC12" & dset == "Field", "FieldC", dset)) %>% 
  left_join(., donor_mdata_slct, by = c("donor_nm" = "short_name")) %>%
  select(donor, bcode, status, Hb_Type, dsetC, donor_nm) %>%
  left_join(., mrc_sk21_summary_mxd, by = "donor") %>%
  mutate(Hb_Type = coalesce(Hb_Type.x, Hb_Type.y)) %>% 
  select(bcode, status, Hb_Type, dsetC, donor_nm) %>% 
  column_to_rownames("bcode") %>% 
  AddMetaData(v3_m2_asex_harmony_ss_res_fld_pt, .)
```
...
!!!!!NOTE Remove mixed infections for now
```{r}
v3_m2_asex_harmony_ss_res_fld_pt_mxdSp <- v3_m2_asex_harmony_ss_res_fld_pt_mxdSp[,!(v3_m2_asex_harmony_ss_res_fld_pt_mxdSp$donor %in% c("MSC25" ,"MSC41", "MSC51"))]
...
```{r}
v3_m2_asex_harmony_ss_res_fld_pt <- v3_m2_asex_harmony_ss_res_fld_pt_mxdSp
```


```{r}
st_dist <- v3_m2_asex_harmony_ss_res_fld_pt@meta.data %>% filter(dset != "Lab" & !(str_detect(StrainDon, "Doublet|Negative"))) %>% count(donor, status, StrainDon) %>% filter(n>20) %>% group_by(donor) %>% filter(n() > 1 )

```

```{r}
st_dist %>% 
  # mutate(across(status, ~str_replace_all(., c("No" = "Asymptomatic", "Yes" = "Symptomatic")))) %>%
  ggplot(aes(x = donor, fill = StrainDon, y = n)) + geom_col(position = position_dodge2()) + facet_wrap(status~., scales = "free_x")
```
...
v3_m2_asex_harmony_sce_ss@colData[,c("status", "Hb_Type", "dsetC")] <- v3_m2_asex_harmony_sce_ss@colData[,c("Hb_Type"), drop = F] %>%
  as.data.frame() %>%
  rownames_to_column("bcode") %>%
  left_join(., v3_m2_asex_harmony_ss_res_fld_pt@meta.data[, c("status", "Hb_Type", "dsetC")]  %>% 
              # mutate(across(Symptomatic, ~str_replace_all(., c("Asymptomatic" = "No", "Symptomatic" = "Yes")))) %>% 
              rownames_to_column("bcode"),
            by = "bcode") %>%
  mutate("Hb_Type" = coalesce(Hb_Type.x, Hb_Type.y)) %>%
  select(status, Hb_Type, dsetC)
...
dons_bal_sset <- c("M49_", "M66_")

dons_bal_sset_nms <- str_remove_all(str_replace_all(dons_bal_sset, "M", "MSC"), "_$|\\|")

m2_asex_sce <- map(dons_bal_sset, ~m2_asex_tr_sce_pt_chunks_sshot[, str_detect(colnames(m2_asex_tr_sce_pt_chunks_sshot),.x) & !str_detect(m2_asex_tr_sce_pt_chunks_sshot@colData$StrainDon, "Negative|Doublet")]) %>% set_names(dons_bal_sset_nms)
...
umap_meta_df <- reducedDims(v3_m2_asex_harmony_sce_ss)$UMAP[,c(1,2)] %>% as.data.frame() %>%rownames_to_column("cell_bc") %>% left_join(., v3_m2_asex_harmony_sce_ss@colData[,c("StagePrelim", "slingPseudotime_1", "orig.ident", "StrainDon", "Symptomatic", "donor", "strain", "dsetC")] %>% as.data.frame() %>% rownames_to_column("cell_bc"), by = "cell_bc") #%>% 
...
strn_comps = list(c("M49_SC1", "M49_SC5"), c("M66_SC5", "M66_SC7"))
...
umap_meta_df_49 <- reducedDims(v3_m2_asex_harmony_sce_ss)$UMAP[,c(1,2)] %>% as.data.frame() %>%rownames_to_column("cell_bc") %>% left_join(., v3_m2_asex_harmony_sce_ss@colData[,c("StagePrelim", "slingPseudotime_1", "orig.ident", "StrainDon", "Symptomatic", "donor", "strain", "stage")] %>% as.data.frame() %>% rownames_to_column("cell_bc"), by = "cell_bc") %>% filter(donor == "MSC49")  #%>% 
# mutate(across(strain_dset, ~factor(., levels = rev(strain_dset_lvls_don)))) %>% arrange(strain_dset
...
tbl = map(m2_asex_bal_sce["MSC49"], ~umap_meta_df_49[umap_meta_df_49$cell_bc %in% colnames(.x),])
all_cells_tbl = map(names(m2_asex_bal_sce["MSC49"]), ~umap_meta_df_49[umap_meta_df_49$donor == .x, ])
...
# tbl = map(m2_asex_bal_sce["MSC49"], ~umap_meta_df_49[umap_meta_df_49$cell_bc %in% colnames(.x),])
# all_cells_tbl = map(names(m2_asex_bal_sce["MSC49"]), ~umap_meta_df_49[umap_meta_df_49$donor == .x, ])
all_cells_tbl_m49 = map(m2_asex_bal_sce["MSC49"], ~umap_meta_df_49[umap_meta_df_49$cell_bc %in% colnames(.x),])
...
# DimPlot(m2_asex_harmony_tr_pt_sset, reduction = "umap.harmony", group.by = "stage", split.by = "Symptomatic", pt.size = 0.1)+
#   scale_color_manual(values = sp_fld_colors) +
#   theme(axis.text=element_blank(), 
#         axis.title=element_blank(),
#         legend.position = "bottom",
...
# 
# FeaturePlot(v3_m2_asex_harmony_ss_res_fld_pt_bal, reduction = "umap.harmony", features = "pseudotime", pt.size = 0.5, dims = c(1,2), split.by = "sympto ")
...
# 
# DimPlot(v3_m2_asex_harmony_ss_res_fld_pt_bal, reduction = "umap.harmony", group.by = "ptime_cat", pt.size = 0.5, dims = c(1,2), label = T, split.by = "Symptomatic")
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_tseq_condtn.nf --w_dir "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/*/tradeseq/m2_asex_bal" --scrpt /nfs/users/nfs_j/jr35/multipurpose_scripts/tradeseq_nf.R --cond_tbl "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/cond_tbl.csv" --resume
...
tde_smooth_plt(tde_obj_lst = asex_bal_ts[str_detect(names(asex_bal_ts), "MSC60")],
               cond_obj_lst = cond_gns_de_asx[str_detect(names(cond_gns_de_asx), "MSC60")],
...
# print(str(gns))
##Edited from the original version to allow the labelling of rows with gene names
msc_yhatSmooth_w_annot <- predictSmooth(asex_bal_ts[["MSC49"]], gene = gns, nPoints = 30, tidy = FALSE) %>% 
  t() %>%scale() %>% t() %>% as.data.frame()%>% 
  rownames_to_column('gene_id') %>% 
  left_join(., Pf3D7_genes_plasmoDB  %>% dplyr::select("Gene", "gene_id", "Ka_Ks"), by = 'gene_id') %>% 
...
# print(str(msc_yhatSmooth))
msc_col_annot <- predictSmooth(asex_bal_ts[["MSC49"]], gene = gns, nPoints = 30, tidy = T)  %>% 
  mutate(point = paste0("point", rep(rep(1:30,length(unique(asex_bal_ts[["MSC49"]]@colData$tradeSeq$conditions))),length(gns))), 
         lin_id = paste0('lineage', lineage,'_condition', condition, '_',point)) %>%
  rownames_to_column('No')  %>% 
  dplyr::select(lin_id,time,condition)%>% 
  distinct()%>% 
...
# print(str(gns))
##Edited from the original version to allow the labelling of rows with gene names
msc_yhatSmooth_w_annot <- predictSmooth(asex_bal_ts[["MSC66"]], gene = gns, nPoints = 30, tidy = FALSE) %>% 
  t() %>%scale() %>% t() %>% as.data.frame()%>% 
  rownames_to_column('gene_id') %>% 
  left_join(., Pf3D7_genes_plasmoDB  %>% dplyr::select("Gene", "gene_id", "Ka_Ks"), by = 'gene_id') %>% 
  left_join(., gene_annot, by = 'gene_id') %>% mutate(across(c("VAR"), ~case_when(is.na(.) ~ "NV", T ~ .))) %>%
...
# print(str(msc_yhatSmooth))
msc_col_annot <- predictSmooth(asex_bal_ts[["MSC66"]], gene = gns, nPoints = 30, tidy = T)  %>% 
  mutate(point = paste0("point", rep(rep(1:30,length(unique(asex_bal_ts[["MSC49"]]@colData$tradeSeq$conditions))),length(gns))), 
         lin_id = paste0('lineage', lineage,'_condition', condition, '_',point)) %>%
  rownames_to_column('No')  %>% 
  dplyr::select(lin_id,time,condition)%>% 
...
### Annotation groups for the header ie column
# gns = cond_gns_de_asx[["MSC49"]] %>% filter(pvalue < 0.05) %>% arrange(desc(waldStat)) %>%   mutate(wstat_rank = rank(-waldStat)) %>% filter(., wstat_rank < 20 | str_detect(Gene, "VAR")) %>% pull(gene_id)
gns = cond_gns_de_asx[["MSC49"]] %>% filter(pvalue < 0.05) %>% arrange(desc(waldStat)) %>%
  mutate(wstat_rank = rank(-waldStat)) %>%
  # filter(., wstat_rank < 20 ) %>%
  pull(gene_id)
...
# overlap_gns <- cond_gns_de_asx[["MSC49"]] %>% filter(pvalue<0.05 & !is.na(gene_id)) %>% inner_join(cond_gns_de_asx[["MSC60"]][cond_gns_de_asx[["MSC60"]]$pvalue < 0.05,], by = "gene_id") 
overlap_gns <- cond_gns_de_asx[["MSC49"]] %>% filter(pvalue<0.05 & !is.na(gene_id)) %>% inner_join(cond_gns_de_asx[["MSC66"]][cond_gns_de_asx[["MSC66"]]$pvalue < 0.05,], by = "gene_id") 
...
tde_smooth_plt(tde_obj_lst = asex_bal_ts[str_detect(names(asex_bal_ts), "MSC49")],
               cond_obj_lst = map(cond_gns_de_asx[str_detect(names(cond_gns_de_asx), "MSC49")], ~.x %>% filter(gene_id %in% overlap_gns$gene_id)),
               pw_comp= 'waldStat_M49_SC1_vs_M49_SC5',
               n_gns = c(1:4)
)
...
Remove lab and MSC12 and those lacking symptomatic status
```{r}
...
```{r}
## Adjusting resolution to find the optimum for slingshot trajectory inference to avoid error  "values must be length 150, but FUN(X[[1]]) result is length 38689"
FeaturePlot(v3_m2_asex_harmony_ss_res_fld_pt, features = "scf_class_rand_dbt_prop", reduction = "umap.harmony", order = T)
...
```{r, fig.width=6, fig.height=3}
## 
VlnPlot(v3_m2_asex_harmony_ss_res_fld_pt, features = c("nFeature_RNA", "nCount_RNA"), ncol = 2, split.by = "StagePrelim", pt.size = 0)
...
```{r, fig.width=6, fig.height=3}
## 
DimPlot(v3_m2_asex_harmony_ss_res_fld_pt,  split.by = "dset", reduction = "umap.harmony", dims = c(2,3))
...
DimPlot(v3_m2_asex_harmony_ss_res_fld_pt, reduction = "umap.harmony",cells.highlight = cbs, pt.size = 0.5, dims = c(1,2), label = F)
