...
// Hypervariable genes ie features
params.hvg = "500"
...
filt_ch = Channel
				...
				.map { file -> tuple(file.getParent().toString().split('\\/')[13], file.baseName, file, file.getParent(), file.baseName.replaceAll(/.RDS/, "") + "_" + "${params.o_sufx}" + ".RDS") }
...
process SCDBLFINDER_RUNS {
    ...
    tuple val(sample_nm), val(obj_nm), path(filt), val(out_p), val(o_file_name)
    ...
    Rscript ${params.scrpt} ${filt} ${o_file_name} ${params.hvg}
...
workflow {
    ...
    sdblf_ch = SCDBLFINDER_RUNS(filt_ch)
    sdblf_ch.view()
