...
```{r}
# source('mali22_code_lnk/local_rmd_nbooks/m2_strain_decon_all_GE_postQC_header.R')
...
all_msc_anotd_preQC <- readRDS("data/processed/Pf/m2_raw_anotd_preQC.RDS")
all_msc_anotd_preQC <- all_msc_anotd_preQC %>% .[sample_name_nms]
```
...
```{r, warning=FALSE, message=FALSE}
##Seurat UMAP
imap(all_msc_anotd_preQC, ~DimPlot(.x, dims = c(1,2), group.by = "StagePrelim", label = T)+ labs(title = .y))
# imap(all_msc_anotd_preQC, ~DimPlot(.x, dims = c(1,2), group.by = "StagePrelim", label = T)+ labs(title = .y))
```
...
```{r, warning=FALSE, message=FALSE}
#ap2g
imap(all_msc_anotd_preQC, ~FeaturePlot(.x, dims = c(1,2), features = "PF3D7-1222600", reduction = "umap", order = T, slot = "counts")+ labs(title = .y))

```

```{r, fig.height = 15, fig.width = 15}

gn_lc <- rep(600, length(sample_name_nms))
ugn_lc <- rep(list(NULL), length(sample_name_nms))
nc_lst <- rep("nCount_RNA", length(sample_name_nms))
...
vp_fn <- function(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, ugnc_lst= ugn_lc, seu_thresh = 1000, ftr = nc_lst, grp = "StagePrelimC", ptsz = 0.01, nrws = ceiling(length(all_msc_anotd_preQC)/5)) {
  ...
  plot_grid(plotlist = ., labels = sample_name_nms, nrow = nrws) 
}
...
```{r, fig.height = 15, fig.width = 15}
gn_lc <- rep(300, length(sample_name_nms))
nf_lst <- rep("nFeature_RNA", length(sample_name_nms))
...
gn_lc <- rep(550, length(sample_name_nms))
...
rna_lc <- rep(600, length(sample_name_nms))
# rna_uc <- c(7500, 13000, 24000, 40000, 17000)
rna_uc <- rep(10000, length(sample_name_nms))
...
qc_cutoffs <- pmap(list(all_msc_anotd_preQC, rep('nFeature_RNA', length(sample_name_nms)), rep('nCount_RNA', length(sample_name_nms))), ~{
...
md_don <- map(all_msc_anotd_preQC, ~.@meta.data #%>% 
  ...
  bind_rows(., .id = 'donor') %>%
  mutate(across(donor, ~factor(., levels = as.character(sample_name_nms))))
...
list(all_msc_anotd_preQC,  sample_name_nms, gh_df_v[[c("rna_lci")]] , gh_df_v[[c("rna_uci")]], gh_df_v[[c("gn_lci")]], gh_df_v[[c("gn_uci")]], names(all_msc_anotd_preQC))  %>% 
  purrr::pmap(.,~{
...
# mt_filt <-list(rep(0.4, length(sample_name_nms)))
...
# mt_filt <- c("MSC33" = 0.5, "MSC48" = 0.25, "MSC49" = 0.4, "MSC55" = 0.4, "MSC60" = 0.3,  "MSC66" = 0.4, "MSC38" = 0.4, "MSC40" = 0.5, "MSC50_MACS" = 0.6, "MSC53" = 0.4, "MSC57" = 0.4, "MSC24" = 0.6, "MSC37" = 0.2, "MSC39" = 0.2, "MSC45" = 0.25, "MSC50_SLO" = 0.6, "MSC54" = 0.4, "MSC67" = 0.4, "MSC68" = 0.2, "MSC70" = 0.25, "MSC41" = 0.4, "MSC51" = 0.4, "MSC25" = 0.4)
mt_filt1 <- unlist(map(all_msc_anotd_preQC, ~quantile(.x@meta.data[, "percent.mt"], 0.995, na.rm = T)))
```
...
```{r, results='asis'}
# cat('## Visualize field parasites {.tabset}   \n')
list(all_msc_anotd_preQC, mt_filt1, sample_name_nms, names(all_msc_anotd_preQC))  %>% 
  purrr::pmap(.,~{
...
raw_umap <- list(all_msc_anotd_preQC, sample_name_nms)  %>%
  purrr::pmap(.,  ~ {
    # create tabset for each group
    cat('####', ..2, '   \n')
...
all_msc_anotd_preQC <- pmap(list(all_msc_anotd_preQC, rep('nFeature_RNA', length(sample_name_nms)), rep('nCount_RNA', length(sample_name_nms))), ~{
  ..1@meta.data %>%
...
```{r, results='asis'}
# strn_knee_p <- list(all_msc_anotd_preQC, sample_name_nms, 1:length(all_msc_anotd_preQC))  %>% 
#   purrr::pmap(.,~{
#     pg <- .@meta.data  %>% 
#       group_by(strain_gt) %>% 
...
# imap(all_msc_anotd_preQC, ~saveRDS(.x, paste0("data/processed/Pf/",.y,"/seu_obj/seu_norm_anotd_edrops_raw.RDS")))
imap(all_msc_anotd_preQC, ~saveRDS(.x, paste0("data/processed/Pf/",str_extract(.y, "MSC[0-9]+"),"/seu_obj/seu_norm_anotd", str_remove(.y, "MSC[0-9]+"), ".RDS")))
```
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_sctnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/{MSC48,MSC68,MSC14}/seu_obj/seu_norm_anotd{_edrops100,}.RDS" --normlzn "sct" --umap_name "UMAP_SCT" --hvg 400 --cluster_resoln 0.1 --cl_nm "seurat_sct_clusters" --umap_redctn_nm "umap_sct" --redctn "pca" --integrtd_pc30 "no" --pca_redctn_nm "pca_sct" --resume 
...
all_msc_anotd_ysct <- list.files("data/processed/", pattern = "seu_norm_anotd.*_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed//Pf/|/seu_obj.*seu_norm_anotd|_sct.*'))) %>% 
  .[sample_name_nms] %>%
  map(readRDS)
```
...
imap(all_msc_anotd_ysct, ~DimPlot(.x, dims = c(1,2), label = T, group.by = "seurat_sct_clusters") + labs(title = .y))
...
# imap(all_msc_anotd_ysct, ~DimPlot(.x, dims = c(1,2), label = T, cells.highlight = .x@meta.data %>% 
#                                     # filter(strain_gt == "Doublet") %>% 
#                                     row.names()) + labs(title = .y))
...
imap(all_msc_anotd_ysct, ~DimPlot(.x, dims = c(1,2), label = T, cells.highlight = .x@meta.data %>% filter(nFeature_RNA < 100) %>% row.names()) + labs(title = .y))
...
```{r, results='asis'}
# list(strn_knee_p, sample_name_nms)  %>%
#   purrr::pmap(.,  ~ {
#     # create tabset for each group
#     cat('#####', ..2, '   \n')
#    
...
# md_don_all %>% 
#   mutate(strain_dbt = case_when(str_detect(strain_gt, "SC") ~ "Singlet", T ~ strain_gt)) %>%
#       # mutate(across(sample, ~factor(., levels = unlist(sample_name_nms[1:4]))))  %>% 
...
# list(all_msc_anotd_ysct, sample_name_nms)  %>%
#   purrr::pmap(.,  ~ {
#     # create tabset for each group
#     cat('#####', ..2, '   \n')
#     a_gns = ..1@meta.data%>% 
...
```{r, warning=FALSE, message=FALSE}
#ap2g
imap(all_msc_anotd_ysct, ~FeaturePlot(.x, dims = c(1,2), features = "PF3D7-1222600", reduction = "umap_norm", order = T)+ labs(title = .y))
...
```{r}
# mt_filt <- unlist(map(all_msc_filt, ~quantile(.x@meta.data[, "percent.mt"], 0.995, na.rm = T)))
mt_filt <- unlist(map(all_msc_filt, ~quantile(.x@meta.data[, "percent.mt"], 0.975, na.rm = T)))
# api_filt <- unlist(map(all_msc_filt, ~quantile(.x@meta.data[, "percent.api"], 0.995, na.rm = T)))
...
map(all_msc_filt, ~{
  .x@meta.data %>%
    count(StagePrelim, StagePrelimC)
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_stdnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/seu_obj/filt_seu_edrops_raw.RDS" --normlzn "norm" --umap_name "UMAP_F_NORM" --hvg 500 --cluster_resoln 0.1 --cl_nm "seurat_clusters_nm" --umap_redctn_nm "umap_norm" --redctn "pca" --integrtd_pc30 "no" --pca_redctn_nm "pca" --resume 
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_sctnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/{MSC48,MSC68,MSC14}/seu_obj/filt_seu_raw{_edrops100,}.RDS" --normlzn "sct" --umap_name "UMAP_F_SCT" --hvg 400 --cluster_resoln 0.1 --cl_nm "seurat_clusters_sct" --umap_redctn_nm "umap_sct" --redctn "pca" --integrtd_pc30 "no" --pca_redctn_nm "pca_sct" --resume 

```

```{r, eval=T}
## write out seurat bpcells object files for normalization processing in farm
# imap(all_msc_bp, ~system(paste0("rm data/processed/Pf/",.y,"/seu_obj/filt_seu_norm.RDS")))
```
...
```{r, message=F, warning=F}
##Read BPcells matrices
# all_msc_filt <- list.files("data/processed/", pattern = "^filt_seu_sct_norm.RDS$", full.names = T, recursive = T, include.dirs = T) %>%
# all_msc_filt <- list.files("data/processed/", pattern = "^filt_seu_.*sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>%
#   set_names(nm = (str_remove_all(.,'data/processed//Pf/|/seu_obj.*'))) %>% 
all_msc_filt <- list.files("data/processed/", pattern = "^filt_seu_raw.*_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed//Pf/|/seu_obj.*filt_seu_raw|_sct.*'))) %>% 
  .[sample_name_nms] %>%
  map(readRDS)
```
...
```{r, warning=FALSE, message=FALSE}
imap(all_msc_filt, ~DimPlot(.x, dims = c(1,2), label = T, group.by = "StagePrelimC", pt.size = 1)+ 
      scale_color_manual(values = sp_fld_colors) + labs(title = .y))
```
...
```{r}
saveRDS(all_msc_filt, "data/processed/Pf/all_msc_edrops_raw_filt.RDS")
