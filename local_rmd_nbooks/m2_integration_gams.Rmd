...
FeaturePlot(m2_fm_harmony_int, features = c("PF3D7-0406200", "PF3D7-1469900", "PF3D7-0903800"), pt.size = 1, reduction = "umap.harmony", ncol = 3, slot = "data")
```
...
```{r}
## 
m2_f_harmony_int <- FindClusters(m2_f_harmony_int, resolution = 0.08, cluster.name = "harmony_clusters_ss")

```

```{r}
DimPlot(m2_f_harmony_int, pt.size = 0.4, reduction = "umap.harmony", group.by = "harmony_clusters_ss")
```


```{r}
Idents(m2_f_harmony_int) <- "harmony_clusters_ss"
```


```{r}
m2_fm_harmony_int_jnd <- JoinLayers(m2_fm_harmony_int)
sx_devt_mrks <- FindAllMarkers(m2_fm_harmony_int_jnd)
```
...
```{r, eval=T}
Seurat_object <- JoinLayers(m2_f_harmony_int)
