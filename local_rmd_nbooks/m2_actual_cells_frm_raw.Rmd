...
## Run emptydrops on all 
# nextflow run ../mali22_code_lnk/nf_scripts/edrops.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/edrops.R" --input_raw_mtx "/lustre/scratch126/tol/teams/lawniczak/projects/malaria_single_cell/mali_field_runs/2022/data/cellranger_runs/Pf/5736STDY13782211/outs/raw_feature_bc_matrix.h5" --o_path "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/" --retain_nm "wo_retain,w_retain" --retain_val "Inf,NULL" --limt "10,50,100,150,200" --fdr_l "0.01" -resume
...
```{r}
# sce <- read10xCounts("/lustre/scratch126/tol/teams/lawniczak/projects/malaria_single_cell/mali_field_runs/2022/data/cellranger_runs/Pf/5736STDY13782211/outs/raw_feature_bc_matrix.h5")
# # mito <- which(rowData(sce)$ID=="MIT")
# # api <- which(rowData(sce)$ID=="API")
# # stats <- perCellQCMetrics(sce, subsets=list("mit"=mito, "api"=api))
# # mats1 <- counts(sce)
# # mats1[1:5,1:5]
```
...
# e.out <- emptyDrops(mats, lower = 50, test.ambient = TRUE)
```
...
# sce2 <- read10xCounts("/lustre/scratch126/tol/teams/lawniczak/projects/malaria_single_cell/mali_field_runs/2022/data/cellranger_runs/Pf/5736STDY13782211/outs/raw_feature_bc_matrix/")
# 
# mats3 <- counts(sce2)
# mats3[1:5,1:5]
```
...
```{r}
# identical(mats, mats3)

```

```{r}

# e.out2 <- emptyDrops(counts(sce2), lower = 50, test.ambient = TRUE)
```
...
# slct_sample_names <-c("MSC48", "MSC68", "MSC14", "MSC49", "MSC70") %>% str_sort(., numeric = T)
slct_sample_names <-c("MSC48", "MSC68", "MSC14", "MSC49", "MSC60", "MSC66", "MSC67", "MSC70", "MSC24", "MSC33", "MSC40", "MSC54", "MSC55") %>% str_sort(., numeric = T)
# slct_sample_names <-c("MSC48", "MSC68", "MSC14", "MSC49", "MSC60", "MSC66", "MSC67",  "MSC24", "MSC33", "MSC40", "MSC54", "MSC55") %>% str_sort(., numeric = T)
...
```{r, eval=T}
sda <- all_msc_bp_filt_pf100[["MSC51"]]
DefaultAssay(sda) <- "RNA"

## Convert seurat object to SCE
obj_sce <- SingleCellExperiment(assays = list(counts = as(sda[["RNA"]]$counts, Class = "dgCMatrix")), colData = sda@meta.data)
...
```{r, message=F, warning=F}
##Read BPcells matrices
all_raw_edrops50 <- list.files("data/processed/Pf", pattern = "bpc_seu.*_norm.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|bpc_seu|_norm.*'))) %>% 
# all_raw_edrops50 <- list.files("data/processed/Pf", pattern = "bpc_seu.*_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
...
```{r, message=F, warning=F}
##Read elbo plots
all_raw_edrops50_elbo <- list.files("data/processed/Pf", pattern = "bpc_seu.*_norm_elbo.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|bpc_seu|_norm.*'))) %>% 
# all_raw_edrops50_elbo <- list.files("data/processed/Pf", pattern = "bpc_seu.*_sct_elbo.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
#   set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/|bpc_seu|_sct.*'))) %>% 
  .[sample_name_all] %>%
  map(readRDS)
...
```{r, warning=FALSE, message=FALSE}
## ccp4
imap(all_raw_edrops50, possibly(~{FeaturePlot(.x, features = "PF3D7-1215500", order = T) + labs(title = .y)}, "missing"))
imap(all_raw_edrops50, possibly(~{FeaturePlot(.x, features = "PF3D7-1215500", reduction = "pca", order = T) + labs(title = .y)}, "missing"))
```

```{r, warning=FALSE, message=FALSE}
## remove the _150
all_msc_ed <- all_raw_edrops50[!str_detect(names(all_raw_edrops50), "_150")]
...
```{r, warning=FALSE, message=FALSE}
# run sctransform
# all_msc_sct <- map(all_msc_ed[c("MSC14", "MSC48", "MSC68", "MSC14_edrops50", "MSC48_edrops50", "MSC68_edrops50")], ~SCTransform(.x, verbose = FALSE, variable.features.n = 300))
```

```{r, warning=FALSE, message=FALSE}
# These are now standard steps in the Seurat workflow for visualization and clustering
# all_msc_sct <- map(all_msc_sct, ~RunPCA(.x, verbose = FALSE, npcs = 10))

```

```{r, warning=FALSE, message=FALSE}
# These are now standard steps in the Seurat workflow for visualization and clustering
# all_msc_sct <- map(all_msc_sct, ~RunUMAP(.x, dims = 1:6, verbose = FALSE))
# all_msc_sct <- map(all_msc_sct, ~FindNeighbors(.x, dims = 1:6, verbose = FALSE))
# all_msc_sct <- map(all_msc_sct, ~FindClusters(.x, verbose = FALSE, resolution = 0.1))
```

```{r, warning=FALSE, message=FALSE}
# imap(all_msc_sct, ~DimPlot(.x, label = T, reduction = "pca")+ labs(title = .y))
# imap(all_msc_sct, ~DimPlot(.x, label = T)+ labs(title = .y))
# imap(all_msc_sct, ~FeaturePlot(.x, features = "nFeature_RNA")+ labs(title = .y))
# imap(all_msc_sct, ~FeaturePlot(.x, features = "nCount_RNA")+ labs(title = .y))
# imap(all_msc_sct, ~VlnPlot(.x, features = "nCount_RNA", group.by = "seurat_clusters")+ labs(title = .y))

```

```{r, warning=FALSE, message=FALSE}
## ap2g
# imap(all_msc_sct, ~FeaturePlot(.x, features = "PF3D7-1222600", order = T) + labs(title = .y))
# imap(all_msc_sct, ~FeaturePlot(.x, features = "PF3D7-1222600", reduction = "pca", order = T) + labs(title = .y))
```

```{r, warning=FALSE, message=FALSE}
## ap2g
# imap(all_msc_sct, ~FeaturePlot(.x, features = "PF3D7-1215500", order = T) + labs(title = .y))
# imap(all_msc_sct, ~FeaturePlot(.x, features = "PF3D7-1215500", reduction = "pca", order = T) + labs(title = .y))
```

########END TRY SCT ########

```{r, warning=FALSE, message=FALSE}
imap(all_msc_ed, ~FeaturePlot(.x, features = "nFeature_RNA") + labs(title = .y))
imap(all_msc_ed, ~FeaturePlot(.x, features = "nFeature_RNA", reduction = "pca") + labs(title = .y))
```

```{r, warning=FALSE, message=FALSE}
## Pfs16
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-0406200") + labs(title = .y))
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-0406200", reduction = "pca") + labs(title = .y))
```


```{r, warning=FALSE, message=FALSE}
## sbp1
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-0501300") + labs(title = .y))
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-0501300", reduction = "pca") + labs(title = .y))
```


```{r, warning=FALSE, message=FALSE}
## mget
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-1469900") + labs(title = .y))
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-1469900", reduction = "pca") + labs(title = .y))
```


```{r, warning=FALSE, message=FALSE}
## ccp4
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-0903800") + labs(title = .y))
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-0903800", reduction = "pca") + labs(title = .y))
```

```{r, warning=FALSE, message=FALSE}
## ap2g
imap(all_msc_ed, possibly(~{FeaturePlot(.x, features = "PF3D7-1222600", order = T) + labs(title = .y)}, "missing"))
imap(all_msc_ed, possibly(~{FeaturePlot(.x, features = "PF3D7-1222600", reduction = "pca", order = T) + labs(title = .y)}, "missing"))
```


```{r, warning=FALSE, message=FALSE}
## ap2g
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-1215500", order = T) + labs(title = .y))
imap(all_msc_ed, ~FeaturePlot(.x, features = "PF3D7-1215500", reduction = "pca", order = T) + labs(title = .y))
```


```{r}

ncount_filt <- unlist(map(all_msc_ed, ~quantile(.x@meta.data[, "nCount_RNA"], 0.999, na.rm = T)))
```
...
```{r, results='asis'}
pmap(list(all_msc_ed, ncount_filt, names(all_msc_ed)), ~{
...
```{r}
mt_filt <- unlist(map(all_msc_ed, ~quantile(.x@meta.data[, "percent.mt"], 0.995, na.rm = T)))
# mt_filt <- unlist(map(all_msc_ed, ~quantile(.x@meta.data[, "percent.mt"], 0.99, na.rm = T)))
# api_filt <- unlist(map(all_msc_ed, ~quantile(.x@meta.data[, "percent.api"], 0.995, na.rm = T)))
...
pmap(list(all_msc_ed, mt_filt, names(all_msc_ed)), ~{
...
```{r}
map2(all_msc_ed, mt_filt, ~.@meta.data %>% dplyr::count(percent.mt>.y ))
```
...
# all_msc_cr_dropd <- map2(all_msc_ed[paste0(sample_name, "_edrops50")], all_msc_ed[sample_name], ~.x[,!(colnames(.x) %in% colnames(.y))])

all_ed <- all_msc_ed[c(paste0(sample_name, "_edrops50"), paste0(sample_name, "_cbendr"))]
...
```{r, message=F, warning=F}
## !!! NOTE calculate com_markers_localMac in local MAC and then copy to farm. the singleR on farm is outdated

# com_markers <- getClassicMarkers(
