...
```{r, message=F, warning=F, eval=T}
##Read BPcells matrices
all_msc_raw_mdata <- map(all_msc_bp_raw, ~.x@meta.data %>% 
                           filter(nCount_RNA > 1) %>%
                           rownames_to_column("bcode")  %>%
...
```{r}
all_msc_bp_cbcr <- pmap(list(all_msc_bp_raw, all_msc_bp_cbendr, all_msc_bp_filt_pf100), ~subset(..1, cells = unique(c(colnames(..2), colnames(..3)))))
```
