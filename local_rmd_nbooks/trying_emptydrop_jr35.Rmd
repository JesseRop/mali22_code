...
```{r}
slct_sample_names <-c("MSC48", "MSC68", "MSC14") %>% str_sort(., numeric = T)
...
##Sample IDs
source_irods <- pf_solo_mixed_mdata_decode_slct[,c('sample_nm', 'irods_id')] %>% deframe() %>%.[slct_sample_names]
...
dir <- "/lustre/scratch126/tol/teams/lawniczak/projects/malaria_single_cell/mali_field_runs/2022/data/"
outdir <- "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/"
...
sp_barcodes_dirs_to_show <- sp_bc_raw %>% 
  set_names(nm = (str_remove_all(.,'/lustre/scratch126/tol/teams/lawniczak/projects/malaria_single_cell/mali_field_runs/2022/data/cellranger_matrices/Pf_matrices/|/outs.*'))) %>% 
  .[source_irods] 
...
msc_ids <- names(source_irods)
# sanger_ids <- sanger_ids[sanger_ids %in% source_irods]
