...
```{r, warning=FALSE, message=FALSE}
# ap2-g
FeaturePlot(v3_m2_asex_cr_filt_norm_harmony_jnd, reduction = "umap.harmony_pc_man", label = T, features = "PF3D7-1222600", order = T)
...
```{r, warning=FALSE, message=FALSE}
# pfs16 gexp02
FeaturePlot(v3_m2_asex_cr_filt_norm_harmony_jnd, reduction = "umap.harmony_pc_man", label = T, features = "PF3D7-0406200",  order = T)
FeaturePlot(v3_m2_asex_cr_filt_norm_harmony_jnd, reduction = "umap.harmony_pc_man", label = T, features = "PF3D7-1102500", order = T)

```

```{r, warning=FALSE, message=FALSE}
# gene_count
VlnPlot(v3_m2_asex_cr_filt_norm_harmony_jnd, features = c( "nCount_RNA"), pt.size = 0) 
```

```{r, warning=FALSE, message=FALSE}
# pfs16 gexp02
FeaturePlot(v3_m2_asex_cr_filt_norm_harmony_jnd, reduction = "umap.harmony_pc_man", label = T, features = "nFeature_RNA")
FeaturePlot(subset(v3_m2_asex_cr_filt_norm_harmony_jnd, subset = nCount_RNA <2500), reduction = "umap.harmony_pc_man", label = T, features = "nCount_RNA")

```

```{r, warning=FALSE, message=FALSE}
cb_asex_de_anot %>% filter(cluster %in% c("13") & avg_log2FC >0)
```
...
```{r, warning=FALSE, message=FALSE}
v3_m2_asex_cr_filt_norm_harmony_jnd <- FindClusters(v3_m2_asex_cr_filt_norm_harmony_jnd, resolution = 0.25, cluster.name = "harmony_clusters_coarse")
```


```{r, warning=FALSE, message=FALSE}
DimPlot(v3_m2_asex_cr_filt_norm_harmony_jnd, label = T, reduction = "umap.harmony_pc_man", group.by = "harmony_clusters_coarse")
...
```{r, warning=FALSE, message=FALSE}
cb_asex_de_coarse <- FindAllMarkers(v3_m2_asex_cr_filt_norm_harmony_jnd, assay = "RNA", slot = "data")
```


```{r, warning=FALSE, message=FALSE, fig.width=16, fig.height=3}
DimPlot(v3_m2_asex_cr_filt_norm_harmony_jnd, label = T, reduction = "umap.harmony_pc_man", group.by = "harmony_clusters_coarse", split.by = "donor")
```

```{r, warning=FALSE, message=FALSE, eval=FALSE}
cb_asex_de_coarse <- FindAllMarkers(v3_m2_asex_cr_filt_norm_harmony_jnd, assay = "RNA", slot = "data", test.use = "MAST", latent.vars = "donor")
```



```{r, warning=FALSE, message=FALSE, eval=FALSE}
saveRDS(cb_asex_de_coarse, "data/processed/Pf/cb_asex_de_coarse.RDS")

```


```{r, warning=FALSE, message=FALSE, eval=T}
cb_asex_de_coarse <- readRDS("data/processed/Pf/cb_asex_de_coarse.RDS")

```

```{r, warning=FALSE, message=FALSE}
cb_asex_de_coarse_anot <- cb_asex_de_coarse %>% filter(p_val_adj < 0.05) %>% group_by(cluster) %>% arrange(p_val_adj, .by_group = T) %>% left_join(., Pf3D7_genes_plasmoDB[,c("gene_id", "Gene")], by = c("gene" = "gene_id"))

cb_asex_de_coarse_anot
```


```{r, warning=FALSE, message=FALSE}
cb_asex_de_coarse_anot %>% filter(p_val_adj == 0 & avg_log2FC > 0) %>% arrange(desc(avg_log2FC)) %>% group_by(cluster) %>% slice_max(avg_log2FC, n =10)
```

```{r, warning=FALSE, message=FALSE}
cb_asex_de_coarse_anot %>% filter(cluster == "3" & avg_log2FC >0)
```
