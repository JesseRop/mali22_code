...
## Remove MSC38 & MSC39
# don_rm <- c("MSC37","MSC38","MSC39","MSC1")
don_rm <- c("MSC37","MSC38","MSC39")
sample_name_nms <- sample_name_nms[!(sample_name_nms %in% don_rm)]
...
# sample_name_nms <- c("m2_all")
sample_name_nms_dir <- c("m2_all")

# Get the optimum QCd K
opt_narrow_clusters_nms_dir <-  list(
  "m2_all" = c("final")
) %>% .[sample_name_nms_dir]

optimum_k_dir = opt_narrow_clusters_nms_dir
...
sample_name_k_dir <- map2(sample_name_nms_dir,
                          opt_narrow_clusters_nms_dir,
                      ~ rep(.x, each = length(.y))) %>% unlist()

# ## NOTE
# sample_name_k <- "m2_all"
...
gtyp_step = "pbulk_gtypes_cln_1VCF"
