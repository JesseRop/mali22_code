...
msc_w_cln_strns_mdata <- msc_w_cln_strns %>% 
  ...
  mutate(bcode_don = paste0(donor, "_", bcode),
         ...
         stage_ag = ifelse(str_detect(stage, "ale"), "Gametocytes", "Asexual"))  
...
stage_dist_plot <- msc_w_cln_strns_w_don_mdata %>%
  filter(donor != "M12")%>% 
  mutate(Stage_ag = factor(str_extract(stage_ag, "^."), levels = c("A", "G")),
         ...
         across(stage, droplevels)) %>%
  count(Condition, donor, Stage_ag, stage, .drop = F) %>% 
...
stage_dist_plot +
  ...
  theme(strip.text.y.right = element_text(angle = 360),
    ...
    guides(fill = guide_legend(title = "Stage", title.position = "top", nrow = 2))
...
DimPlot(m2_seu_norm_harmony, reduction = "scvi_umap", group.by = c("stage"), pt.size = 0.01, dims = c(1,2), raster = F) + scale_color_manual(values = sp_fld_colors3)

```

```{r}

DimPlot(m2_seu_norm_harmony, reduction = "scvi_umap", group.by = c("donor"), pt.size = 0.01, dims = c(1,2), raster = F)
