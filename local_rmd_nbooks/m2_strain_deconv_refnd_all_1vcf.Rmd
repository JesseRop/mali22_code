...
gt_ss_afm_p1_bi <-pmap(list(sample_name_k_dir, optimum_k_dir), ~{
  sm <- ..1
  gp <- ..2
...
View locations where we are reading from
```{r, warning=FALSE, message=FALSE, eval=T}
##Read in the genotypes and annotate

# pmap(list(sample_name_k_dir, optimum_k_dir), ~{
#   sm <- ..1
#   gp <- ..2
#   map(grps, ~paste0("data/processed/",species, "/", gtyp_step, "/",algn_nm,"/p1/fb_biallelic.vcf"), paste0("data/processed/",species, "/", gtyp_step, "/",algn_nm,"/p1/fb_biallelic.gds"),  storage.option="ZIP_RA")
# 
# })
...
# gt_ss_afm_p1_bi <-pmap(list(sample_name_k, optimum_k_dir), ~{
#   sm <- ..1
#   gp <- ..2
#   map(grps, ~seqVCF2GDS(paste0("data/processed/",species, "/", gtyp_step, "/",algn_nm,"/p1/fb_biallelic.vcf"), paste0("data/processed/",species, "/", gtyp_step, "/",algn_nm,"/p1/fb_biallelic.gds"),  storage.option="ZIP_RA")
# )
# })
...
vtx_rd <- function(sample_id, grp, opt_k ){
  ...
  alt_mtx <- list.files(paste0("data/processed/",species, "/", sample_id ,"/", gtyp_step, "/", grp, "_k", opt_k, "_", algn_nm,"/p1/vartrix_biallelic/"), pattern = "^[D|SC|G|P|N|MSC].*.mtx", full.names = T, recursive = T, include.dirs = T) %>% 
...
##Read in the alternate allele matrix for all the stg/strn combinations and combine into a list - also shortening the names of the matrices (list items)
# sf <- list.files(paste0("data/processed/",species, "/", "MSC33" ,"/", gtyp_step, "/", "stage_afm_strain", "_k", "8", "_", algn_nm,"/p1/vartrix_biallelic/"), pattern = "^[D|SC|G|P|N|MSC].*.mtx", full.names = T, recursive = T, include.dirs = T) %>% 
...
ss_afm_vtx_mtx <-pmap(list(sample_name_k_dir, optimum_k_dir), ~{
  sm <- ..1
  k <- ..2
...
snps_qc_filts <- function(m14_gt){
  ...
  snp_qc_tbl <- m14_gt %>% filter(filt_reason1 == 'Pass') %>% rename_with( ~str_replace(., '$','_raw'), matches("^(G|D|SC|P_|N|MSC).*(_A|_F|_M|_G|_Lo|_G|blet|tive|Unassigned|fail|[0-9])$")) %>% dplyr::select(chrom, pos, matches('.*_raw$|.*_qc$|.*_sNum$|.*_cProp$|.*_DP$|.*_RefN$|.*_AltN$'), type, filter,VQSLOD,CDS,AC,AN,AF, alleles)   %>% 
      pivot_longer(.,cols = matches('^(G|D|SC|P_|N|MSC).*'), 
                   names_pattern = "(^[G|D|SC|P_|N|MSC].*[A|M|F|G|Lo|t|tive|Unassigned|stgQC_fail|(0-9)])_(alt_sNum|alt_cProp|ref_sNum|ref_cProp|raw|qc|DP|RefN|AltN)", 
...
imap(ss_afm_pf7M_qc, ~system(paste0('mkdir data/processed/',species,'/', str_remove(.y, '\\..*'),'/', sv_dir, '/',str_remove(.y, 'M.*\\.'),'_k', optimum_k_dir[str_remove(.y, '\\..*')],'_', algn_nm,'/p1/snp_qc')))

imap(ss_afm_pf7M_qc, ~write_csv(.x, paste0('data/processed/',species,'/', str_remove(.y, '\\..*'),'/', sv_dir, '/',str_remove(.y, 'M.*\\.'),'_k', optimum_k_dir[str_remove(.y, '\\..*')],'_', algn_nm,'/p1/snp_qc/ss_afm_pf7M_qc.csv')))
