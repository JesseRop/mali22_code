...
bal_df <- ptime_chunk_calc_df_fn(sc_df = v3_m2_asex_cr_filt_norm_harmony_pt@meta.data, ptime = "slingPseudotime_1", chunk_num = 20, p_cat_nm = "ptime_cat", p_bal_cat_nm = "ptime_bal_cat")
```
...
balanced_cells_tbl <- balanced_cells_fn(sc_df = m2_asex_seu@meta.data, dsample_condtn = "Strain_DN" , ptime_strategy = c("donor", "ptime_bal_cat"), multpl = 1, don_yes_no = "no")
```
...
m2_asex_bal_seu@meta.data %>% data.frame() %>% dplyr::count(ptime_bal_cat, StrainDon)
...
```{r}
## subset where field parasites are for slingshot
m2_asex_bal_seu@meta.data %>% count(ptime_bal_cat, donor, Strain_DN)
```
...
```{r}
## subset where field parasites are for slingshot
m2_asex_bal_seu@meta.data %>% count(donor,ptime_bal_cat,  Strain_DN) %>% group_by(donor,ptime_bal_cat) %>% mutate(strn_n = n_distinct(Strain_DN)) %>% filter(strn_n >1)
```


```{r}
## subset where field parasites are for slingshot
m2_asex_bal_seu@meta.data %>% count( donor, Strain_DN) %>% group_by( donor) %>% mutate(strn_n = n_distinct(Strain_DN)) %>% filter(strn_n >1)
```
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=3.5}
imap(likely_cnvs_don, possibly(~FeaturePlot(subset(m2_asex_bal_seu, subset = donor == .y), features = .x, split.by = "Strain_DN", reduction = "umap.harmony_PC_man", order = T)))
```
