...
## sequencing methods
seq_nms = c('soupc', 'soupc_lr')

## Combine donor ids with alignment methods since each donor is ran once per alignment method
sample_name_nms_al <- rep(sample_name_nms, length(seq_nms))
seq_nms_al <- rep(seq_nms, each = length(sample_name_nms))

sm_aln_nms_al <- paste(rep(sample_name_nms, length(seq_nms)),
                       rep(seq_nms, each = length(sample_name_nms)),
                       sep = '_')
...
opt_clusters_nms_aln <- rep(opt_clusters_nms, length(seq_nms)) %>% set_names(sm_aln_nms_al)
...
sorted_names_al <- paste0(sample_name,
                          "_",
                          rep(seq_nms, each = length(sample_name)),
                          "_",
                          opt_clusters)
