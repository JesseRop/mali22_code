...
DimPlot(v3_m2_asex_harmony_ss_res_fld_pt, reduction = "umap.harmony", group.by = "harmony_clusters_asex", pt.size = 0.5, dims = c(1,2), label = T)
...
```{r}
st_dist <- v3_m2_asex_harmony_ss_res_fld_pt@meta.data %>% filter(dset != "Lab" & !(str_detect(StrainDon, "Doublet|Negative"))) %>% count(donor, Symptomatic, StrainDon) %>% filter(n>20) %>% group_by(donor) %>% filter(n() > 1 )
...
```{r}
v3_m2_asex_harmony_ss_res_fld_pt@meta.data %>% filter(dset != "Lab" & !(str_detect(StrainDon, "Doublet|Negative"))) %>% count(donor, Symptomatic, StrainDon) %>% mutate(across(Symptomatic, ~str_replace_all(., c("No" = "Asymptomatic", "Yes" = "Symptomatic")))) %>% ggplot(aes(x = donor, fill = StrainDon, y = n)) + geom_col() 
```


```{r, fig.width= 10, fig.height= 5}
v3_m2_asex_harmony_ss_res_fld_pt@meta.data %>% filter(dset != "Lab" & !(str_detect(StrainDon, "Doublet|Negative"))) %>% count(donor, Symptomatic, StrainDon) %>% filter(n>20) %>% mutate(across(Symptomatic, ~str_replace_all(., c("No" = "Asymptomatic", "Yes" = "Symptomatic")))) %>% ggplot(aes(x = donor, fill = StrainDon, y = n)) + geom_col(position = position_dodge2()) + facet_wrap(Symptomatic~., scales = "free_x") + theme_classic()+ theme(legend.position = "right", axis.text.x = element_text(angle = 45, hjust = 1), text = element_text(size = 20)) 
```
...
umap_meta_df <- reducedDims(v3_m2_asex_harmony_sce_ss)$UMAP[,c(1,2)] %>% as.data.frame() %>%rownames_to_column("cell_bc") %>% left_join(., v3_m2_asex_harmony_sce_ss@colData[,c("StagePrelim", "slingPseudotime_1", "orig.ident", "StrainDon", "Symptomatic", "donor", "strain")] %>% as.data.frame() %>% rownames_to_column("cell_bc"), by = "cell_bc") #%>% 
...
map(c(tbl, all_cells_tbl), ~.x%>%
  ggplot(., aes(x = slingPseudotime_1, col=strain, fill=strain)) + 
...
map(c(tbl, all_cells_tbl), ~.x%>%
  ggboxplot(., y = "slingPseudotime_1",x="strain", col="strain") + 
...
cond_gns_005_ub_de <- pmap(list(asex_bal_de_adj, asex_bal_ts),~{
...
## var genes
tde_smooth_plt(tde_obj_lst = asex_bal_ts[str_detect(names(asex_bal_ts), "MSC49")],
               cond_obj_lst = cond_gns_005_ub_de[str_detect(names(cond_gns_005_ub_de), "MSC49")][[1]] %>% filter(str_detect(Gene, "VAR")) %>% list(),
               pw_comp= 'waldStat_M49_SC1_vs_M49_SC5',
               n_gns = c(1:20),
               wstat = 10,
               pval = 0.05
...
tseq_de_hmap <- function(tseq = asex_bal_ts[[1]], de_tbl , pnts = 30, rnames = F, comp_grps, Pv = 0.05){
...
hmap_plt<- list(asex_bal_ts[1], 
  ...
  # tseq_de_hmap(tseq = asex_bal_ts[[1]], gns= map(cond_gns_005_ub_de[1], ~.x %>% filter(if_any(matches("pvalue"), ~. < 0.05)) %>% pull(gene_id)) %>% flatten() %>% as.character(), pnts = 30, rnames = F, title = "Strain DE")
  #                      
hmap_plt
...
```{r}
### Annotation groups for the header ie column
gns = cond_gns_005_ub_de[[2]] %>% filter(pvalue < 0.05) %>% arrange(desc(waldStat)) %>%   mutate(wstat_rank = rank(-waldStat)) %>% filter(., wstat_rank < 20 | str_detect(Gene, "VAR")) %>% pull(gene_id)

# print(str(gns))
##Edited from the original version to allow the labelling of rows with gene names
msc_yhatSmooth_w_annot <- predictSmooth(asex_bal_ts[[2]], gene = gns, nPoints = 30, tidy = FALSE) %>% 
  t() %>%scale() %>% t() %>% as.data.frame()%>% 
  rownames_to_column('gene_id') %>% 
  left_join(., Pf3D7_genes_plasmoDB  %>% dplyr::select("Gene", "gene_id", "Ka_Ks"), by = 'gene_id') %>% 
...
# print(str(msc_yhatSmooth))
msc_col_annot <- predictSmooth(asex_bal_ts[[2]], gene = gns, nPoints = 30, tidy = T)  %>% 
  mutate(point = paste0("point", rep(rep(1:30,length(unique(asex_bal_ts[[2]]@colData$tradeSeq$conditions))),length(gns))), 
         lin_id = paste0('lineage', lineage,'_condition', condition, '_',point)) %>%
  rownames_to_column('No')  %>% 
  dplyr::select(lin_id,time,condition)%>% 
  distinct()%>% 
...
```{r, fig.height=5.8, fig.width=4.3}
hmap
# c("#b7e577", "#3ce07e")
```
...
overlap_gns <- cond_gns_005_ub_de[["MSC49"]] %>% filter(pvalue<0.05 & !is.na(gene_id)) %>% inner_join(cond_gns_005_ub_de[["MSC60"]][cond_gns_005_ub_de[["MSC60"]]$pvalue < 0.05,], by = "gene_id") 
```
