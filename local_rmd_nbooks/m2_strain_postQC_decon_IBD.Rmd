...
ss_afm_pf7M_qc <- readRDS(paste0("data/processed/",species, "/" , sv_dir, "/ss_afm_pf7M_qc.RDS")) %>% .[str_detect(names(.), paste(paste0(sample_name_nms, ".st"), collapse = "|"))]
ss_afm_pf7M_cln_gt <- readRDS(paste0("data/processed/",species, "/" , sv_dir, "/ss_afm_pf7M_cln_gt.RDS")) %>% .[str_detect(names(.), paste(paste0(sample_name_nms, ".st"), collapse = "|"))]
...
ss_cln_gt_tbl_ag <- ss_afm_pf7M_cln_gt[c(seq(2,length(ss_afm_pf7M_cln_gt), length(grps)))] %>%
    imap(~ rename_with(.x, function(x) paste(.y, x, sep = "_"), -c(id,chrom, pos, alleles, chr))) %>%
    purrr::reduce(left_join, by = c('id','chrom', 'pos', 'alleles', 'chr')) %>%
...
ss_cln_gt_tbl_s <- ss_afm_pf7M_cln_gt[c(seq(1,length(ss_afm_pf7M_cln_gt), length(grps)))] %>%
    imap(~ rename_with(.x, function(x) paste(.y, x, sep = "_"), -c(id,chrom, pos, alleles, chr))) %>%
    purrr::reduce(left_join, by = c('id','chrom', 'pos', 'alleles', 'chr')) %>% 
  rename_with(~str_remove_all(., '\\.st.*ain|k[0-9]+_'), starts_with(c("MSC")) )%>% 
  rename_with(~str_replace_all(., c('MSC'='M', '\\.'='_')), starts_with(c("MSC")) )
...
donor_cols = c("#C1EAE5", "#A7B19D", "#73935B", "#669CE2", "#62E1D0", "#DB6F87", "#DC48A2", "#E593D6", "#E65840", "#B29BDB", "#66EA54", "#B8E9B5", "#AFE685", "#D6ACCC", "#76586A", "#D8A46B", "#736AD9", "#62919E", "#65E297", "#E3AE3B", "#DA4CE6", "#BF6BD1", "#6FCEE8", "#8537E1", "#E3E544", "#D4D0E8", "#B0E653", "#E1A797", "#DFDC7F", "#E7E3C4" )[1:length(sample_name_nms)] %>% set_names(str_remove(sample_name_nms, "SC"))
```
...
hms<- pmap(list(gt_all_ibs_mtx, list( clustr_numbers_ag_m1_m2, clustr_numbers_s_m1_m2), c("strain_ag_n", "strain_n" )), ~ibs_don_hm(ibsmtx=..1, clustr_numbers=..2, strn_cat=..3))
...
hms<- pmap(list(gt_all_ibs_mtx_polyc_asx, list( clustr_numbers_ag_m1_m2, clustr_numbers_s_m1_m2), c("strain_ag_n", "strain_n" )), ~ibs_don_hm(ibsmtx=..1, clustr_numbers=..2, strn_cat=..3))
```
...
imap(ss_cln_all_high_n_mtx[2], ~.x %>% dplyr::select(-c(id, chrom, alleles))%>% dplyr::select(chr, pos, any_of(sort(colnames(.)))) %>% filter(!if_all(starts_with('M'), ~is.na(.))) %>% mutate(across(everything(), ~as.numeric(replace_na(., '-1')))) %>% write_tsv(., paste0("data/processed/Pf/ss_ibd_cln_", .y,"_mtx.tsv")))
```

Remove the monoclonal as they need more invesitagation later

```{r}
sample_name_nms_polyc <- sample_name_nms[map(opt_narrow_clusters_nms, ~.x > 1) %>% unlist()] %>% str_remove(., "SC")
# sample_name_nms_monoc <- sample_name_nms[map(opt_narrow_clusters_nms, ~.x == 1) %>% unlist()] 

# ss_cln_all_mtx_polyc <- imap(ss_cln_all_high_n_mtx, ~.x %>% .[!(str_detect(names(.), paste(sample_name_nms_monoc %>% str_remove(., "SC") , collapse = "|")))])
ss_cln_all_mtx_polyc <- imap(ss_cln_all_high_n_mtx, ~.x %>% .[str_detect(names(.), paste(c(sample_name_nms_polyc, "alleles", "chr", "chrom", "id", "pos") , collapse = "|"))])


imap(ss_cln_all_mtx_polyc[2], ~.x %>% dplyr::select(-c(id, chrom, alleles))%>% dplyr::select(chr, pos, any_of(sort(colnames(.)))) %>% filter(!if_all(starts_with('M'), ~is.na(.))) %>% mutate(across(everything(), ~as.numeric(replace_na(., '-1')))) %>% write_tsv(., paste0("data/processed/Pf/ss_ibd_cln_", .y,"_mtx_polyc.tsv")))
```
...
map(names(ss_cln_all_high_n_mtx[2]), ~system(paste0(wdir,'/hmmIBD -i data/processed/Pf/ss_ibd_cln_', .x,'_mtx.tsv -o data/processed/Pf/ss_ibd_cln_', .x,' -m 100')))
map(names(ss_cln_all_high_n_mtx[2]), ~system(paste0(wdir,'/hmmIBD -i data/processed/Pf/ss_ibd_cln_', .x,'_mtx_polyc.tsv -o data/processed/Pf/ss_ibd_cln_polyc_', .x,' -m 100')))
```


```{r}
# read in ibd results
ss_ibd_cln_all_strain_hmmibd <- map(names(ss_cln_all_high_n_mtx[2]), ~read_delim(paste0('data/processed/Pf/ss_ibd_cln_', .x,'.hmm.txt')))
ss_ibd_cln_all_strain_hmmibd_f <- map(names(ss_cln_all_high_n_mtx[2]), ~read_delim(paste0('data/processed/Pf/ss_ibd_cln_', .x,'.hmm_fract.txt')))

ss_ibd_cln_all_strain_hmmibd <- ss_ibd_cln_all_strain_hmmibd[[1]]
ss_ibd_cln_all_strain_hmmibd_f <- ss_ibd_cln_all_strain_hmmibd_f[[1]]
...
```{r}
# read in ibd results
ss_ibd_cln_all_strain_hmmibd_polyc <- map(names(ss_cln_all_high_n_mtx[2]), ~read_delim(paste0('data/processed/Pf/ss_ibd_cln_polyc_', .x,'.hmm.txt')))
ss_ibd_cln_all_strain_hmmibd_f_polyc <- map(names(ss_cln_all_high_n_mtx[2]), ~read_delim(paste0('data/processed/Pf/ss_ibd_cln_polyc_', .x,'.hmm_fract.txt')))

ss_ibd_cln_all_strain_hmmibd_polyc <- ss_ibd_cln_all_strain_hmmibd_polyc[[1]]
ss_ibd_cln_all_strain_hmmibd_f_polyc <- ss_ibd_cln_all_strain_hmmibd_f_polyc[[1]]
...
```{r, fig.height=16, fig.width=20, eval=T}
colFun = suppressWarnings(brewer.pal(100, 'Blues'))
colFun = circlize::colorRamp2(seq(0, 1, length.out = length(colFun)), colFun)
##Merge strain numbers to IBD results
...
# ss_n_ibd_tbl100 = ss_ibd_cln_all_strain_hmmibd_f %>% dplyr::select(sample1, sample2, fract_sites_IBD,N_informative_sites) %>% 
ss_n_ibd_tbl100 = ss_ibd_cln_all_strain_hmmibd_f_polyc %>% dplyr::select(sample1, sample2, fract_sites_IBD,N_informative_sites) %>% 
...
```{r, warning=F}
# strain stage (afm) chromosome painting
# ss_afm_chromp_k1 <- map(ss_afm_wndws[str_detect(names(ss_afm_wndws), paste(names(don_k1), collapse = "|"))], possibly(~{
ss_afm_chromp_k1 <- map(ss_afm_wndws[str_detect(names(ss_afm_wndws), "MSC55")], possibly(~{
  smp_tbl <- .x
  smp_tbl %>% split(., ~factor(comps)) %>% imap(.,possibly(~chrom_paintr(gtypes_comps_tbl =.x, ttl=.y), 'No overlapping SNPs between the 2 groups'))
}, 'No overlapping SNPs between the 2 groups'))
...
k8_ibd_sites_f <- map(list(ss_ibd_cln_all_strain_hmmibd), ~{
  tb1=.x
  list(tb1) %>%
  map(~{
    .x%>% 
