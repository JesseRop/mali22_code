...
sample_challenges <- c("MSC38","MSC39", "MSC55")
...
ss_afm_pf7M_qc <- readRDS("data/processed/Pf/ss_afm_pf7M_qc.RDS") %>% .[str_detect(names(.), paste(sample_name_nms, collapse = "|"))]
ss_afm_pf7M_cln_gt <- readRDS("data/processed/Pf/ss_afm_pf7M_cln_gt.RDS") %>% .[str_detect(names(.), paste(sample_name_nms, collapse = "|"))]
...
clustr_numbers <- map(strn_c_new_mdata_m22_min, ~.x %>% 
        add_count(stage_ag, StrainQCd_Man, name = "strain_ag_n") %>%
          filter(strain_ag_n >= 20) %>%
        dplyr::count(StrainQCd_Man, stage_ag) %>%  
      filter(stage_ag == 'Gametocyte') %>%
        mutate(across(stage_ag, ~str_remove_all(., 'ametocyte|sexual'))) %>%
...
clustr_numbers_ag <- map(strn_c_new_mdata_m22_min, ~.x %>% 
                           add_count(stage_ag, StrainQCd_Man, name = "strain_ag_n") %>%
...
clustr_numbers_s <- map(strn_c_new_mdata_m22_min, ~.x %>% 
                           add_count(stage_ag, StrainQCd_Man, name = "strain_ag_n") %>%
        filter(strain_ag_n >= 20 & !is.na(StrainQCd_Man)) %>%
...
hms<- pmap(list(gt_all_ibs_mtx, list( clustr_numbers_ag, clustr_numbers_s), c("strain_ag_n", "strain_n" )), ~ibs_don_hm(ibsmtx=..1, clustr_numbers=..2, strn_cat=..3))
...
imap(ss_cln_all_mtx, ~.x %>% dplyr::select(-c(id, chrom, alleles))%>% dplyr::select(chr, pos, any_of(sort(colnames(.)))) %>% filter(!if_all(starts_with('M'), ~is.na(.))) %>% mutate(across(everything(), ~as.numeric(replace_na(., '-1')))) %>% write_tsv(., paste0("data/processed/Pf/ss_ibd_cln_", .y,"_mtx.tsv")))
...
map(names(ss_cln_all_mtx), ~system(paste0(wdir,'/hmmIBD -i data/processed/Pf/ss_ibd_cln_', .x,'_mtx.tsv -o data/processed/Pf/ss_ibd_cln_', .x,' -m 100')))
...
ss_ibd_cln_all_strain_hmmibd <- read_delim('data/processed/Pf/ss_ibd_cln_all.strain_qcd.hmm.txt')
ss_ibd_cln_all_strain_hmmibd_f <- read_delim('data/processed/Pf/ss_ibd_cln_all.strain_qcd.hmm_fract.txt')
