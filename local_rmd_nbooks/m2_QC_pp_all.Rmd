...
##Seurat UMAP
map(all_msc_anotd_preQC, ~DimPlot(.x, dims = c(1,2), label = T))
```
...
# 
# all_msc_anotd_preQC <- mcmapply(function(x, y) {
#   FindClusters(.x, resolution = .y, verbose = FALSE, k.param = 30)
# },
# all_msc_anotd_preQC, res_list)
...
```{r, warning=FALSE, message=FALSE}
##Seurat UMAP
map(all_msc_anotd_preQC, ~DimPlot(.x, dims = c(1,2), group.by = "StagePrelim", label = T))
```
...
vp_fn <- function(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, ugnc_lst= ugn_lc, seu_thresh = 1000, ftr = nc_lst, grp = "StagePrelim", ptsz = 0.01, nrws = ceiling(length(all_msc_anotd_preQC)/5)) {
...
```{r, fig.height = 15, fig.width = 15}
# options(repr.plot.width=32, repr.plot.height=7)
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 1000, ftr = nc_lst, grp = "StagePrelim", ptsz = 0.01)
...
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 100000, ftr = nf_lst, grp = "StagePrelim", ptsz = 0)
...
```{r, fig.height = 15, fig.width = 15}
gn_lc <- rep(300, length(sample_id_lst))
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 800, ftr = nf_lst, grp = "StagePrelim", ptsz = 0.01)
...
```{r, fig.height = 15, fig.width = 15}
gn_lc <- rep(600, length(sample_id_lst))
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 1000000, ftr = nc_lst, grp = "StagePrelim", ptsz = 0.0)

```

```{r, fig.height = 15, fig.width = 15}
gn_lc <- rep(550, length(sample_id_lst))
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 2000, ftr = nc_lst, grp = "StagePrelim", ptsz = 0.0)
...
list(all_msc_anotd_preQC,  sample_id_lst, gh_df_v[[c("rna_lci")]] , gh_df_v[[c("rna_uci")]], gh_df_v[[c("gn_lci")]], gh_df_v[[c("gn_uci")]])  %>% 
  purrr::pmap(.,~{
...
## Add annotation
# all_msc_anotd_mdata
# all_msc_anotd_preQC <- list(all_msc_anotd_preQC, all_msc_anotd_mdata) %>%
#   pmap(~ ..2 %>%
#          dplyr::select(Stage_V3, StagePrelim)  %>%
#          AddMetaData(..1, .) 
#        )
# map(all_msc_anotd_preQC, ~..1@meta.data) %>% saveRDS(., "data/processed/DB52e_star/all_msc_anotd_mdata.RDS")
...
```{r}
map(all_msc_anotd_preQC, ~.x@meta.data %>% filter(strain_gt == "Doublet") %>% dplyr::count(StagePrelim, strain_gt))
```
...
map(all_msc_anotd_preQC, ~DimPlot(.x, dims = c(1,2), label = T, cells.highlight = .x@meta.data %>% filter(strain_gt == "Doublet") %>% row.names()))
...
# all_msc_anotd_ysct <- imap(all_msc_anotd_preQC, ~{
...
# all_msc_anotd_ysct <- mcmapply(function(x, y) {
#   obj <- x %>%
#     seurat_SCT() %>%
#     RunUMAP(reduction = "pca", dims = 1:30, verbose = FALSE, seed.use = 372)%>%
#     FindNeighbors(reduction = "pca", dims = 1:30, verbose = FALSE)
...
#     return(obj)
# },
# all_msc_anotd_preQC, names(all_msc_anotd_preQC))
...
# for (i in 32:33) {
  ...
  saveRDS(all_msc_anotd_preQC[[sample_name[i]]],
    paste0('data/processed/Pf/', sample_name[i], '/seu_obj/msc_anotd.RDS')
    )
...
all_msc_anotd_ysct <- list.files("data/processed/", pattern = "anotd_seu_sctnorm.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
...
```{r}
# saveRDS(all_msc_anotd_ysct, "data/processed/Pf/all_msc_anotd_ysct.RDS")
```
...
```{r}
# all_msc_filt <- all_msc_anotd_preQC
```
...
# pc_list <-pc_list
# all_msc_anotd_preQC <- all_msc_anotd_preQC[4] %>%
# all_msc_filt <- all_msc_filt %>%
...
```{r, results='asis'}
# cat('## Visualize field parasites {.tabset}   \n')
list(all_msc_filt, mt_filt, sample_id_lst)  %>% 
  purrr::pmap(.,~{

    FeatureScatter(..1, feature1 = "nCount_RNA", feature2 = "percent.mt", pt.size = 1)+ 
      geom_hline(yintercept = ..2)  +
      theme(axis.text=element_text(size=18), 
            axis.title=element_text(size=18,face="bold"), 
            legend.text = element_text(size=22), 
...
```{r, warning=FALSE, message=FALSE}
# pc_list <-list('12')
pc_list <-rep(5L, length(all_msc_filt))
...
# all_msc_filt <- mcmapply(function(x, y) {
#   RunUMAP(x, reduction = "pca", dims = 1:.y, verbose = FALSE, seed.use = 372)%>%
#     FindNeighbors(reduction = "pca", dims = 1:.y, verbose = FALSE)
# },
# all_msc_filt, pc_list)
...
```{r}
imap(all_msc_filt, ~saveRDS(.x, paste0("data/processed/Pf/all_msc_filt_",.y,".RDS")))
```

```{r}
# imap(all_msc_filt, ~paste0("d",.y,".RDS"))
# saveRDS(all_msc_anotd_preQC, "data/processed/Pf/all_msc_anotd_preQC.RDS")
```

```{r}
# imap(all_msc_filt, ~saveRDS(paste0(.x, "data/processed/Pf/all_msc_filt_",.y,".RDS")))
# saveRDS(all_msc_anotd_preQC, "data/processed/Pf/all_msc_anotd_preQC.RDS")
```
