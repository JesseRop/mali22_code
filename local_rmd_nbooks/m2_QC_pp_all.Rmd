...
```{r, warning=FALSE, message=FALSE}
##Seurat UMAP
map(all_msc_anotd_preQC, ~DimPlot(.x, dims = c(1,2), group.by = "StagePrelimC", label = T))
```
...
```{r, fig.height = 15, fig.width = 15}
gn_lc <- rep(300, length(sample_id_lst))
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 800, ftr = nf_lst, grp = "StagePrelimC", ptsz = 0.01)
...
```{r, fig.height = 15, fig.width = 15}
gn_lc <- rep(600, length(sample_id_lst))
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 1000000, ftr = nc_lst, grp = "StagePrelimC", ptsz = 0.0)

```

```{r, fig.height = 15, fig.width = 15}
gn_lc <- rep(550, length(sample_id_lst))
vp_fn(seu_lst = all_msc_anotd_preQC, gnc_lst = gn_lc, seu_thresh = 2000, ftr = nc_lst, grp = "StagePrelimC", ptsz = 0.0)
...
#   pmap(~ ..2 %>%
#          dplyr::select(Stage_V3, StagePrelimC)  %>%
#          AddMetaData(..1, .) 
#        )
...
map(all_msc_anotd_ysct, ~DimPlot(.x, dims = c(1,2), label = T, group.by = "StagePrelimC"))
...
map(all_msc_filt, ~{
  .x@meta.data %>%
    count(StagePrelimC, StagePrelimCAll)
