...
```{r}
mrc_sk21_summary %>% filter(str_detect(mixed_infection, "pf") & str_detect(shortids, "SetB|SetC") )
```

```{r}
## Getting only the jumpcode run where a sample was ran twice (non-jumpcode and jumpcode)
pf_22_jcode <- mrc_sk21_summary %>% 
  filter(str_detect(mixed_infection, "pf") & str_detect(shortids, "SetB|SetC") ) %>%
  mutate(sample_nm = str_remove_all(Sample_Name, "_JC")) %>%
  group_by(sample_nm) %>%
  add_count(name = "runs") %>%
...
Copy the irods metadata file from local into farm
```{r, eval=T}
write_csv(pf_solo_mixed, "data/raw/pf_solo_mixed.csv")
```
...
##Sample ID
sample_name <- pf_solo_mixed$sample_nm
smpl_nms <-pf_solo_mixed$sample_nm

##Sample IDs
source_irods <- pf_solo_mixed[,c('sample_nm', 'irods_id')] %>% deframe
