...
# for (i in 32:33) {
  ...
  ##Transfers
  ##souporcell genotyping files
  saveRDS(all_msc[[sample_name[i]]],
    paste0('data/processed/Pf/', sample_name[i], '/seu_obj/', sample_name[i],'_seu.RDS')
    )
...
```{r, message=F, warning=F}
##Read BPcells matrices
all_msc <- list.files("data/processed/", pattern = "seu_stdnorm.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
...
# all_msc <- mclapply(all_msc, function(x) {
#   seurat_norm_stand(x)
# })
