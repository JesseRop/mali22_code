...
```{r, message=F, warning=F}
##Read elbo plots
all_msc_elbo <- list.files("data/processed/", pattern = "^seu_norm_elbo.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed//Pf/|/seu_obj.*'))) %>% 
  .[sample_name] %>%
  map(readRDS)
