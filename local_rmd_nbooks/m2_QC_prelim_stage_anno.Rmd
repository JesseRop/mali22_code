...
all_msc_anotd_preQC <- pmap(list(all_msc_anotd_preQC, rep(0.4,length(sample_name)), c(rep('Late ring|Early trophozoite|late ring|early trophozoite|developing', length(sample_name)))), ~{
  ..1@meta.data %>%
    ...
    mutate(StagePrelim = case_when(
      str_detect(StagePrelim_lf, "gametocyte (developing)|schizont") ~ "Unassigned",
