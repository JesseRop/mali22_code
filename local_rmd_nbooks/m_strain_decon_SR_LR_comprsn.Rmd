...
```{r, message=FALSE, warning=FALSE}
## Read in log likelihoods for estimation of number of clusters from elbow plots
# clusters_log_likelihoods_lst = map2(sample_name_nms_al, rep(seq_nms, each= length(sample_name_nms)), ~read_table(paste0('data/processed/',species,'/', .x,'/', soup_dir, '/',.y,'/clusters_log_likelihoods.txt'), col_names = c('No_of_clusters', 'td1','td2','td3', 'td4','log_likelihood')) %>% select(-(starts_with("td"))) %>% mutate(across(No_of_clusters, ~str_remove_all(., ".*out_dir_k|/clusters.err:best")))) %>% set_names(paste(sample_name_nms_al, rep(seq_nms, each= length(sample_name_nms)), sep ='_'))
```
...
```{r, eval=T}
## !!NOTE: LongRead - for long read only - reverse compliment barcodes to make similar to 10X
# soupo_kselect_al %>% 
#   mutate("barcodeHMO" = ifelse(str_detect(sample_k, "soupc_lr"), paste0(as.character(reverseComplement(DNAStringSet(str_remove(barcode, "-1")))), "-1"), barcode)) %>% View
```
...
## !!NOTE: LongRead
hsat_mnmp_comp = hsat_mnmp_comp %>% filter(if_all(contains("soupc"), ~!is.na(.x)))
...
soupo_kselect_list <- soupo_kselect_al %>%
  ...
  mutate(across(sample_k, ~str_remove(.,paste0("_", seq_nm)))) %>% 
  dplyr::group_split(sample_k) %>%
  purrr::set_names(purrr::map_chr(., ~.x$sample_k[1])) %>%
...
for (s in names(strn_c_mdata)) {
  ...
  for (k in narrow_k) {
    for (str_q in c('strain')){
      ...
      # system(
      #   paste0('rsync -av /Users/jr35/Google\\ Drive/My\\ Drive/PHD_documents/malaria_phd/',sample_set,'/data/processed/',species,'/',s_nm,'/', sv_dir,'/bcodes/',algn_nm, '/bcodes_no_dbt/barcodes.tsv ', 'jr35@farm5:/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/',sample_set,'/data/processed/',species,'/',s_nm,'/', sv_dir,'/bcodes/',algn_nm, '/bcodes_no_dbt/')
      # )
  }
  # system(
