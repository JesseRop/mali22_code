...
```{r, fig.width=15, fig.height=5}
donor_mdata <- read_csv("/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/raw/MRC_Sample_Manifest_METADATA_raw.csv", col_types =  paste0(c(rep("c", 7), "d", "c","d","d", rep("c", 14)), collapse = ""), show_col_types = T)  
...
donor_mdata %>%
  ...
  filter(short_name %in% names(all_msc_w_strns_don)) %>% 
...
donor_mdata_slct <- donor_mdata_cln %>% 
  select(short_name, parasitaemia, smpl_wait_time, gender_m_f, weight_kg, symptomatic) %>% left_join(., adara_donor_mdata %>% select(short_name, age_years), by = "short_name")
```
...
```{r, fig.width=15, fig.height=5}
donor_mdata_slct  %>% filter(short_name %in% names(all_msc_w_strns_don)) %>% select(age_years, symptomatic) %>%  ggplot(aes(x = symptomatic, y = age_years)) + geom_boxplot()
```


```{r, fig.width=15, fig.height=5}
donor_mdata_slct  %>% filter(short_name %in% names(all_msc_w_strns_don)) %>% ggplot(aes(x = symptomatic, y = smpl_wait_time)) + geom_boxplot() + geom_point()
```
...
```{r, fig.width=15, fig.height=5}
v3_m2_asex_harmony_fld_sset@meta.data %>% 
  filter(dset == "Field" & !is.na(slingPseudotime_1)) %>%
  ggplot(aes(x = smpl_wait_time, y = slingPseudotime_1, colour = Symptomatic)) +
  geom_point()
```
...
v3_m2_asex_harmony_fld_sset@meta.data %>% 
  filter(dset == "Field" & !is.na(slingPseudotime_1)) %>%
  ggplot(aes(x = parasitaemia, y = slingPseudotime_1, colour = Symptomatic)) +
  geom_point()
```
...
v3_m2_asex_harmony_fld_sset@meta.data %>% 
  filter(dset == "Field" & !is.na(slingPseudotime_1)) %>%
  ggplot(aes(x = donor, y = slingPseudotime_1, colour = StrainQCd)) +
  geom_boxplot()
```
...
v3_m2_asex_harmony_fld_sset@meta.data %>% 
  filter(dset == "Field" & !is.na(slingPseudotime_1)) %>%
  lm(formula =slingPseudotime_1 ~ smpl_wait_time + parasitaemia, data = .)
```
...
v3_m2_asex_harmony_fld_sset@meta.data %>% 
  filter(dset == "Field" & !is.na(slingPseudotime_1)) %>%
  lm(formula =Symptomatic ~ smpl_wait_time + parasitaemia, data = .)
```

add metadata
```{r, fig.width=15, fig.height=5}
donor_mdata_cln %>% filter(short_name %in% unique(v3_m2_asex_harmony_fld_sset@meta.data$donor)) %>% select(short_name, smpl_wait_time, comments, symptomatic)
```
...
## Only those with asexuals and included in analysis
donor_mdata_slct   %>% 
  filter(short_name %in% st_dist$donor) %>%
  ggplot(aes(x = symptomatic, y = age_years)) +
  geom_boxplot()+
  geom_point()
```
...
donor_mdata_slct   %>% 
  filter(short_name %in% st_dist$donor) %>%
  ggplot(aes(x = symptomatic, y = smpl_wait_time)) +
  geom_boxplot()+
  geom_point()
```
...
m2_asex_tr_sce_pt_chunks_345 <- m2_asex_tr_sce_pt_chunks[,m2_asex_tr_sce_pt_chunks@colData$ptime_cat %in% c("ptime_3", "ptime_4", "ptime_5")]
```
...
```{r, results="asis"}
# only B cells
topTable(res.dl[["ptime_3"]], coef = "SymptomaticYes") %>% arrange(adj.P.Val)
```

```{r, results="asis"}
plotForest(res.dl, coef = "SymptomaticYes", gene = c("PF3D7-1346400"))
```
...
```{r, results="asis"}
plotVolcano(res.dl_don, coef = "SymptomaticYes")
```
...
conflicts_prefer(dreamlet::topTable)
# results from full analysis
topTable(res.dl_don, coef = "SymptomaticYes") 
```


```{r, results="asis"}
# only B cells
topTable(res.dl_don[["ptime_3"]], coef = "SymptomaticYes") %>% arrange(adj.P.Val)
```

```{r, results="asis"}
plotForest(res.dl_don, coef = "SymptomaticYes", gene = c("PF3D7-1346400"))
```

```{r, results="asis"}
genes <- c("PF3D7-1346400", "PF3D7-1478600", "PF3D7-0205000")
plotGeneHeatmap(res.dl_don, coef = "SymptomaticYes", genes = genes)
```
