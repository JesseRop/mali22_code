...
source("../Mali2/mali22_code_lnk/pf_common_vars.R")
```
...
```{r}
sample_names <- c("T08h_DMSO", "T08h_RAPA", "T16h_DMSO", "T16h_RAPA", "T24h_DMSO", "T24h_RAPA", "T32h_DMSO", "WT")
  
```


```{r}
list.files("/lustre/scratch126/tol/teams/lawniczak/users/sk21/Backup/pk_pipseq/rdata/", pattern = "msc_filt_w_dblts_anotd*", full.names = T, recursive = T, include.dirs = T) 
  
```

```{r}
pk_sk21 <- list.files("/lustre/scratch126/tol/teams/lawniczak/users/sk21/Backup/pk_pipseq/rdata/", pattern = "msc_filt_w_dblts_anotd*", full.names = T, recursive = T, include.dirs = T)  %>% 
  set_names(nm = (str_remove_all(.,'/lustre.*Pk_|/msc_.*'))) %>%
  .[sample_names] %>%
  .[!is.na(.)] %>%
  map(readRDS)
...
```{r}
pk_sk21[["WT"]]@meta.data %>% glimpse
```


```{r}
pk_sk21[["WT"]]@meta.data %>% count(StageHR_V3, StageLR_V3, Stage_scmap_V3)
```


```{r, eval=F}
sp_fld_colors_lcase <- sp_fld_colors2
names(sp_fld_colors_lcase) <- str_to_lower(names(sp_fld_colors_lcase))
```

```{r, eval=F}
imap(pk_sk21, ~DimPlot(.x, group.by = "StageLR_V3") + labs(title = .y) + scale_color_manual(values = sp_fld_colors_lcase))
```

```{r, eval=F}
FeaturePlot(pk_sk21[["WT"]], features = "PKNH-1149300", order = T, pt.size = 0.5)
```

```{r, eval=F}
imap(pk_sk21, ~DimPlot(.x) + labs(title = .y))
...
##merge all 
pk_merged <- merge(pk_sk21[[1]], pk_sk21[2:length(pk_sk21)])
...
```{r, warning=FALSE, message=FALSE}
pk_merged[["RNA"]] <- split(pk_merged[["RNA"]], f = pk_merged$orig.ident)
pk_merged
```

```{r, warning=FALSE, message=FALSE}
# run sctransform
# pk_merged <- SCTransform(pk_merged, verbose = FALSE, variable.features.n = 100)

pk_merged <- NormalizeData(pk_merged)
pk_merged <- FindVariableFeatures(pk_merged, selection.method = "vst", nfeatures = 300)
pk_merged <- ScaleData(pk_merged, verbose = F)
```

```{r, warning=FALSE, message=FALSE}
# These are now standard steps in the Seurat workflow for visualization and clustering
pk_merged <- RunPCA(pk_merged, verbose = FALSE, npcs = 20)

```

```{r, warning=FALSE, message=FALSE}
# run sctransform
DimPlot(pk_merged, reduction = "pca")
DimPlot(pk_merged, reduction = "pca", group.by = "StageLR_V3")
...
pk_merged <- FindNeighbors(pk_merged, dims = 1:5, reduction = "harmony")
pk_merged <- FindClusters(pk_merged, resolution = 0.2)
```

```{r, warning=FALSE, message=FALSE}
# These are now standard steps in the Seurat workflow for visualization and clustering
pk_merged <- RunUMAP(pk_merged, dims = 1:5, verbose = FALSE, n.components = 3, seed.use = 867, reduction = "harmony")
# pk_merged <- FindNeighbors(pk_merged, dims = 1:5, verbose = FALSE, reduction = "harmony")
# pk_merged <- FindClusters(pk_merged, verbose = FALSE, resolution = 0.1)
```

```{r, warning=FALSE, message=FALSE}
DimPlot(pk_merged, label = T, reduction = "pca")
DimPlot(pk_merged, label = T)
# DimPlot(pk_merged, label = T, group.by = "StageLR_V3") + scale_color_manual(values = sp_fld_colors_lcase)
DimPlot(pk_merged, group.by = "orig.ident")
FeaturePlot(pk_merged, features = "nFeature_RNA")
FeaturePlot(pk_merged, features = "nCount_RNA") + geom_hline(yintercept = 10000)
VlnPlot(pk_merged, features = "nCount_RNA", group.by = "seurat_clusters")

```
```{r, warning=FALSE, message=FALSE}
pk_merged <- pk_merged@meta.data %>%
         mutate(condition = str_remove(orig.ident, "S.*Pk_"))  %>%
         AddMetaData(pk_merged, .) 

```

```{r, warning=FALSE, message=FALSE}

sample_cols <- c("#ADD8E6", "#00008B", "#90EE90", "#006400", "#FFA07A", "#8B0000", "#D8BFD8", "lightgrey")
names(sample_cols) <- sample_names
```

```{r, warning=FALSE, message=FALSE}
DimPlot(pk_merged, group.by = "condition")+ scale_color_manual(values = sample_cols)
DimPlot(pk_merged, group.by = "condition", dims = c(1,3))+ scale_color_manual(values = sample_cols)

DimPlot(pk_merged, label = T, dims = c(1,3))
DimPlot(pk_merged, label = T, dims = c(1,3), group.by = "StageLR_V3") + scale_color_manual(values = sp_fld_colors_lcase)
```

```{r, warning=FALSE, message=FALSE}
DimPlot(pk_merged, group.by = "condition", order = sample_names)+ scale_color_manual(values = sample_cols)
DimPlot(pk_merged, group.by = "condition", order = sample_names, dims = c(1,3))+ scale_color_manual(values = sample_cols)
```


```{r, warning=FALSE, message=FALSE}
map(sample_names, ~DimPlot(pk_merged, group.by = "condition", cells.highlight = colnames(pk_merged[,pk_merged$condition == .x]), dims = c(1,3), sizes.highlight = 0.2 ) + labs(title = .x))
```

```{r, warning=FALSE, message=FALSE, fig.width=15, fig.height=3}
DimPlot(pk_merged, split.by = "condition", label = T, dims = c(1,3)) 
```


```{r, warning=FALSE, message=FALSE, fig.width=15, fig.height=3}
DimPlot(pk_merged, split.by = "condition", label = T, dims = c(1,2), reduction = "pca") 
```


```{r, fig.width=15, fig.height=3}
FeaturePlot(pk_merged, features = "PKNH-1149300", split.by = "condition", order = T, pt.size = 0.5)
```

```{r, warning=FALSE, message=FALSE}
DimPlot(pk_merged, label = T, dims = c(2,3))
DimPlot(pk_merged, label = T, dims = c(1,3))

```
```{r, warning=FALSE, message=FALSE}
DimPlot(pk_merged, label = T, dims = c(2,3), reduction = "pca")
DimPlot(pk_merged, label = T, dims = c(1,3), reduction = "pca")

```

```{r, warning=FALSE, message=FALSE}
pk_merged <- FindClusters(pk_merged, resolution = 0.1, cluster.name = "seurat_clusters_ss")

```

```{r, warning=FALSE, message=FALSE}
DimPlot(pk_merged, label = T, dims = c(2,3), group.by = "seurat_clusters_ss")
DimPlot(pk_merged, label = T, dims = c(1,3), group.by = "seurat_clusters_ss")

```

```{r, eval=T}
# gogo()
pk_merged <- JoinLayers(pk_merged)
...
```{r, warning=FALSE, message=FALSE}
avg_exp_condition <- map(sample_names, ~AverageExpression(
    pk_merged,
    return.seurat = FALSE,
    group.by = "condition",
    layer = "data",
...
```{r, warning=FALSE, message=FALSE}
gn_exp_tbl <- avg_exp_condition[[1]]$RNA %>% data.frame() %>% rename_all(., ~str_replace(.,"\\.", "_")) %>% select(-c(WT))
```

```{r, warning=FALSE, message=FALSE}
combn(sample_names[1:6], m = 2)
```

```{r, warning=FALSE, message=FALSE}
cond_combis <- list(c("T08h_DMSO", "T08h_RAPA"), c("T16h_DMSO", "T16h_RAPA"), c("T24h_DMSO", "T24h_RAPA"))
```

```{r, warning=FALSE, message=FALSE}
map(cond_combis, ~gn_exp_tbl %>%
  ggplot(aes(x = log2(!!rlang::sym(.x[1])), y = log2(!!rlang::sym(.x[2])))) +
  geom_point()
)
```
...
```{r, warning=FALSE, message=FALSE}
## ap2g
de_gns_pk <- map(cond_combis, ~FindMarkers(pk_merged, test.use = "MAST", ident.1 = .x[1], ident.2 = .x[2]))

```


```{r, warning=FALSE, message=FALSE}
# de_gns_anot_pk <- map(de_gns_pk, ~.x %>% rownames_to_column("gene_id") %>%  left_join(., Pf3D7_genes_plasmoDB[,c("gene_id", "Gene")]))
# 
# de_gns_anot_pk
```
...
```{r, warning=FALSE, message=FALSE}
de_gns_pk <- map(de_gns_pk, ~.x %>% rownames_to_column("gene_id"))

```

```{r, warning=FALSE, message=FALSE}
## cells that contain 2 or more ap2g UMI
map(de_gns_pk[1:3], ~.x  %>% filter(p_val_adj < 0.05 & avg_log2FC > 0))
...
EnhancedVolcano(de_gns_pk[[1]],
                  lab = de_gns_pk[[1]]$gene_id,
                  selectLab =de_gns_pk[[1]] %>% 
                    # filter(p_val_adj < 0.05 & (avg_log2FC > 1 | avg_log2FC < -1) & !str_detect(Gene, 'PF3D7')) %>%
                    filter(p_val_adj < 0.05 & (avg_log2FC > 1 | avg_log2FC < -1) ) %>%
                    pull(gene_id),
                  x = 'avg_log2FC',
...
```{r, warning=FALSE, message=FALSE}
## cells that contain 2 or more ap2g UMI
de_gns_pk_mgd <- full_join(de_gns_pk[[1]] %>% rownames_to_column("bcode"), de_gns_pk[[2]] %>% rownames_to_column("bcode"), by = "bcode", suffix = c("_8hr", "_16hr"))
...
## cells that contain 2 or more ap2g UMI
de_gns_pk_mgd %>% 
  ggplot(aes(x = avg_log2FC_8hr, y = avg_log2FC_16hr)) +
  geom_point()
```
...
## cells that contain 2 or more ap2g UMI
de_gns_pk_mgd %>% 
  filter(avg_log2FC_8hr > 5, avg_log2FC_16hr >5) 
```
...
```{r, eval=T}
# gogo()
pk_merged_sce <- as.SingleCellExperiment(pk_merged)

```

```{r, eval=T}
rowData(pk_merged_sce)$feature_symbol <- rownames(pk_merged_sce)
```
...
pk_merged_sce_ss <- slingshot(pk_merged_sce,
            clusterLabels = "seurat_clusters_ss",
            reducedDim = "UMAP",
            start.clus = 3,
            end.clus = 4,
            # shrink =1,
...
pk_merged_sce_ss_sds <- SlingshotDataSet(pk_merged_sce_ss)

# plot(reducedDims(pk_merged_sce_ss)$UMAP[,1:2], col = brewer.pal(9,"Set1")[as.factor(pk_merged_sce_ss$stage)], pch=16, asp = 0.7)
plot(reducedDims(pk_merged_sce_ss)$UMAP[,c(1,3)], col = brewer.pal(9,"Set1")[as.factor(pk_merged_sce_ss$seurat_clusters)], pch=16, asp = 0.7, )

lines(pk_merged_sce_ss_sds, lwd=2, col="black",dims = c(1,3))
```
...
```{r, warning=FALSE, message=FALSE}
map(sample_names, ~DimPlot(pk_merged, group.by = "condition", cells.highlight = colnames(pk_merged[,pk_merged$condition == .x]), dims = c(1,3) ) + labs(title = .x))
```

```{r, warning=FALSE, message=FALSE}
## ap2g
FeaturePlot(pk_merged, features = "PF3D7-1222600", order = T, pt.size = 0.5) 
FeaturePlot(pk_merged, features = "PF3D7-1222600", reduction = "pca", order = T, pt.size = 0.5) 
```

```{r, warning=FALSE, message=FALSE}
## ap2g
DefaultAssay(pk_merged) <- "RNA"
FeaturePlot(pk_merged, features = "PF3D7-1222600", order = T, pt.size = 0.5) 
FeaturePlot(pk_merged, features = "PF3D7-1222600", reduction = "pca", order = T, pt.size = 0.5) 
DefaultAssay(pk_merged) <- "SCT"
```

```{r, warning=FALSE, message=FALSE, eval=T}
# Display the combined datasets as an interactive 3D plot

plot_ly(data = data.frame(pk_merged@reductions[["umap"]]@cell.embeddings),
        x = ~umap_1, y = ~umap_2, z = ~umap_3,
        color= pk_merged@meta.data$seurat_clusters,
        # colors = c('blue', 'red'),
        opacity = 0.8,
        size = 1, 
        type = 'scatter3d'
...
de_gns_fmall_cln <- FindAllMarkers(pk_merged)
