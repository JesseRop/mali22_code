...
# m14_sweep.res = map(all_msc_corrected_dblt[4], ~paramSweep_v3(.x, PCs = 1:10, sct = T))
# # m14_sweep.res = list(a)
# m14_gt.calls = map2(all_msc_corrected_dblt[4], m14_sweep.res, ~.x@meta.data[rownames(.y[[1]]), "GT"])
...
## # ## pK Identification (ground-truth) ------------------------------------------------------------------------------------------
# m_sweep.res = map(all_msc_corrected_dblt, ~paramSweep_v3(.x, PCs = 1:10, sct = T))
sct_logcl <- rep(c(T,F), each = length(all_msc_corrected_dblt)/2)
m_sweep.res = map2(all_msc_corrected_dblt, sct_logcl, ~paramSweep_v3(.x, PCs = 1:10, sct = .y))
```


```{r, results="hide", warning=FALSE, message=FALSE}
## # ## pK Identification (ground-truth) ------------------------------------------------------------------------------------------
m_gt.calls = map2(all_msc_corrected_dblt, m_sweep.res, ~.x@meta.data[rownames(.y[[1]]), "GT"])
...
# sweep_all <- all_msc_corrected_dblt %>%
...
homo.prop_all <- all_msc_corrected_dblt %>%
  map(. %>% .$seurat_clusters %>% modelHomotypic(.))

nDub_all <- map(all_msc_corrected_dblt, ~ncol(.) * 0.00001)

nExp_poi_all <- map2(nDub_all, all_msc_corrected_dblt, ~round(.x*nrow(.y)))
...
# strains_bld_dFinder = list(all_msc_corrected_dblt, pK_choose_all, nExp_poi_all, nExp_poi_adj_all) %>%
...
strains_bld_dFinder = list(all_msc_corrected_dblt, pK_choose_all, nExp_poi_all, nExp_poi_adj_all, sct_logcl) %>%
  pmap(~ ..1 %>% doubletFinder_v3(., PCs = 1:10, pN = 0.25, pK = ..2, nExp = ..3, sct = ..5) %>%
         doubletFinder_v3(., PCs = 1:10, pN = 0.25, pK = ..2, nExp = ..4, reuse.pANN = paste0("pANN_0.25_",..2,"_",..3), sct = ..5))
...
```{r}
all_msc_corrected_dblt <- list(strains_bld_dFinder, pK_choose_all, nExp_poi_all, nExp_poi_adj_all, all_msc_corrected_dblt) %>%
  pmap(~ ..1 %>% 
         .@meta.data %>% 
...
```{r}
## Takes long to run dbltfinder
# saveRDS(all_msc_corrected_dblt, "data/processed/all_msc_corrected_dblt.RDS")
```

```{r}
##No ground truth option
all_msc_corrected_dblt <- readRDS("data/processed/all_msc_corrected_dblt.RDS")
```
...
all_msc_corrected_dblt %>% 
  imap(.,~{
    # create tabset for each group 
    cat('###### ',.y,'   \n')
    p1 <- DimPlot(.x, cells.highlight = .x@meta.data %>% filter(dblt_finder1=='Doublet') %>% row.names(), sizes.highlight = 1)
    print(p1)
    cat('\n  ')
    
    p2 <- DimPlot(.x, cells.highlight = .x@meta.data %>% filter(dblt_finder2=='Doublet') %>% row.names(), sizes.highlight = 1)
    print(p2)
    cat('\n')
    cat("\n")
  })
...
```{r, message=FALSE, eval=FALSE}
##Doublet removal
all_msc_nosct_corrected_dblt_sce <- all_msc_nosct_corrected_dblt_sce %>%
  map(. %>% scDblFinder(., clusters = "seurat_clusters")) #%>%
  # map(. %>% scDblFinder(.))
...
# all_msc_w_dblts <- list(all_msc_corrected_dblt, all_msc_filt) %>%
#   pmap(~ ..1@meta.data %>% 
#          dplyr::rename('seurat_clusters_dblt' = seurat_clusters) %>%
#          mutate(scrub_doublet = case_when(scrub_doublet == T ~ "Doublet", scrub_doublet == F ~ "Singlet")) %>%
#          dplyr::select(any_of(c('scrub_doublet', 'dblt_finder1', 'dblt_finder2', 'seurat_clusters_dblt', 'dblt_finder_pANN')))  %>%
...
```{r}
# all_msc_w_dblts <- list(all_msc_nosct_corrected_dblt_sce, all_msc_w_dblts) %>%
#   pmap(~ ..1 %>% 
#          SummarizedExperiment::colData() %>%
#          data.frame()  %>% 
#          dplyr::select('scDblFinder.class') %>% 
#          dplyr::rename('scDblFinder_class' = `scDblFinder.class`) %>% 
...
```{r, message=FALSE, eval=FALSE}
##Doublet removal
all_msc_w_dblts <- list(all_msc_w_dblts, all_msc_nosct_corrected_dblt_sce[paste0(names(all_msc_filt), "_norm")], all_msc_nosct_corrected_dblt_sce[paste0(names(all_msc_filt), "_sct")]) %>%
  pmap(~ {
    ..2 %>% 
      SummarizedExperiment::colData() %>%
      data.frame()  %>% 
...
map(all_msc_w_dblts, ~DimPlot(.x, dims = c(1,2), label = T, cells.highlight = .x@meta.data %>% filter(scDblFinder_class_sct == "Doublet") %>% row.names()))
```
...
dbt_methods <- c(paste0(c("scDblFinder_class", "dblt_finder2"), rep(c("_norm", "_sct"), each=2)), "scrub_doublet")
...
# dbt_methods <- c(paste0(c("scDblFinder_class_norm", "dblt_finder2"), rep(c("_norm", "_sct"), each=2)), "scrub_doublet")
# 
# dbt_sens_speci <-map2(all_msc_w_dblts, rep(list(dbt_methods), length(all_msc_w_dblts)), ~{
#   dblt_calls <- .x@meta.data %>% 
