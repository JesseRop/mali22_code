...
```{r, eval=F}
all_msc_raw_paths <- list.files("/lustre/scratch126/tol/teams/lawniczak/projects/malaria_single_cell/mali_field_runs/2022/data/cellranger_runs/Pf/", pattern = "raw_feature_bc_matrix$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'/lustre.*Pf//|/outs.*') %>% str_replace(., '/', '_'))) %>%
  .[irods_id_sk21_mdata_cln[, c("sample_nm", "irods_id")] %>% deframe() %>% .[sample_id_lst]] %>%
...
#   data <- seurat_norm_stand(Seurat_object = data, hvg = 500)
#   return(list(data, sc))
# }
```
...
```{r, message=FALSE, eval=F}
##Doublet removal
all_msc_nosct_corrected_dblt_stg <- all_msc_nosct_corrected_dblt_sce %>%
  map(. %>% scDblFinder(., clusters = "StagePrelimC", returnType = "table", nfeatures = 500)) #%>%
  # map(. %>% scDblFinder(.))
```
...
```{r, message=FALSE, eval=F}
##Doublet removal
all_msc_nosct_dblt_stg <- all_msc_nosct_dblt_sce %>%
  map(. %>% scDblFinder(., clusters = "StagePrelimC", returnType = "table", nfeatures = 500)) #%>%
  # map(. %>% scDblFinder(.))
```
...
scf_outputs <- list(all_msc_nosct_corrected_dblt_clst, all_msc_nosct_corrected_dblt_stg, all_msc_nosct_corrected_dblt_rand, all_msc_nosct_dblt_clst, all_msc_nosct_dblt_stg, all_msc_nosct_dblt_rand) %>%
  pmap(~ {
    spx <- list(..1, ..2, ..3, ..4)
    pmap(list(spx, list("_supx_norm", "_supx_stg", "_supx_rand", "_norm", "_stg", "_rand")), ~ {
...
map(all_msc_w_dblts, ~.x@meta.data %>% ggplot(aes(x = seurat_clusters, y= sim_lab1, color = strain_gt)) + geom_boxplot() + geom_hline(yintercept = 0.85) + facet_wrap(. ~ StagePrelimC, nrow = 1, scales = "free_x"))
```
...
map(all_msc_w_dblts, ~DimPlot(.x, dims = c(1,2), group.by = "StagePrelimC", reduction = "umap_soupx"))
```
...
all_msc_filt_no_dblts <- all_msc_filt_no_dblts %>%
    ...
    map(. %>% subset(., subset = scrub_doublet_dbt_prop < 0.1))
  # map(. %>% subset(., subset = scf_class_supx_rand == "Singlet" & scrub_doublet == 'Singlet'))
...
# for (i in 32:33) {
  ...
  saveRDS(all_msc_filt[[sample_name[i]]],
    paste0('data/processed/Pf/', sample_name[i], '/seu_obj/filt_seu_sct.RDS')
    )
