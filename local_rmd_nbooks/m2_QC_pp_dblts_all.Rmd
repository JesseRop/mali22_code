...
```{r}
# rm(all_msc_nosct_corrected_dblt)
```
...
all_msc_nosct_corrected_dblt_sce <- all_msc_nosct_corrected_dblt %>%
...
```{r, message=FALSE}
##Doublet removal
all_msc_w_dblts <- list(all_msc_filt, all_msc_nosct_corrected_dblt_clst, all_msc_nosct_corrected_dblt_rand) %>%
  pmap(~ {
    ..2 %>% 
      data.frame()  %>% 
      dplyr::select('class', 'score') %>% 
      # dplyr::rename('scf_class' = class) %>% 
      dplyr::rename_with(.fn = ~paste0("scf_",.), .cols = everything()) %>% 
      mutate(across(scf_class, str_to_sentence)) %>% 
...
for (i in names(all_msc_w_dblts)) {
  all_msc_w_dblts[[i]]@reductions[["umap_n_soupx"]] <- all_msc_nosct_corrected_dblt[[i]]@reductions$umap
}
```
...
```{r}
rm(all_msc_nosct_corrected_dblt_sce, all_msc_nosct_corrected_dblt)
```
...
dbt_methods <- c(paste0(c("scf_class"), c("_norm", "_rand")), "scrub_doublet")
