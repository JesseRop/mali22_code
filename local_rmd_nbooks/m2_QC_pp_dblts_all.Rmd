...
# nextflow run submsn_seu_norm.nf --scrpt "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/multipurpose_scripts/seurat_sce/seu_stdnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/seu_obj/filt_seu_supx.RDS" --o_file_name "filt_seu_norm_supx.RDS" --umap_name "UMAP_F_S_NORM" --hvg 500 --cluster_resoln 0.1 --cl_nm "seurat_clusters_supx" --umap_redctn_nm "umap.f.s.norm" --redctn "pca" --integrtd_pc30 "no" --elbo_nm "filt_seu_norm_supx_elbo.RDS" --findpc_mthd "perpendicular line" --resume
```


```{r}
filt_seu_norm_supx_elbo <-  list.files("data/processed/Pf", pattern = "^filt_seu_norm_supx_elbo.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*'))) %>% 
  .[sample_name] %>%
  map(readRDS)
...
```{r, message=F, warning=F}
##Read BPcells matrices
all_msc_nosct_supx_4_dblt <- list.files("data/processed/Pf", pattern = "^filt_seu_norm_supx.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*'))) %>% 
  .[sample_name] %>%
  map(readRDS)
```
...
# subset(all_msc_filt_no_strainDblts[[4]], subset = seurat_clusters_dblt == "7") %>% .@meta.data %>% dplyr::count(scf_class_norm, scrub_doublet)

##Remove cluster 8 with lots of doublets (0.205) MSC14 - losing 31 singlets
# all_msc_filt_no_strainDblts[[4]] <- subset(all_msc_filt_no_strainDblts[[4]], subset = seurat_clusters_dblt != "7")

##Remove cluster 8 with lots of doublets (0.368) MSC14 - losing 12 singlets
# all_msc_filt_no_strainDblts[[4]] <- subset(all_msc_filt_no_strainDblts[[4]], subset = seurat_clusters_dblt != "8")
...
map(all_msc_filt_no_strainDblts, ~.x@meta.data %>% filter(scf_class_norm_dbt_prop > 0.1) %>% count(seurat_clusters, scf_class_norm_dbt_prop,scf_class_rand_dbt_prop, scrub_doublet_dbt_prop,   scf_class_norm_dbt_nums, scrub_doublet_dbt_nums))
...
map(all_msc_filt_no_strainDblts, ~.@meta.data %>% dplyr::count(scf_class_norm, scrub_doublet))
...
map(all_msc_filt_no_strainDblts, ~.x@meta.data %>% count(scf_class_norm_dbt_prop > 0.1))
...
map(all_msc_filt_no_strainDblts, ~.x@meta.data %>% filter(!((scf_class_norm_dbt_prop > 0.1 & scf_class_rand_dbt_prop > 0.1) | scrub_doublet_dbt_prop > 0.1)) %>% rownames_to_column("bcode") %>% mutate(across(c(scf_class_norm_dbt_prop,scf_class_rand_dbt_prop, scrub_doublet_dbt_prop), ~round(., 2))) %>% count(seurat_clusters, scf_class_norm_dbt_prop,scf_class_rand_dbt_prop, scrub_doublet_dbt_prop))
...
map(all_msc_filt_no_strainDblts, ~.x@meta.data %>% filter(scf_class_norm == "Singlet") %>% count(scf_class_norm_dbt_prop < 0.20 & scf_class_rand_dbt_prop < 0.20))
...
map(all_msc_filt_no_strainDblts, ~.x@meta.data %>% filter(scf_class_norm == "Singlet" & scf_class_norm_dbt_prop < 0.20 & scf_class_rand_dbt_prop < 0.20) %>% count(scrub_doublet_dbt_prop < 0.40))
...
map(all_msc_filt_no_strainDblts, ~.x@meta.data %>% filter(scf_class_norm == "Singlet" & scf_class_norm_dbt_prop < 0.20 & scf_class_rand_dbt_prop < 0.20) %>% count(scrub_doublet))

```

```{r}

map(all_msc_filt_no_strainDblts, ~.x@meta.data %>% filter(scf_class_norm == "Singlet") %>% count(scf_class_norm_dbt_prop, scf_class_rand_dbt_prop) %>% filter(scf_class_norm_dbt_prop>0.2 ))
...
imap(all_msc_filt_no_strainDblts["MSC50_SLO"], ~DimPlot(.x, group.by = c("scrub_doublet", "StagePrelim", "scrub_doublet_dbt_prop", "scf_class_norm_dbt_prop", "scf_class_rand_dbt_prop"), ncol = 5) + labs(title =.y))
...
imap(all_msc_filt_no_strainDblts["MSC50_SLO"], ~DimPlot(.x, group.by = c("scrub_doublet", "StagePrelim", "scrub_doublet_dbt_prop", "scf_class_norm_dbt_prop"), reduction = "pca") + labs(title =.y))
...
# all_msc_filt_no_strainDblts <- all_msc_filt_no_strainDblts %>%
...
all_msc_filt_no_StgStrainDblts <- all_msc_filt_no_strainDblts %>%
    # map(. %>% subset(., subset = scf_class_norm == "Singlet" & scf_class_norm_dbt_prop < 0.1 & scf_class_rand_dbt_prop < 0.1)) %>%
...
map(all_msc_filt_no_strainDblts, dim) %>% as.data.frame() %>% .[2,] %>% bind_rows(., map(all_msc_filt_no_StgStrainDblts, dim) %>% as.data.frame() %>% .[2,]) %>% t()
```
...
imap(all_msc_filt_no_StgStrainDblts["MSC50_SLO"], ~DimPlot(.x, group.by = c("scrub_doublet", "StagePrelim", "scrub_doublet_dbt_prop")) + labs(title =.y))
...
map(all_msc_filt_no_StgStrainDblts, ~.x@meta.data %>% count(scrub_doublet))

```

```{r}

map(all_msc_filt_no_StgStrainDblts, ~.x@meta.data %>% count(scrub_doublet_dbt_prop, scf_class_norm_dbt_prop, scf_class_rand_dbt_prop) %>% filter(scrub_doublet_dbt_prop>0.2 ))
...
all_msc_filt_no_StgStrainDblts <- all_msc_filt_no_StgStrainDblts %>%
    map(. %>% subset(., subset = scrub_doublet == "Singlet")) %>%
    map(. %>% subset(., subset = scrub_doublet_dbt_prop < 0.5))  ## !!NOTE - was 0.1 but MSC50_SLO has high number of doublets - maybe SLO lysed many gams and soup content high
...
map(all_msc_filt_no_StgStrainDblts, dim)
...
for (i in names(all_msc_filt_no_StgStrainDblts)) {
  DefaultAssay(all_msc_filt_no_StgStrainDblts[[i]]) <- "RNA"
}
...
# for (i in 32:33) {
  ...
  saveRDS(all_msc_filt_no_StgStrainDblts[[sample_name[i]]],
    paste0('data/processed/Pf/', sample_name[i], '/seu_obj/all_msc_filt_no_StgStrainDblts.RDS')
    )
...
gogo()
## Run SNORM on farm like below
# nextflow run submsn_seu_norm.nf --scrpt "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/multipurpose_scripts/seurat_sce/seu_stdnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/seu_obj/all_msc_filt_no_StgStrainDblts.RDS" --o_file_name "msc_filt_no_dblts.RDS" --umap_name "UMAP_" --hvg 500 --cluster_resoln 0.1 --cl_nm "seurat_clusters_nm" --umap_redctn_nm "umap" --redctn "pca" --integrtd_pc30 "no" --elbo_nm "msc_filt_no_dblts_elbo.RDS" --findpc_mthd "perpendicular line" --resume
...
##Read BPcells matrices
all_msc_filt_no_dblts <- list.files("data/processed/Pf", pattern = "msc_filt_no_dblts.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*'))) %>% 
  .[sample_name] %>%
  map(readRDS)
```
...
```{r}
imap(all_msc_filt_no_dblts, possibly(~DimPlot(.x, cells.highlight = colnames(.x[,.x@meta.data$StagePrelim == "Unassigned"])) + labs(title = .y)))
```


```{r}
map(all_msc_filt_no_dblts, ~.x@meta.data %>% filter(StagePrelim == "Unassigned") %>% select(StagePrelim, contains("_lab")))
```

```{r}
map(all_msc_filt_no_dblts, ~.x@meta.data %>% filter(str_detect(StagePrelim, "schizont")) %>% select(contains("_lab")))
```

Remove unassigned and schizonts
```{r}
##Remove schizonts and unassigned
all_msc_qcd_snglt_stgd <- all_msc_filt_no_dblts %>%
...
map(all_msc_filt_no_dblts, dim) %>% as.data.frame() %>% .[2,] %>% bind_rows(., map(all_msc_qcd_snglt_stgd, dim) %>% as.data.frame() %>% .[2,]) %>% t()
```


```{r}
saveRDS(all_msc_qcd_snglt_stgd, 'data/processed/Pf/m2_all/all_msc_qcd_snglt_stgd.RDS')
```

#### Visualize reclustered QCd cells

```{r, results='asis', message=FALSE, warning=FALSE}
imap(all_msc_filt_no_dblts, ~VlnPlot(., features = c("nCount_RNA", "nFeature_RNA"), group.by = c("seurat_clusters"), idents = NULL, pt.size = 0.01) + labs(title = .y))
```

```{r, results='asis', message=FALSE, warning=FALSE}
imap(all_msc_filt_no_dblts, ~VlnPlot(., features = c("nCount_RNA", "nFeature_RNA"), group.by = c("StagePrelimC"), idents = NULL, pt.size = 0.01) + labs(title = .y))
```
```{r, results='asis', message=FALSE, warning=FALSE}
imap(all_msc_filt_no_dblts, ~DimPlot(., reduction = "umap", label = T, pt.size = 0.5, group.by = "StagePrelim") + labs(title = .y) + scale_color_manual(values = sp_fld_colors))
```
