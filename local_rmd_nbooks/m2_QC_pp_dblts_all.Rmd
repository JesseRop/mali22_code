...
all_msc_filt_no_StgDblts <- all_msc_w_dblts %>%
    # map(. %>% subset(., subset = scf_class_norm == "Singlet" & scf_class_norm_dbt_prop < 0.1 & scf_class_rand_dbt_prop < 0.1)) %>%
...
map(all_msc_w_dblts, dim) %>% as.data.frame() %>% .[2,] %>% bind_rows(., map(all_msc_filt_no_StgDblts, dim) %>% as.data.frame() %>% .[2,]) %>% t()
```
...
imap(all_msc_filt_no_StgDblts["MSC50_SLO"], ~DimPlot(.x, group.by = c("scrub_doublet", "StagePrelim", "scrub_doublet_dbt_prop")) + labs(title =.y))
...
map(all_msc_filt_no_StgDblts, ~.x@meta.data %>% count(scrub_doublet))

```

```{r}

map(all_msc_filt_no_StgDblts, ~.x@meta.data %>% count(scrub_doublet_dbt_prop, scf_class_norm_dbt_prop, scf_class_rand_dbt_prop) %>% filter(scrub_doublet_dbt_prop>0.2 ))
...
all_msc_filt_no_StgDblts <- all_msc_filt_no_StgDblts %>%
...
map(all_msc_filt_no_StgDblts, dim)
...
for (i in names(all_msc_filt_no_StgDblts)) {
  DefaultAssay(all_msc_filt_no_StgDblts[[i]]) <- "RNA"
}
...
# for (i in 32:33) {
  ...
  saveRDS(all_msc_filt_no_StgDblts[[sample_name[i]]],
    paste0('data/processed/Pf/', sample_name[i], '/seu_obj/all_msc_filt_no_StgDblts.RDS')
    )
