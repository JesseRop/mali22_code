...
sample_name <- c("MSC14", "MSC48", "MSC68")
sample_names <- rep(sample_name, 2)
# sample_name_nms <- sample_name

sample_name_all<- c("MSC14", "MSC48", "MSC68", "MSC14_edrops100", "MSC48_edrops100", "MSC68_edrops100")
sample_name_nms <- sample_name_all
##Samples
...
```{r, eval=T}
all_msc_filt <- readRDS("data/processed/Pf/all_msc_edrops_raw_filt.RDS")
```


```{r}

all_msc_filt %>% map(., ~.x@meta.data %>% 
                       mutate(stage_c_ag = case_when(StagePrelimC %in% c("Early trophozoite", "Late ring") ~ "Asexual", str_detect(StagePrelimC, "ale") ~ "Gametocytes", T ~ StagePrelimC)) %>% 
                       count(stage_c_ag)) #%>% 
                       #count(stage_c_ag, Strain_DN))
...
all_msc_nosct_supx_4_dblt_objs <- list.files("data/processed/Pf", pattern = "^filt_seu_raw.*_soupx_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*filt_seu_raw|_soupx_sct.*'))) %>% 
  .[sample_name_nms] %>%
  map(readRDS)
```
...
imap(all_msc_nosct_supx_4_dblt, ~saveRDS(.x, paste0("data/processed/Pf/", str_extract(.y, "MSC[0-9]+"),"/seu_obj/filt_seu_raw_supx", str_remove(.y, "MSC[0-9]+"), ".RDS")))
# imap(all_msc_filt, ~saveRDS(.x, paste0("data/processed/Pf/", str_extract(.y, "MSC[0-9]+"),"/seu_obj/filt_seu_raw", str_remove(.y, "MSC[0-9]+"), ".RDS")))
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_stdnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/seu_obj/filt_seu_supx.RDS" --normlzn "norm" --umap_name "UMAP_F_S_NORM" --hvg 500 --cluster_resoln 0.1 --cl_nm "seurat_clusters_supx" --umap_redctn_nm "umap.f.s.sct" --redctn "pca" --integrtd_pc30 "no" --findpc_mthd "perpendicular line" --pca_redctn_nm "pca_soupx_sct" --resume


# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_sctnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/{MSC48,MSC68,MSC14}/seu_obj/filt_seu_raw_supx{_edrops100,}.RDS" --normlzn "sct" --umap_name "UMAP_F_SCT" --hvg 400 --cluster_resoln 0.1 --cl_nm "seurat_clusters_sct" --umap_redctn_nm "umap.f.s.sct" --redctn "pca" --integrtd_pc30 "no" --pca_redctn_nm "pca_soupx_sct" --resume 
...
```{r}
filt_seu_norm_supx_elbo <-  list.files("data/processed/Pf", pattern = "^filt_seu_raw_supx.*_sct_elbo.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*filt_seu_raw_supx|_sct.*'))) %>% 
  .[sample_name_nms] %>%
  map(readRDS)
...
```{r, message=F, warning=F}
##Read BPcells matrices
all_msc_nosct_supx_4_dblt <- list.files("data/processed/Pf", pattern = "^filt_seu_raw_supx.*_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*filt_seu_raw_supx|_sct.*'))) %>% 
  .[sample_name_nms] %>%
  map(readRDS)
```
...
fp_fn <- function(obj = all_msc_nosct_supx_4_dblt[[1]], mks = marker_nms, mk_nms = names(marker_nms), rdctn = "umap.f.s.sct") {
# fp_fn <- function(obj = all_msc_nosct_supx_4_dblt[[1]], mks = marker_nms, mk_nms = names(marker_nms), rdctn = "umap") {
  list(mks, mk_nms) %>%
  pmap(.,~{
...
```{r, fig.height = 5, fig.width = 15}
imap(all_msc_nosct_supx_4_dblt, ~fp_fn(obj = .x, rdctn = "pca") %>% 
       plot_grid(ggdraw() + draw_label(.y), . ,ncol = 1, rel_heights = c(0.1, 1)))
```
...
# nextflow run ../mali22_code_lnk/nf_scripts/doubletF.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/scdblfindr.R" --filt_obj "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/seu_obj/filt_seu_sct_norm.RDS" --spx_obj "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/seu_obj/filt_seu_supx_norm.RDS" --o_sufx "scdblf" --resume

# nextflow run ../mali22_code_lnk/nf_scripts/doubletF.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/scdblfindr.R" --filt_obj "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/{MSC48,MSC68,MSC14}/seu_obj/filt_seu_raw{_edrops100,}_sct.RDS" --spx_obj "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/{MSC48,MSC68,MSC14}/seu_obj/filt_seu_raw_supx{_edrops100,}_sct.RDS" --o_sufx "scdblf" --resume
```

Read in seurat objects processed in farm
```{r, message=F, warning=F}
##Read BPcells matrices
scf_outputs <- list.files("data/processed/Pf", pattern = "^filt_seu_raw_supx.*_sct_scdblf.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*filt_seu_raw_supx|_sct_scdblf.*'))) %>% 
  .[sample_name_nms] %>%
  map(readRDS)
```
...
```{r, message=FALSE}
##Doublet removal
DimPlot(all_msc_nosct_supx_4_dblt[[1]], reduction = 'umap.f.s.sct')
# DimPlot(all_msc_w_dblts[[1]],reduction = "umap.f.s.sct")
```

```{r}
## Add UMAP dimensions from the soupx corrected object used to get doublets
for (i in names(all_msc_w_dblts)) {
  all_msc_w_dblts[[i]]@reductions[["umap.f.s.sct"]] <- all_msc_nosct_supx_4_dblt[[i]]@reductions[["umap.f.s.sct"]]
  all_msc_w_dblts[[i]]@reductions[["pca_soupx_sct"]] <- all_msc_nosct_supx_4_dblt[[i]]@reductions[["pca_soupx_sct"]]
}
```
...
```{r}
library(Matrix)
write(names(counts_msc)[!str_detect(names(counts_msc), "_edrops100")], file = "data/processed/Pf/don_nms_raw.txt")
# write(str_remove(names(counts_msc), "_edrops100"), file = "data/processed/Pf/don_nms_raw.txt")

imap(counts_msc, ~writeMM(.x, paste0("data/processed/Pf/", str_extract(.y, "MSC[0-9]+"),"/seu_obj/counts_msc_raw", str_remove(.y, "MSC[0-9]+"), ".mtx")))
imap(genes_msc, ~write(.x, paste0("data/processed/Pf/", str_extract(.y, "MSC[0-9]+"),"/seu_obj/genes_msc_raw", str_remove(.y, "MSC[0-9]+"), ".txt")))
...
scrb_doublet_scores <- list.files("data/processed/Pf", pattern = "^doublet_scores_raw.*.csv$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*doublet_scores_raw|.csv'))) %>% 
  .[sample_name_nms] %>%
...
```{r}
## Read scrublet output
predicted_doublets <- list.files("data/processed/Pf", pattern = "^predicted_doublets_raw.*.csv$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*predicted_doublets_raw|.csv'))) %>% 
  .[sample_name_nms] %>%
  map(., ~read.csv(., header = F) %>% t() %>% .[,1] %>% as.vector)
```
...
scrb_png <- list.files("data/processed/Pf", pattern = "scrblt_thresh_raw.*.png$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*scrblt_thresh_raw|.png'))) %>% 
  .[sample_name_nms] %>%
  map(readPNG)
...
scrb_umap_png <- list.files("data/processed/Pf", pattern = "scrblt_umap_raw.*.png$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*scrblt_umap_raw|.png'))) %>% 
  .[sample_name_nms] %>%
  map(readPNG)
...
imap(all_msc_w_dblts, ~.x@meta.data %>% ggplot(aes(x = seurat_clusters, y= sim_lab1)) + geom_boxplot() + geom_hline(yintercept = 0.85) + facet_wrap(. ~ StagePrelimC, nrow = 1, scales = "free_x")+
    labs(title = .y))
```
...
imap(all_msc_w_dblts, ~DimPlot(.x, dims = c(1,2), label = T, cells.highlight = .x@meta.data %>% filter(hetero_dblts == "Doublet") %>% row.names(), reduction = "umap.f.s.sct") + labs(title = .y))
...
all_msc_w_dblts[[3]]@meta.data %>%
      ...
      select(sim_lab1,StagePrelimC,  lo_outlr_thresh, lo_outlrs, hetero_dblts)
...
map(all_msc_w_dblts, ~DimPlot(.x, dims = c(1,2), group.by = "StagePrelimC", reduction = "umap.f.s.sct"))
```
...
dbt_methods_gtyp <- c(dbt_methods, "hetero_dblts")
...
```{r, fig.height = 8, fig.width = 15}
# 
# map(all_msc_w_dblts, ~scf_fn(obj = .x, nrws = ceiling(length(dbt_methods_gtyp)/5), redct = "pca_soupx_sct"))
```
...
```{r}
dblt_calls <- all_msc_w_dblts[[1]]@meta.data %>% dplyr::select( dbt_methods_stg) %>% mutate(across(everything(), ~factor(., levels = c("Singlet", "Doublet")))) 
...
cell_qc_objs<- list(all_msc_w_dblts, mt_filt) %>%
  ...
  mutate('QC_reason' =case_when(#strain_gt == "Negative" ~ 'sc_neg', 
                                ...
                                percent.mt>=..2 ~ 'hi_mtp',
                                scf_class_supx_rand == 'Doublet' & scrub_doublet == 'Doublet' ~ 'ss_db', 
                                scf_class_supx_rand == 'Doublet' & scrub_doublet == 'Singlet' ~ 'ss_db', 
                                scf_class_supx_rand == 'Singlet' & scrub_doublet == 'Doublet' ~ 'ss_db', 
                                scf_class_supx_rand == 'Doublet' & scrub_doublet == 'Doublet' ~ 'stg_db', 
                                scf_class_supx_rand == 'Singlet' & scrub_doublet == 'Singlet' ~ 'strn_db', 
                                scf_class_supx_rand == 'Doublet' & scrub_doublet == 'Singlet' ~ 'stg_db',
                                scf_class_supx_rand == 'Singlet' & scrub_doublet == 'Doublet' ~ 'stg_db', 
                                scf_class_supx_rand == 'Singlet' & scrub_doublet == 'Singlet' ~ 'pass')) %>%
...
saveRDS(all_msc_w_dblts, "data/processed/Pf/m2_all/all_msc_w_dblts_edrops.RDS")
...
# saveRDS(all_msc_w_dblts[c("MSC33", "MSC57")] %>% lapply(., function(x) x@meta.data), "data/processed/Pf/m2_all/all_msc_w_dblts_m33_m57_edrops.RDS")
...
```{r}
imap(all_msc_filt_no_ScfStgDblts, possibly(~DimPlot(.x, cells.highlight = colnames(.x[,.x@meta.data$scf_class_rand == "Doublet"]), reduction = "pca_soupx_sct") + labs(title = .y)))
```
...
#     paste0('data/processed/Pf/', sample_name_nms[i], '/seu_obj/msc_qcd_snglt_stgd_edrops.RDS')
#     )
#   
# }
...
```{r, eval=T}
## write out seurat bpcells object files for normalization processing in farm
imap(all_msc_qcd_snglt_stgd, ~saveRDS(.x, paste0("data/processed/Pf/", str_extract(.y, "MSC[0-9]+"),"/seu_obj/qcd_snglt_stgd_raw", str_remove(.y, "MSC[0-9]+"), ".RDS")))
# imap(all_msc_filt, ~saveRDS(.x, paste0("data/processed/Pf/", str_extract(.y, "MSC[0-9]+"),"/seu_obj/filt_seu_raw", str_remove(.y, "MSC[0-9]+"), ".RDS")))
...
# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_stdnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/{MSC48,MSC68,MSC14}/seu_obj/qcd_snglt_stgd_raw{_edrops100,}_sct.RDS" --normlzn "norm" --umap_name "UMAP_" --hvg 500 --cluster_resoln 0.1 --cl_nm "seurat_clusters_nm" --umap_redctn_nm "umap" --redctn "pca" --integrtd_pc30 "no" --findpc_mthd "perpendicular line" --ncores "2" --mem "50 GB"  

# nextflow run ../mali22_code_lnk/nf_scripts/submsn_seu_norm.nf --scrpt "/nfs/users/nfs_j/jr35/multipurpose_scripts/seurat_sce/seu_sctnorm.R" --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/{MSC48,MSC68,MSC14}/seu_obj/qcd_snglt_stgd_raw{_edrops100,}.RDS" --normlzn "sct" --umap_name "UMAP_" --hvg 400 --cluster_resoln 0.1 --cl_nm "seurat_clusters_sct" --umap_redctn_nm "umap" --redctn "pca" --integrtd_pc30 "no" --pca_redctn_nm "pca" --findpc_mthd "perpendicular line" --ncores "3" --mem "50 GB" --resume 
...
##Read BPcells matrices
all_msc_stg_qcd <- list.files("data/processed/Pf", pattern = "^qcd_snglt_stgd_raw.*_sct.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj.*qcd_snglt_stgd_raw|_sct.*'))) %>% 
  .[sample_name_nms] %>%
  map(readRDS)
```
...
imap(all_msc_stg_qcd, ~fp_fn(obj = .x, rdctn = "pca") %>% 
       plot_grid(ggdraw() + draw_label(.y), . ,ncol = 1, rel_heights = c(0.1, 1)))
```


```{r}
saveRDS(all_msc_stg_qcd, 'data/processed/Pf/m2_all/all_msc_stg_qcd_edrops.RDS')
```
...
# gogo()
# all_msc_qcd_snglt_stgd2 <- readRDS("data/processed/Pf/m2_all/all_msc_qcd_snglt_stgd_edrops.RDS")
