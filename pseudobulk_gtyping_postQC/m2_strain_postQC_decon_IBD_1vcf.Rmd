...
source("mali22_code_lnk/pseudobulk_gtyping_postQC/postQC_pbulk_gtypes_headers/m2_pbulk_gtypes_postQC_1VCF_final_header.R")
...
```{r}
ss_afm_pf7M_cln_gt_calledin5_maf.01 <- map(ss_afm_pf7M_cln_gt, ~{
  
  df = .x
  
  ## 1. Extract genotype matrix
  geno <- as.matrix(df[ , grep("^M", names(df))])
  geno <- matrix(as.numeric(geno), nrow = nrow(geno))
  
  ## 2. Sample-level QC: keep samples with ≥100 called SNPs
  sample_called <- colSums(!is.na(geno))
  keep_samples <- sample_called >= 100
  geno <- geno[ , keep_samples, drop = FALSE]
  
  ## 3. Site-level stats
  n_samples <- ncol(geno)
  
  n_called <- rowSums(!is.na(geno))
  alt_freq <- rowMeans(geno == 1, na.rm = TRUE)
  maf <- pmin(alt_freq, 1 - alt_freq)
  
  ## 4. Site-level QC: call rate + MAF
  keep_sites <- n_called >= ceiling(0.5 * n_samples) & maf > 0.05
  geno <- geno[keep_sites, , drop = FALSE]
  
  ## 5. Keep samples with ≥100 called SNPs
  sample_called <- colSums(!is.na(geno))
  geno <- geno[ , sample_called >= 100, drop = FALSE]
  
  ## 5. (Optional) keep metadata in sync
  df_qc <- df[keep_sites, c(
    "id", "chrom", "pos", "alleles", "chr",
    names(df)[grep("^M", names(df))][keep_samples]
  )]
...
## save clean with maf filters
saveRDS(dcf_pm, "data/processed/Pf/dcf_pm_cln_maf.rds")
...
## bsub -Is -q normal -n 20 -M 100000 -R "span[hosts=1] select[mem>100000] rusage[mem=100000]" R
...
```{r, eval=F}
# ### !!!!!!! NOTE - Similar as above but for clean filtered genotypes
# library(dplyr)
# library(dcifer)
# library(purrr)
# 
# setwd("/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/")
# dcf_pm_cln_maf <- readRDS("data/processed/Pf/dcf_pm_cln_maf.rds")
# #
# dsmp_pm_cln_maf <- formatDat(dcf_pm_cln_maf, svar = "sampleID", lvar = "locus", avar = "allele")
# saveRDS(dsmp_pm_cln_maf, "data/processed/Pf/dsmp_pm_cln_maf.rds")
# #
# coi_pm_cln_maf  <- rep(1, dim(dcf_pm_cln_maf)[1]) %>% set_names(paste0(dcf_pm_cln_maf$sampleID,".", dcf_pm_cln_maf$locus))
# saveRDS(coi_pm_cln_maf, "data/processed/Pf/coi_pm_cln_maf.rds")
# #
# afreq_pm_cln_maf <- calcAfreq(dsmp_pm_cln_maf, coi_pm_cln_maf, tol = 1e-5)
# str(afreq_pm_cln_maf, list.len = 2)
# saveRDS(afreq_pm_cln_maf, "data/processed/Pf/afreq_pm_cln_maf.rds")
# #
# dres0_pm_cln_maf <- ibdDat(dsmp_pm_cln_maf, coi_pm_cln_maf, afreq_pm_cln_maf, pval = TRUE, confint = TRUE, rnull = 0,
#                 alpha = 0.05, nr = 1e3)
# saveRDS(dres0_pm_cln_maf, "data/processed/Pf/dres0_pm_cln_maf.rds")
```
...
## save clean with maf filters
saveRDS(dcf_pm_ag, "data/processed/Pf/dcf_pm_ag_cln_maf.rds")
...
# dcifer
# module load HGI/softpack/groups/team222/Pf_scRNAseq/34
# bsub -Is -q normal -n 10 -M 100000 -R "span[hosts=1] select[mem>100000] rusage[mem=100000]" R
...
# 
# setwd("/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/")
# dcf_pm_ag_cln_maf <- readRDS("data/processed/Pf/dcf_pm_ag_cln_maf.rds")
# 
# dsmp_pm_ag_cln_maf <- formatDat(dcf_pm_ag_cln_maf, svar = "sampleID", lvar = "locus", avar = "allele")
# saveRDS(dsmp_pm_ag_cln_maf, "data/processed/Pf/dsmp_pm_ag_cln_maf.rds")
# 
# coi_pm_ag_cln_maf  <- rep(1, dim(dcf_pm_ag_cln_maf)[1]) %>% set_names(paste0(dcf_pm_ag_cln_maf$sampleID,".", dcf_pm_ag_cln_maf$locus))
# saveRDS(coi_pm_ag_cln_maf, "data/processed/Pf/coi_pm_ag_cln_maf.rds")
# 
# afreq_pm_ag_cln_maf <- calcAfreq(dsmp_pm_ag_cln_maf, coi_pm_ag_cln_maf, tol = 1e-5)
# str(afreq_pm_ag_cln_maf, list.len = 2)
# saveRDS(afreq_pm_ag_cln_maf, "data/processed/Pf/afreq_pm_ag_cln_maf.rds")
# 
# dres0_pm_ag_cln_maf <- ibdDat(dsmp_pm_ag_cln_maf, coi_pm_ag_cln_maf, afreq_pm_ag_cln_maf, pval = TRUE, confint = TRUE, rnull = 0,
#                 alpha = 0.05, nr = 1e3)
# saveRDS(dres0_pm_ag_cln_maf, "data/processed/Pf/dres0_pm_ag_cln_maf.rds")
```
...
dsmp_pm <- readRDS("data/processed/Pf/dsmp_pm_cln_maf.rds")

# coi_pm <- readRDS("data/processed/Pf/coi_pm.rds")

# afreq_pm <- readRDS("data/processed/Pf/afreq_pm.rds")

dres0_pm <- readRDS("data/processed/Pf/dres0_pm_cln_maf.rds")
```
...
```{r}
colnames(est) <- str_replace(colnames(est), "C_", "_")
rownames(est) <- str_replace(rownames(est), "C_", "_")

colnames(pval) <- str_replace(colnames(pval), "C_", "_")
rownames(pval) <- str_replace(rownames(pval), "C_", "_")
...
```{r, fig.height=18.5, fig.width=15, eval=T}
  ...
  str2remove_rowcolnms = "M[0-9]+_|_.$|SLO_|MACS_"
...
diag(p_mtx) <- 1
  ...
  # fa = str_remove(colnames(sibd_pht_mtx), '_SC.*')
  fa = str_remove_all(colnames(sibd_pht_mtx), '_SC.*|_SLO|_MACS')
  c_d = cluster_within_group(sibd_pht_mtx, fa)
  ...
  fr = str_remove_all(rownames(sibd_pht_mtx), '_SC.*|_SLO|_MACS')
  print(fr)
  c_r = cluster_within_group(sibd_pht_mtx, fr)
  ...
  fa_df <- data.frame("rname" = colnames(sibd_pht_mtx), "donor" = str_remove_all(colnames(sibd_pht_mtx), '_SC.*|_SLO|_MACS')) %>% left_join(all_pf_status, by = "donor") %>% column_to_rownames("rname")
  
  # nms = unique(str_extract(colnames(sibd_pht_mtx), 'M[0-9]+'))
  nms = unique(str_remove_all(colnames(sibd_pht_mtx), '_SC.*|_SLO|_MACS'))
  # sample_c_cols <- met.brewer("Thomas")[1:length(nms)] %>% set_names(nms)
...
# dsmp_pm_ag <- readRDS("data/processed/Pf/dsmp_pm_ag_cln_maf.rds")
dsmp_pm_ag <- readRDS("data/processed/Pf/dsmp_pm_ag_cln_maf.rds")
...
# dres0_pm_ag <- readRDS("data/processed/Pf/dres0_pm_ag_cln_maf.rds")
dres0_pm_ag <- readRDS("data/processed/Pf/dres0_pm_ag_cln_maf.rds")
...
```{r}
# optionally, extract location information
meta_ag <- data.frame("sampleID" = str_replace_all(names(dsmp_pm_ag ), "C_", "_"), "donor" = str_remove_all(names(dsmp_pm_ag ), '_SC.*|C')) %>% 
  left_join(all_pf_status, by = "donor")# %>% 
  # column_to_rownames("rname")
...
## Clean donor names to remove 'SLO and C'
colnames(est_ag) <- str_replace(str_remove(colnames(est_ag), "_SLO"), "C_", "_")
rownames(est_ag) <- str_replace(str_remove(rownames(est_ag), "_SLO"), "C_", "_")

strn_numbers <- clustr_numbers_ag_m1_m2
## Remove strain clusters with less than 150 cells
strain_ag_150 <- colnames(est_ag)[colnames(est_ag) %in% (strn_numbers %>% filter(strain_ag_n > 200) %>% pull(sample_cluster))]
est_ag <- est_ag[strain_ag_150, strain_ag_150]
...
colnames(pval_ag) <- str_replace(str_remove(colnames(pval_ag), "_SLO"), "C_", "_")
rownames(pval_ag) <- str_replace(str_remove(rownames(pval_ag), "_SLO"), "C_", "_")

## Remove strain clusters with less than 150 cells
pval_ag <- pval_ag[strain_ag_150, strain_ag_150]
...
```{r}
## Remove clusters with less than 150 cells
strn_numbers <- clustr_numbers_ag_m1_m2
strain_ag_150 <- colnames(est_a)[colnames(est_a) %in% (strn_numbers %>% filter(strain_ag_n > 200) %>% pull(sample_cluster))]
...
```{r, fig.height=12.5, fig.width=11, eval=T}
  ...
  strn_numbers = clustr_numbers_ag_m1_m2 
  # strn_numbers = strn_numbers %>% filter(strain_ag_n > 150)
  strn_number_col = "strain_ag_n" 
  str2remove_rowcolnms = "M[0-9]+_|SLO_|MACS_"
