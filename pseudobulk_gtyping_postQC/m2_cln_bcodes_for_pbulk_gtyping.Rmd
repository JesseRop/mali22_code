...
```{r}
msc_w_cln_strns_raw <- SplitObject(m2_all_raw_mrg, split.by = "donor")
...
```{r}
msc_w_cln_strns_raw <- map(msc_w_cln_strns_raw, ~.x@meta.data %>% 
      mutate(stage = str_remove_all(stage, "\\(|\\)")) %>%
      select(stage) %>%
      AddMetaData(.x, .))
...
stg_strns_cln <- map(msc_w_cln_strns_raw, ~{
  .x@meta.data %>% 
    ...
    mutate(bcode = str_remove(bcode, ".*_")) %>%
    select(bcode, Strain_DN, stage_ag, stage) #%>% 
    # AddMetaData(.x,.)
})
...
## Get donors with more than 1 Strain_DN
...
for (s in names(optimum_k_polyk)) {
  ...
  for (k in narrow_k) {
    print(narrow_k)
    # for (str_q in c('Strain_DN')){
      
      ss_qcd_ano <- map(strn_mdata_tbl[s], ~.x %>% rename("str" = Strain_DN) %>% group_by(str) %>% filter(n() >= Strain_qc_cutoff) %>% ungroup() %>% mutate(across(c(str), ~droplevels(factor(.)))) %>% split(f = ~ str, drop = T))
      ...
      # for(stg_c in c('stage_ag')){
        ...
        ss_qcd_ano <- map(strn_mdata_tbl[s], ~.x %>% rename("str" = Strain_DN, "stg" = stage_ag) %>% group_by(str, stg) %>% filter(n() >= Strain_qc_cutoff) %>% ungroup() %>% mutate(across(c(str, stg), ~droplevels(factor(.)))) %>% split(f = ~ str+stg, drop = T))
        names(ss_qcd_ano[[1]]) <- names(ss_qcd_ano[[1]]) %>% str_replace(., '\\.', '_')
        print(names(ss_qcd_ano[[1]]))
...
#NOTE - LATEST RUN
# nextflow run ../mali22_code_lnk/nf_scripts/pbulk_gtyping_1vcf_no_tag.nf --sv_dir "pbulk_gtypes_cln_1VCF" --bam "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/pbulk_gtypes_cln_1VCF/st*/new_bams/*_sset_sorted_rg_don.bam" --bcodes "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/pbulk_gtypes_cln_1VCF/bcodes/minmap/st*/*tsv" --odir "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all" --ref  "/lustre/scratch126/tol/teams/lawniczak/users/jr35/genomes_gtfs_indexs/masked_pf_ref/PlasmoDB-68_Pfalciparum3D7_Genome_VAR_masked.fasta" -resume
