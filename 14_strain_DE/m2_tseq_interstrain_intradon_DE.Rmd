...
source("/nfs/users/nfs_j/jr35/Mali2/mali22_code/strain_DE/strain_DE_common_fns.R")
```
...
m2_asex_bal_sce <- map2(m2_asex_sce, balanced_cells_tbl, ~.x[, .y$cell_bc])
...
umap_meta_df %>%
    mutate(Strain = factor(case_when(donor == "MSC68C" & ptime_bal_cat %in% "ptime_2" ~ strain, 
                                     T ~ "Other"), 
                           levels = strain_nms_lvls),
           strn_rank = factor(case_when(donor == "MSC68C" & ptime_bal_cat %in% "ptime_2" ~ "Strain_rnk", 
                                        T ~ "Other"), 
                              levels = c("Strain_rnk", "Other")),
           pt_set = paste0("MSC68C", "_", "1")) %>%
...
umap_meta_df %>%
    mutate(Strain = factor(case_when(donor == "MSC68C" & ptime_bal_cat %in% "ptime_2" ~ strain, 
           ...
           strn_rank = factor(case_when(donor == "MSC68C" & ptime_bal_cat %in% "ptime_2" ~ "Strain_rnk", 
                                        T ~ "Other"), 
                              levels = c("Strain_rnk", "Other")),
           pt_set = paste0(donor, "_", ptime_bal_cat)) %>%
    ggplot(., aes(x = umapharmonyPCman_1, y = umapharmonyPCman_2, colour = Strain, size = Strain))+
    geom_point() +
    scale_color_manual(values = strain_cols_n)+
    scale_size_manual(values = strn_sizes) 
...
```
```{r, results="asis", fig.width= 3.5, fig.height=2.8}



# DimPlot(m2_asex_harmony_tr_pt_sset, reduction = "umap.harmony_PC_man", group.by = "stage", split.by = "Symptomatic", pt.size = 0.1)+
#   scale_color_manual(values = sp_fld_colors) +
#   theme(axis.text=element_blank(), 
#         axis.title=element_blank(),
#         legend.position = "bottom",
...
```{r}
asex_bal_de <- list.files('data/processed/Pf', pattern = "m2_asex_bal_de.RDS$", full.names = T, recursive = T, include.dirs = T) %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/tradeseq.*'))) %>%
  .[names(m2_asex_bal_sce)] %>%
  map(readRDS)
```
...
saveRDS(asex_bal_de_adj, 'data/processed/Pf/asex_bal_de_adj.RDS')
```
...
```{r}
##save condition results for GSEA
saveRDS(cond_gns_de_asx, 'data/processed/Pf/cond_gns_de_asx.RDS')
```
