...
v3_m2_asex_cr_filt_norm_harmony_pt <- v3_m2_asex_cr_filt_norm_harmony_pt@meta.data %>% 
  ...
  AddMetaData(v3_m2_asex_cr_filt_norm_harmony_pt, .)
```
...
bal_df <- ptime_chunk_calc_df_fn(sc_df = v3_m2_asex_cr_filt_norm_harmony_pt@meta.data, ptime = "slingPseudotime_1", chunk_num = 20, p_cat_nm = "downsampling_ptime_cat", p_bal_cat_nm = "downsampling_ptime_bal_cat")
```
...
```{r}
## subset where field parasites are for slingshot
DimPlot(v3_m2_asex_cr_filt_norm_harmony_pt, reduction = "umap.harmony_PC_man", group.by = "downsampling_ptime_cat" )
DimPlot(v3_m2_asex_cr_filt_norm_harmony_pt, reduction = "umap.harmony_PC_man", group.by = "downsampling_ptime_bal_cat" )
```
...
```{r}
DimPlot(v3_m2_asex_cr_filt_norm_harmony_pt, reduction = "umap.harmony_PC_man", group.by = "downsampling_ptime_bal_cat", cols = ptc_cols, label = T ) +
  theme(legend.position = "none")
```

```{r}
saveRDS(v3_m2_asex_cr_filt_norm_harmony_pt, "data/processed/Pf/m2_all/strain_DE/v3_m2_asex_cr_filt_norm_harmony_pt_downsampling_chunks.RDS")
```


```{r}
# Count the donors with atleast 2 strrains each with atleast 100 cells
don_100n <- v3_m2_asex_cr_filt_norm_harmony_pt@meta.data %>% count(orig.ident, StrainDon) %>% filter(n>100) %>% group_by(orig.ident) %>% filter(n_distinct(StrainDon) > 1) %>% pull(orig.ident) %>% unique() %>% str_remove(., ".mrna")
```
...
# m2_asex_seu <- map(dons_bal_sset_nms, ~v3_m2_asex_cr_filt_norm_harmony_pt[, str_detect(v3_m2_asex_cr_filt_norm_harmony_pt@meta.data$orig.ident,.x) & !str_detect(v3_m2_asex_cr_filt_norm_harmony_pt@meta.data$StrainDon, "Negative|Doublet")]) %>% set_names(dons_bal_sset_nms)

m2_asex_seu <- v3_m2_asex_cr_filt_norm_harmony_pt[, str_detect(v3_m2_asex_cr_filt_norm_harmony_pt@meta.data$orig.ident, paste(dons_bal_sset_nms, collapse = "|")) & !str_detect(v3_m2_asex_cr_filt_norm_harmony_pt@meta.data$StrainDon, "Negative|Doublet")]
...
# balanced_cells_tbl <- map2(m2_asex_seu, ds_grp, ~balanced_cells_fn(sc_df = .x@colData, dsample_condtn = .y , ptime_strategy = c("donor", "ptime_bal_cat"), multpl = 1, don_yes_no = "no"))
balanced_cells_tbl <- balanced_cells_fn(sc_df = m2_asex_seu@meta.data, dsample_condtn = "strain" , ptime_strategy = c("donor", "downsampling_ptime_bal_cat"), multpl = 1, don_yes_no = "no")
```
...
# m2_asex_bal_seu <- map2(m2_asex_seu, balanced_cells_tbl, ~.x[, .y$cell_bc])

m2_asex_bal_seu <- m2_asex_seu[, balanced_cells_tbl$cell_bc]

```

```{r}
# map(m2_asex_bal_seu, ~.x@colData[,c("ptime_bal_cat", "StrainDon")] %>% data.frame() %>% dplyr::count(ptime_bal_cat, StrainDon))
m2_asex_bal_seu@meta.data %>% data.frame() %>% dplyr::count(downsampling_ptime_bal_cat, StrainDon)
...
# v3_m2_asex_harmony_sce_ss_df2 <- map(m2_asex_bal_seu, ~ptime_chunk_calc_df_fn(sc_df = .x@meta.data, ptime = "pseudotime", chunk_num = 7, p_cat_nm = "ptime_cat", p_bal_cat_nm = "ptime_bal_cat"))
# m2_asex_bal_seu_df2 <- map(m2_asex_bal_seu, ~ptime_chunk_calc_df_fn(sc_df = .x@meta.data, ptime = "pseudotime", chunk_num = 7, p_cat_nm = "ptime_cat", p_bal_cat_nm = "ptime_bal_cat"))
# m2_asex_bal_seu_df2 <- ptime_chunk_calc_df_fn(sc_df = m2_asex_bal_seu@meta.data, ptime = "pseudotime", chunk_num = 8, p_cat_nm = "ptime_cat", p_bal_cat_nm = "ptime_bal_cat")
bal_df2 <- ptime_chunk_calc_df_fn(sc_df = m2_asex_bal_seu@meta.data, ptime = "slingPseudotime_1", chunk_num = 7, p_cat_nm = "ptime_cat", p_bal_cat_nm = "ptime_bal_cat")
...
```{r, warning=FALSE, message=FALSE}
## Calculate pseudotime chunks
# v3_m2_asex_cr_filt_norm_harmony_pt_bal <- map2(v3_m2_asex_harmony_sce_ss_df2, m2_asex_bal_seu ~AddMetaData(.x, .y) )
m2_asex_bal_seu <- AddMetaData(m2_asex_bal_seu, bal_df2) 

```

```{r}
# v3_m2_asex_cr_filt_norm_harmony_pt_no_rings <- m2_asex_bal_seu
...
# m2_asex_bal_seu <- m2_asex_bal_seu[, !m2_asex_bal_seu$harmony_clusters_ss %in% c("1")]
# m2_asex_bal_seu <- v3_m2_asex_cr_filt_norm_harmony_pt_bal

```

```{r}
## subset where field parasites are for slingshot
m2_asex_bal_seu@meta.data %>% count(ptime_bal_cat, donor, strain)
```

```{r}
## subset where field parasites are for slingshot
DimPlot(m2_asex_bal_seu, reduction = "umap.harmony_PC_man", group.by = "ptime_cat" , label = T )
DimPlot(m2_asex_bal_seu, reduction = "umap.harmony_PC_man", group.by = "ptime_bal_cat" , label = T)
```
...
m2_asex_bal_seu <- m2_asex_bal_seu[, m2_asex_bal_seu@meta.data %>% 
...
```{r}
## subset where field parasites are for slingshot
m2_asex_bal_seu@meta.data %>% count(donor,ptime_bal_cat,  strain) %>% group_by(donor,ptime_bal_cat) %>% mutate(strn_n = n_distinct(strain)) %>% filter(strn_n >1)
```


```{r}
## subset where field parasites are for slingshot
m2_asex_bal_seu@meta.data %>% count( donor, strain) %>% group_by( donor) %>% mutate(strn_n = n_distinct(strain)) %>% filter(strn_n >1)
```
...
Remove the strain that are overal less than 100 in a donor
```{r}
# m2_asex_ap2g_bal_sce <- m2_asex_ap2g_bal_sce[, m2_asex_ap2g_bal_sce@colData[, c('ap2g_exp_all', 'ptime_bal_cat')] %>% 
m2_asex_bal_seu <- m2_asex_bal_seu[, m2_asex_bal_seu@meta.data %>% 
                                               rownames_to_column('bcode') %>% 
...
```{r}
DimPlot(m2_asex_bal_seu, reduction = "umap.harmony_PC_man", group.by = "ptime_bal_cat", cols = ptc_cols2, label = T ) +
  theme(legend.position = "none")

```

```{r, warning=FALSE, message=FALSE, fig.width=5, fig.height=3}
DimPlot(m2_asex_bal_seu, reduction = "umap.harmony_PC_man", group.by = "ptime_bal_cat", label.size = 4, label.box = T, label = T) +
  theme_classic() +
...
```{r}
m2_asex_bal_seu@meta.data %>% count( ptime_bal_cat) 
...
```{r, eval=FALSE}
DimPlot(m2_asex_bal_seu, reduction = "umap.harmony_PC_man", group.by = "ptime_bal_cat" )
```
...
# m2_asex_bal_seu <- m2_asex_bal_seu
m2_asex_bal_seu[["SCT"]] <- NULL
# m2_asex_bal_seu[["RNA"]]$scale.data <- NULL
DefaultAssay(m2_asex_bal_seu) <- "RNA"

```


```{r, message=FALSE, warning=FALSE}
m2_asex_bal_seu <- JoinLayers(m2_asex_bal_seu, layers = c("counts", "data", "scale.data"))
```
...
```{r, message=FALSE, warning=FALSE}
# Add to metadata the dataset/conditions and ptime cat
m2_asex_bal_seu@meta.data[, "don_ptime"] <- paste0(m2_asex_bal_seu$donor, "_", m2_asex_bal_seu$ptime_bal_cat)

```

```{r, message=FALSE, warning=FALSE}
# Save for tradeseq and plotting 
saveRDS(m2_asex_bal_seu, 'data/processed/Pf/m2_all/m2_asex_bal_seu.RDS')

```

```{r, message=FALSE, warning=FALSE}
# Add to metadata the dataset/conditions and ptime cat
# all_asex_de_bal_nosct <- SplitObject(m2_asex_bal_seu, split.by = "don_ptime")
all_asex_de_bal_nosct <- SplitObject(m2_asex_bal_seu, split.by = "donor")
...
```{r, message=FALSE, warning=FALSE}
de_idents <- rep("strain", length = length(asx_m2_bal_seu))
...
# pts_all<- c(rep(pts,1))
pts_all<- rep(list(pts),length(asx_m2_bal_mast))
# pts_all<- pts

### !!! NOTE - RUNNING FOR NECESSARY DONORS, WILL REVERT LATER
asx_pairwise_mkrs_seu_clustr <- list(asx_m2_bal_mast, pts_all, de_idents, names(asx_m2_bal_mast)) %>%
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=3.5}
DimPlot(m2_asex_bal_seu, split.by = "donor", reduction = "umap.harmony_PC_man")
```

```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=3.5}
imap(likely_cnvs_don, possibly(~FeaturePlot(subset(m2_asex_bal_seu, subset = donor == .y), features = .x, split.by = "strain", reduction = "umap.harmony_PC_man", order = T)))
```
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=3.5}
pmap(list(likely_cnvs_don$donor, likely_cnvs_don$gene_id, likely_cnvs_don$labl), possibly(~DotPlot(subset(v3_m2_asex_cr_filt_norm_harmony_pt, subset = donor == ..1), features = ..2, group.by = "strain") + labs(title = ..3)))
```

```{r, warning=FALSE, message=FALSE, fig.width=20, fig.height=3.5}
pmap(list(likely_cnvs_don$donor, likely_cnvs_don$gene_id, likely_cnvs_don$labl), possibly(~FeaturePlot(subset(v3_m2_asex_cr_filt_norm_harmony_pt, subset = donor == ..1), features = ..2, split.by = "strain", reduction = "umap.harmony_PC_man", order = F) + labs(title = ..3)))
```
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=3.5}
imap(top_gns_list, possibly(~DotPlot(subset(v3_m2_asex_cr_filt_norm_harmony_pt, subset = donor == .y), features = .x, group.by = "strain") + labs(title = .y)))
```
