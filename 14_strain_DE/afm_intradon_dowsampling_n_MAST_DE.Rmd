...
# m2_asex_seu <- map(dons_bal_sset_nms, ~afm_leiden_w_donors_pt[, str_detect(afm_leiden_w_donors_pt@meta.data$orig.ident,.x) & !str_detect(afm_leiden_w_donors_pt@meta.data$StrainDon, "Negative|Doublet")]) %>% set_names(dons_bal_sset_nms)
...
```{r}
# m2_afm_sce <- c(m2_afm_sce[dons_bal_sset_nms], m2_afm_tr_sce_pt_chunks_sshot) %>% set_names(c(dons_bal_sset_nms, "FL"))
# m2_afm_sce <- m2_afm_sce
...
# m2_afm_bal_seu <- m2_afm_seu[, balanced_cells_tbl$cell_bc]
m2_afm_bal_seu <- map2(m2_afm_seu, balanced_cells_tbl, ~.x[, .y$cell_bc])

```

```{r}
# map(m2_afm_bal_seu, ~.x@colData[,c("ptime_bal_cat", "StrainDon")] %>% data.frame() %>% dplyr::count(ptime_bal_cat, StrainDon))
...
```{r, warning=FALSE, message=FALSE}
## Calculate pseudotime chunks
# afm_leiden_w_donors_pt_bal <- map2(v3_m2_asex_harmony_sce_ss_df2, m2_asex_bal_seu ~AddMetaData(.x, .y) )
m2_afm_bal_seu <- map2(m2_afm_bal_seu, bal_df2, ~AddMetaData(.x, .y) )

```

```{r}
# afm_leiden_w_donors_pt_no_rings <- m2_afm_bal_seu
...
# m2_afm_bal_seu <- m2_afm_bal_seu[, !m2_afm_bal_seu$harmony_clusters_ss %in% c("1")]
# m2_afm_bal_seu <- afm_leiden_w_donors_pt_bal
...
```{r}
# m2_afm_ap2g_bal_sce <- m2_afm_ap2g_bal_sce[, m2_afm_ap2g_bal_sce@colData[, c('ap2g_exp_all', 'ptime_bal_cat')] %>% 
...
# # m2_afm_bal_seu <- m2_afm_bal_seu
# m2_afm_bal_seu[["SCT"]] <- NULL
# # m2_afm_bal_seu[["RNA"]]$scale.data <- NULL
# DefaultAssay(m2_afm_bal_seu) <- "RNA"
...
```{r, message=FALSE, warning=FALSE}
# Add to metadata the dataset/conditions and ptime cat
# m2_afm_bal_seu@meta.data[, "don_ptime"] <- paste0(m2_afm_bal_seu$donor, "_", m2_afm_bal_seu$ptime_bal_cat)
...
# DimPlot(afm_leiden_w_donors_pt, split.by = "donor", reduction = "umap.harmony_PC_man")
```

```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=3.5}
# DimPlot(m2_afm_bal_seu, split.by = "donor", reduction = "umap.harmony_PC_man")
```
