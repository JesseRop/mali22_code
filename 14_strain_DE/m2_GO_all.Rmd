...
# all_sig_asx_gns <- map(mast_tde_corr_tbls[solo_don], ~{
#   .x %>% 
...
```{r, results='asis', warning=F, message=F}
all_sig_asx_gns <- mast_tde_corr_tbls_sig
```
...
all_sig_gns_df <- map(list(#all_sig_f_gns, all_sig_m_gns, 
                           all_sig_asx_gns), ~{
  .x %>%
  bind_rows(., .id = "donor") %>%
  group_by(gene_id) %>%
...
```{r, results='asis', warning=F, message=F}
DE_genes_freq <- map(all_sig_gns_df, ~{.x %>% 
...
```{r, results='asis', warning=F, message=F}
saveRDS(all_sig_gns_df, "data/processed/Pf/m2_all/strain_DE/all_sig_gns_df.RDS")
```

```{r, results='asis', warning=F, message=F}
saveRDS(DE_genes_freq, "data/processed/Pf/m2_all/strain_DE/DE_genes_freq.RDS")
```
...
fem_gns_don <- map2(c( all_sig_gns_df["asx"]), c(2), ~{.x %>% 
