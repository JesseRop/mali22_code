...
```{r}
# Generated in 'RStudio Server.Rmd' for plotting
afm_leiden_w_donors_pt_downsampling_chunks <- map(stg_nms, ~readRDS(paste0(read_dir, "afm_leiden_w_donors_pt_downsampling_chunks_", .x, ".RDS")))

```

```{r}
map(afm_leiden_w_donors_pt_downsampling_chunks, ~DimPlot(.x, reduction = "umap.harmony_PC_man", group.by = "downsampling_ptime_bal_cat", cols = ptc_cols, label = T ) +
  theme(legend.position = "none"))
```


```{r, warning=FALSE, message=FALSE, fig.width=3.5, fig.height=3}
map(afm_leiden_w_donors_pt_downsampling_chunks, ~DimPlot(.x, reduction = "umap.harmony_PC_man", group.by = "downsampling_ptime_bal_cat", label.size = 5, label.box = T, label = T, label.color = "white", repel = F) +
  geom_text(aes(label=paste0("N=", dim(afm_leiden_w_donors_pt_downsampling_chunks)[2]), x = 1, y = 4), size = 8, fontface ='plain')+
  theme_classic() +
...
m2_afm_bal_seu_plot <- readRDS(paste0(read_dir, 'm2_afm_bal_seu.RDS'))
...
```{r}
m2_afm_bal_seu_plot <- map(m2_afm_bal_seu_plot, ~.x@meta.data %>%
...
```{r, warning=FALSE, message=FALSE, fig.width=3.5, fig.height=3}
map(m2_afm_bal_seu_plot, ~DimPlot(.x, reduction = "umap.harmony_PC_man", group.by = "ptime_bal_cat", label.size = 6, label.box = T, label = T, label.color = "black") +
  geom_text(aes(label=paste0("N=", dim(.x)[2]), x = 1, y = 4), size = 8, fontface ='plain')+
  theme_classic() +
...
map(m2_afm_bal_seu_plot, ~.x@meta.data %>%
  count( donor,  strain) %>% 
...
umap_meta_df_bal_cells_plot <- readRDS(paste0(read_dir, 'umap_meta_df_bal_cells_plot.RDS'))
```

```{r, fig.width=10, fig.height=8}
umap_meta_df_bal_cells_plot_cln <- map(umap_meta_df_bal_cells_plot, ~.x %>%
  mutate(across(pt_set, ~str_replace(str_remove(., "SC"), "C_", "_"))))

```

```{r, fig.width=10, fig.height=8}
umap_meta_df_bal_cells_plot_cln_labels <- map(umap_meta_df_bal_cells_plot_cln, ~.x %>%
...
```{r, fig.width=10, fig.height=8}
map2(umap_meta_df_bal_cells_plot_cln, umap_meta_df_bal_cells_plot_cln_labels, ~.x %>%
...
map(umap_meta_df_bal_cells_plot_cln, ~.x %>%
...
map(m2_afm_bal_seu_plot, possibly(~FeaturePlot(.x[, .x$donor == "MSC49C" & .x$strain == "SC2"], reduction = "umap.harmony_PC_man", features = "pseudotime", pt.size = 2) +
  # geom_text(aes(label=paste0("N=", dim(m2_afm_bal_seu_plot)[2]), x = 1, y = 4), size = 8, fontface ='plain')+
...
m2_asex_bal_seu_plot_df <- map(m2_afm_bal_seu_plot, ~.x@reductions$umap.harmony_PC_man@cell.embeddings[,c(1,2,3)] %>% 
  as.data.frame() %>%
  rownames_to_column("cell_bc") %>%
  left_join(., .x@meta.data[,c("stage_fld", "slingPseudotime_1",  "orig.ident", "dset", "donor", "strain")] %>%
              as.data.frame() %>% rownames_to_column("cell_bc"), by = "cell_bc") %>% mutate(tr = 0)
)
```

```{r, fig.width=4, fig.height=4}
map(m2_asex_bal_seu_plot_df, ~.x %>%
  ggplot(., aes(x = umapharmonyPCman_1, y = umapharmonyPCman_2))+
      geom_point(data = .x %>% filter(donor == "MSC49C"), aes(x = umapharmonyPCman_1, y = umapharmonyPCman_2, colour = slingPseudotime_1), inherit.aes = F) +
      # scale_size_manual(values = strn_sizes) +
      # geom_text(data = umap_meta_df_bal_cells_plot_cln_labels %>% filter(donor == "MSC49C") , 
      #           aes(x = 1, y = y_pos, vjust = y_vjust-4.6, label = strain_n), 
      #           size = 5#, 
...
map(m2_asex_bal_seu_plot_df, ~.x %>%
  ggplot(., aes(x = umapharmonyPCman_1, y = umapharmonyPCman_2, colour = dset))+
      ...
      geom_point(data = .x %>% filter(donor == "MSC49C" & strain == "SC2"), aes(x = umapharmonyPCman_1, y = umapharmonyPCman_2, colour = slingPseudotime_1), size = 2.5, inherit.aes = F) +
      scale_color_distiller(direction = 1, palette = "Blues") + 
  theme_classic() +
 theme(#legend.position = "bottom", 
        legend.position = "none",
...
map(m2_asex_bal_seu_plot_df, ~.x %>%
  ggplot(., aes(x = umapharmonyPCman_1, y = umapharmonyPCman_2, colour = dset))+
      ...
      # scale_color_manual(values = "#ededed")+
      ggnewscale::new_scale_color() +
      geom_point(data = .x %>% filter(donor == "MSC49C" & strain == "SC5"), aes(x = umapharmonyPCman_1, y = umapharmonyPCman_2, colour = slingPseudotime_1), size = 2.5, inherit.aes = F) +
      scale_color_distiller(direction = 1, palette = "Blues") + 
  theme_classic() +
 theme(#legend.position = "bottom", 
        legend.position = "none",
...
mast_tde_corr_tbls <- readRDS(paste0(read_dir, "mast_tde_corr_tbls.RDS"))

```

```{r}
# Generated in 'm2_GO_all.Rmd' 
mast_tde_corr_tbls_sig <- readRDS(paste0(read_dir, "mast_tde_corr_tbls_sig.RDS"))
...
## Get file list
afm_bal_ts_files <- Sys.glob("data/processed/Pf/*/tradeseq_afm/[a|f|m]_afm_bal_ts.RDS")
afm_bal_ts_files
```

Read in asexual, female and male tradeseq output
```{r, message=F, warning=F, eval=T}
## read filtered barcodes
afm_bal_ts <- map(afm_bal_ts_files, ~.x %>% 
...
```{r, results='asis', warning=F, message=F}
a_groups <-  names(afm_bal_ts)[str_detect(names(afm_bal_ts), "a.MSC")]
f_groups <-  names(afm_bal_ts)[str_detect(names(afm_bal_ts), "f.MSC")]
m_groups <-  names(afm_bal_ts)[str_detect(names(afm_bal_ts), "m.MSC")]
```


```{r, fig.width=10, fig.height=12}
# generated in 'm2_GO_all.Rmd'
DE_genes_freq <- readRDS(paste0(read_dir, "DE_genes_freq.RDS"))
...
a <- imap(gns2plotsmoother[[1]][1],~{
  ...
  map(gn_tbl, ~{
  lg_map <- str_remove_all(levels(afm_bal_ts[[.x$donor]]$tradeSeq$conditions), "M.*_|M.*SLOC_") %>% set_names(paste0("Lineage 1_", levels(afm_bal_ts[[.x$donor]]$tradeSeq$conditions)))
  ...
  tde_smooth_plt_light(tde = afm_bal_ts[[.x$donor]], lege_pos = "bottom", gn = .x$gene_id, gn_name = .x$donor, curve_cols = cols) +
...
```{r, results='asis', warning=F, message=F}
# Generated in 'm2_tseq_MAST_DE_comprsn.Rmd' 
gn_tble_gg_smoother<- readRDS(paste0(read_dir, "gn_tble_gg_smoother.RDS") )
...
manual_tseq_smoothers_plot(tseq_dset = afm_bal_ts[["m.MSC60C"]], gn_xprsn_tbl = gn_tble_gg_smoother[["m.MSC60C"]], gene_id = "PF3D7-0113000", plot_ttl = "id")
```
...
a <- imap(gns2plotsmoother[[1]][1], ~{
  ...
  map(gn_tbl, ~{
    manual_tseq_smoothers_plot(tseq_dset = afm_bal_ts[[.x$donor]], gn_xprsn_tbl = gn_tble_gg_smoother[[.x$donor]], gene_id = .x$gene_id, plot_ttl = .x$donor)
...
df_curves <- predictSmooth(afm_bal_ts[["m.MSC60C"]], gene = "PF3D7-0113000", nPoints = 100, tidy = TRUE) %>% 
  mutate(strain = str_remove(condition, "M.*_"))

ggplot(gn_tble_gg_smoother[["m.MSC60C"]], aes(x = slingPseudotime_1 , y = log1p(`PF3D7-0113000`+1), colour = strain)) + 
                             geom_point() +
                             geom_line(data = df_curves, aes(x = time, y = log1p(yhat+1), colour = strain), linewidth = 1) +
                             scale_colour_manual(values = strain_cols_n) +
  labs(title = "Gene") +
...
df_curves_don <- map(afm_bal_ts[c("a.MSC33C", "a.MSC49C", "a.MSC66C")], ~predictSmooth(.x, gene = "PF3D7-0113000", nPoints = 100, tidy = TRUE) %>% 
  mutate(strain = str_remove(condition, "M.*_"))) %>% bind_rows(., .id = "donor")
gn_tble_gg_smoother_don <- gn_tble_gg_smoother[c("a.MSC33C", "a.MSC49C", "a.MSC66C")] %>% bind_rows(., .id = "donor_stg")
...
gn_tble_gg_smoother_don_all <- gn_tble_gg_smoother %>% 
  bind_rows(., .id = "donor_stg") %>% 
  select(strain, cell_bc, starts_with("PF3D7"), donor_stg, slingPseudotime_1)  %>% 
...
df_curves_don_all <- map(afm_bal_ts, ~predictSmooth(.x, gene = rownames(.x), nPoints = 100, tidy = TRUE) %>% 
  mutate(strain = str_remove(condition, "M.*_"))) %>%
  bind_rows(., .id = "donor_stg") %>% 
  rename("gene_id" = gene) %>% 
...
```{r, fig.width=8, fig.height=5}
df_curves_don <- df_curves_don_all %>% filter(gene_id %in% c("PF3D7-0113000", "PF3D7-1102700") & donor_stg %in% c("a.MSC33C", "a.MSC49C", "a.MSC66C"))

gn_tble_gg_smoother_don <- gn_tble_gg_smoother_don_all %>% 
  filter(donor_stg %in% c("a.MSC33C", "a.MSC50_SLOC", "a.MSC49C", "a.MSC66C", "a.MSC68C") & gene_id %in% c("PF3D7-0113000", "PF3D7-1102700")) 

ggplot(gn_tble_gg_smoother_don, aes(x = slingPseudotime_1 , y = log1p(expression+1), colour = strain)) + 
                             geom_point(size = 1) +
                             geom_line(data = df_curves_don, aes(x = time, y = log1p(yhat+1), colour = strain), linewidth = 1) +
                             facet_grid(gene_id ~ donor_stg, scales = "free") +
                             scale_colour_manual(values = strain_cols_n) +
  labs(title = "Gene") +
  theme_classic2()+
                             theme(legend.position="bottom") +
...
tseq_smoother_facet_allDons_plot_fn <- function(gns = top_wstat_mastLFC_noVAR$gene_id) {
  ...
  df_curves_don <- df_curves_don_all %>% filter(gene_id %in% gns & donor_stg %in% c("a.MSC33C", "a.MSC49C", "a.MSC50_SLOC", "a.MSC60C", "a.MSC66C", "a.MSC68C"))

gn_tble_gg_smoother_don <- gn_tble_gg_smoother_don_all %>% 
  filter(donor_stg %in% c("a.MSC33C", "a.MSC49C", "a.MSC50_SLOC", "a.MSC60C", "a.MSC66C", "a.MSC68C") & gene_id %in% gns) 

ggplot(gn_tble_gg_smoother_don, aes(x = slingPseudotime_1 , y = log1p(expression+1), colour = strain)) + 
                             geom_point(size = 0.5) +
                             geom_line(data = df_curves_don, aes(x = time, y = log1p(yhat+1), colour = strain), linewidth = 1.5) +
                             facet_grid2(Gene ~ donor_stg, scales = "free") +
                             scale_colour_manual(values = strain_cols_n) +
  # labs(title = "Gene") +
  theme_classic2()+
  theme(legend.position = "bottom", 
...
df_curves_don <- df_curves_don_all %>% filter(gene_id %in% top_wstat_mastLFC_noVAR$gene_id & donor_stg %in% c("a.MSC33C", "a.MSC49C", "a.MSC50_SLOC", "a.MSC60C", "a.MSC66C", "a.MSC68C"))

gn_tble_gg_smoother_don <- gn_tble_gg_smoother_don_all %>% 
  filter(donor_stg %in% c("a.MSC33C", "a.MSC49C", "a.MSC50_SLOC", "a.MSC60C", "a.MSC66C", "a.MSC68C") & gene_id %in% top_wstat_mastLFC_noVAR$gene_id) 
...
df_curves_don2 <- gns2plotsmoother_df[[1]] %>% select(donor_stg, gene_id) %>% left_join(., df_curves_don_all, by = c("donor_stg", "gene_id")) 

gn_tble_gg_smoother_don2 <- gns2plotsmoother_df[[1]] %>% 
  select(donor_stg, gene_id) %>% 
  left_join(., gn_tble_gg_smoother_don_all, by = c("donor_stg", "gene_id"))  
...
ggplot(gn_tble_gg_smoother_don2, aes(x = slingPseudotime_1 , y = log1p(expression+1), colour = strain)) + 
                             ...
                             facet_nested_wrap(vars(Gene, donor_stg), scales = "free", ncol = 6) +
                             scale_colour_manual(values = strain_cols_n) +
  # labs(title = "Gene") +
  theme_classic2()+
                             theme(legend.position="bottom") +
...
gns = all_sig_gns_df_afm[[1]] %>% 
  filter(donor == "a.MSC49C") %>%
  pull(gene_id)

# print(str(gns))
##Edited from the original version to allow the labelling of rows with gene names
msc_yhatSmooth_w_annot <- predictSmooth(afm_bal_ts[["a.MSC49C"]], gene = gns, nPoints = 30, tidy = FALSE) %>% 
...
msc_col_annot <- predictSmooth(afm_bal_ts[["a.MSC49C"]], gene = gns, nPoints = 30, tidy = T)  %>% 
  mutate(point = paste0("point", rep(rep(1:30,length(unique(afm_bal_ts[["a.MSC49C"]]@colData$tradeSeq$conditions))),length(gns))), 
...
```{r, fig.width=10, fig.height=12}
# generated in 'm2_GO_all.Rmd'
all_sig_gns_df_afm <- readRDS(paste0(read_dir, "all_sig_gns_df_afm.RDS"))
```


```{r, results='asis', warning=F, message=F, fig.width= 4, fig.height=3}
map(all_sig_gns_df_afm, ~{.x %>% 
