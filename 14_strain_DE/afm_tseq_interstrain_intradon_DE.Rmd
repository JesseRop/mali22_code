...
v3_fm_harmony_sce_ss_list_files <- fm_leiden_w_donors_pt_list_files[str_detect(fm_leiden_w_donors_pt_list_files, "w_pt7_f_sce_ss|all_m_sce_ss")]
...
```{r, warning=FALSE, message=FALSE}
v3_afm_harmony_sce_ss <- c(list(v3_m2_asex_harmony_sce_ss), v3_fm_harmony_sce_ss[c("w_pt7_f", "all_m")])
```


```{r, warning=FALSE, message=FALSE}
names(v3_afm_harmony_sce_ss) <- c("a", "f", "m")
```
######################### Read in slingshot SCEs needed for tradeseq #########################
...
bal100_de_cells <- map(fm_bal_sce, ~.x@colData[, c("ptime_bal_cat", "StrainDon")] %>% as.data.frame()) %>% bind_rows(.id = "donor") %>% mutate(bal_de = "yes") %>% rownames_to_column("cell_bc") %>% separate_wider_delim(donor, delim = ".", names = c("stage", "donor")) %>% split(., f = factor(.$stage))
...
# ## Conditions for DE in nextflow tradeseq
# cond_tbl = map(list(
#   names(fm_bal_sce)[str_detect(names(fm_bal_sce), "all_m")],
