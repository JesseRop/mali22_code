...
```{r}
asex_bal_ts_cond_renamed <- asex_bal_ts
```

```{r, warning=FALSE, message=FALSE}
for (nm in names(asex_bal_ts_cond_renamed)) {
  asex_bal_ts_cond_renamed[[nm]]$tradeSeq$conditions<- droplevels(factor(str_remove(asex_bal_ts[[nm]]$tradeSeq$conditions, "M[0-9]+_"), levels = strain_nms_lvls))
}
```
...
```{r, message=FALSE, warning=FALSE}
asx_m2_bal_mast <- readRDS('data/processed/Pf/m2_all/asx_m2_bal_mast3.RDS')
...
# asx_pairwise_mkrs_seu_clustr_sst <- asx_pairwise_mkrs_seu_clustr[c(1:5, 7:20)]
# msc_g_noLE_v3_combi_afm_seu_sst <- msc_g_noLE_v3_combi_afm_seu[c(1:5,7:20)]
asx_pairwise_mkrs_seu_clustr_sst <- asx_pairwise_mkrs_seu_clustr
msc_g_noLE_v3_combi_afm_seu_sst <- asx_m2_bal_mast
msc_g_noLE_ref_ts_sce_ub_de <- asex_bal_ts_cond_renamed
cond_gns_005_ub_de_sst <- cond_gns_de_asx
pts_sst <- pts
...
```{r}
map2(asx_pairwise_mkrs_seu_clustr_sst, msc_g_noLE_v3_combi_afm_seu_sst, ~.x %>% filter(gene_id %in% rownames(.y)[!(rowSums(.y@assays$RNA@layers$counts != 0) > 5)])) %>% bind_rows(., .id = 'Don')

```

Remove genes above ????NB NB - refer line 734
```{r}
asx_pairwise_mkrs_seu_clustr_sst <- map2(asx_pairwise_mkrs_seu_clustr_sst, msc_g_noLE_v3_combi_afm_seu_sst, ~.x %>% filter(gene_id %in% rownames(.y)[(rowSums(.y@assays$RNA@layers$counts != 0) > 5)]))
...
map(asx_pairwise_mkrs_seu_clustr_sst, ~.x %>% count(pw_comparison))
...
```{r, fig.width= 8, fig.height=3, warning=FALSE}
mast_tde_corr_tbls_w_na <- list(cond_gns_005_ub_de_sst_formated, asx_pairwise_mkrs_seu_clustr_sst) %>%
    pmap(
...
```{r}
saveRDS(mast_tde_corr_tbls, "data/processed/Pf/m2_all/strain_DE/mast_tde_corr_tbls.RDS")
...
list(asx_pairwise_mkrs_seu_clustr_sst[1]) %>%
  pmap(., ~{
...
strns_comps_w_lab =  map(asx_pairwise_mkrs_seu_clustr_sst, ~unique(.$pw_comparison)) %>% unlist() %>% unique() 
...
```{r, eval=FALSE}
list(asx_pairwise_mkrs_seu_clustr_sst[10]) %>%
  pmap(., ~{
    mast_de_tbl = ..1
    map(strns_comps, ~ mast_de_tbl %>% 
          filter(p_val_adj < 0.05  & pw_comparison== .x ) %>% 
...
list(asx_pairwise_mkrs_seu_clustr_sst[c(1)]) %>%
  pmap(., possibly( ~{
    mast_de_tbl = ..1 %>% filter(p_val_adj < 0.05) %>% arrange(p_val_adj)
    strns_comps = mast_de_tbl %>% pull(pw_comparison) %>% unique()
    print(strns_comps)
...
```{r, results='asis', warning=F, message=F}
saveRDS(gn_tble_gg_smoother, "data/processed/Pf/m2_all/strain_DE/gn_tble_gg_smoother.RDS")
saveRDS(gn_tble_gg_smoother_data, "data/processed/Pf/m2_all/strain_DE/gn_tble_gg_smoother_data.RDS")
...
tde_disparities <- list(mast_tde_corr_tbls, asx_pairwise_mkrs_seu_clustr_sst) %>%
  pmap(
    ~..1 %>%
...
tde_disparities_gns_plts <- stg_strn_mast_gns_smooth_plts(gn_xprsn_tbls = gn_tble_gg_smoother, 
                                                          gns_strn = map(tde_disparities, 
                                                                         . %>%
                                                                           ...
                                                                           dplyr::select(gene_id, pw_comparison)), 
                                                          stage_nms = names(asx_pairwise_mkrs_seu_clustr_sst), 
                                                          ptimes = pts_sst,
                                                          comp_g = 'pw_comparison',
                                                          top_gns = 8,
                                                          strain_label = "strain",
...
mast_disparities <- list(mast_tde_corr_tbls, asx_pairwise_mkrs_seu_clustr_sst) %>%
  pmap(
    ~..1 %>%
      filter(!str_detect(pw_comparison, 'SC._vs_Lab')) %>%
      filter(waldStat > 10 & p_val_adj > 0.05) 
...
top_sig_gns_plts <- stg_strn_mast_gns_smooth_plts(gn_xprsn_tbls = c( gn_tble_gg_smoother), 
                                                  gns_strn = map(asx_pairwise_mkrs_seu_clustr_sst, 
                                                                 . %>%
                                                                   filter(p_val_adj < 10e-20 & (avg_log2FC > 1 | avg_log2FC < -1))  %>% 
                                                                   arrange(p_val_adj) %>%
                                                                   dplyr::select(gene_id, pw_comparison)), 
...
p2 =EnhancedVolcano(asx_pairwise_mkrs_seu_clustr_sst[[15]] %>% filter(pw_comparison == 'SC2_vs_SC7'),
                    lab = left_join(asx_pairwise_mkrs_seu_clustr_sst[[15]], Pf3D7_genes_plasmoDB, by = 'gene_id')%>% filter(pw_comparison == 'SC2_vs_SC7') %>% pull(Gene),
                    x = 'avg_log2FC',
                    y = 'p_val_adj',
                    hline = c(10e-10),
                    pCutoff = 0.05,
...
p3 =EnhancedVolcano(asx_pairwise_mkrs_seu_clustr_sst[[15]] %>% filter(pw_comparison == 'SC5_vs_SC7'),
                    lab = left_join(asx_pairwise_mkrs_seu_clustr_sst[[15]], Pf3D7_genes_plasmoDB, by = 'gene_id')%>% filter(pw_comparison == 'SC5_vs_SC7') %>% pull(Gene),
                    x = 'avg_log2FC',
                    y = 'p_val_adj',
                    hline = c(10e-10),
                    pCutoff = 0.05,
...
# strns_comps =  c('SC2_vs_SC5', 'SC2_vs_SC7', 'SC5_vs_SC7')
list(asx_pairwise_mkrs_seu_clustr_sst[11:14]) %>%
  pmap(., ~{
    mast_de_tbl = ..1
    map(strns_comps, ~ mast_de_tbl %>% 
