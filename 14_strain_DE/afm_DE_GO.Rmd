...
Read in asexual, female and male MAST and tradeseq condition test results
```{r}
mast_tde_corr_tbls <- readRDS(paste0(save_dir, "mast_tde_corr_tbls.RDS"))

```

############### Read in gametocyte files ###########################

Keep if needed for selection of some donors
```{r, results='asis', warning=F, message=F}
...
```{r, results='asis', warning=F, message=F}
mast_tde_corr_tbls_sig <- map(mast_tde_corr_tbls, ~.x %>%
    # filter(waldStat > 20 & pvalue < 0.01 & p_val_adj < 0.01) %>%
    filter(waldStat > 20 & pvalue < 0.05 & p_val_adj < 0.05) %>%
      left_join(., Pf3D7_genes_plasmoDB %>% dplyr::select(gene_id, Gene), by = 'gene_id'))

mast_tde_corr_tbls_sig
```

Read in significant asexual, female and male MAST and tradeseq condition test results
```{r}
saveRDS(mast_tde_corr_tbls_sig, paste0(save_dir, "mast_tde_corr_tbls_sig.RDS"))
...
all_sig_gns_df_afm <- map(list(mast_tde_corr_tbls_sig[a_groups], mast_tde_corr_tbls_sig[f_groups], mast_tde_corr_tbls_sig[m_groups]), ~{
  .x %>%
  bind_rows(., .id = "donor") %>%
  group_by(gene_id) %>%
  mutate("freq_btwn_don" = n_distinct(donor)) %>%
  group_by(donor, gene_id) %>%
  mutate("freq_wthn_don" = n_distinct(pw_comparison)) %>%
  ungroup()
      }) %>% 
  set_names(stg_nms)
...
```{r, results='asis', warning=F, message=F}
DE_genes_freq <- map(all_sig_gns_df_afm, ~{.x %>% 
...
```{r}
## Save the DE genes for the paper
saveRDS(DE_genes_freq, paste0(save_dir, "DE_genes_freq.RDS"))

```

```{r}
## Save the DE genes for the paper
saveRDS(all_sig_gns_df_afm, paste0(save_dir, "all_sig_gns_df_afm.RDS"))
...
```{r, results='asis', warning=F, message=F}
# afm_gns_don <- map2(all_sig_gns_df_afm,c(4,4,2), ~{.x %>% 
# afm_gns_don <- map2(c(all_sig_gns_df_afm, all_sig_gns_df_afm["a"]), c(3,3,2,1), ~{.x %>% 
afm_gns_don <- map2(all_sig_gns_df_afm, c(2,3,3), ~{.x %>%
# afm_gns_don <- map2(c( all_sig_gns_df_afm["a"]), c(2), ~{.x %>% 
...
```{r, results='asis', warning=F, message=F, eval=T}
afm_gns_don[["f"]] %>% 
  pull(gene_id) %>%
  str_replace(., "-", "_") %>%
  dput()
...
```{r, eval=T}
afm_gns_don[["f"]] %>% pull(gene_id) %>% str_replace(., "-", "_") %>% write(., "data/processed/Pf/m2_all/GO_dump/afm_gns_don.txt")
```
...
```{r, results='asis', warning=F, message=F}
de_gns <- map(afm_gns_don, ~{.x %>% pull(gene_id) %>% unique()})
...
DE_unvrs_gns_f <- map(afm_bal_ts[f_groups], ~{
...
DE_unvrs_gns_afm <- map(list(a_groups, f_groups, m_groups), ~{
...
# # DE_unvrs <- list(DE_unvrs_gns_asx) %>% set_names(stg_nms2)
...
##!!!!NOTE - PF68 GO FILES ARE BROKEN AND I'M NOT ABLE TO REMAKE THEM. Using the old 63 instead
Pf3D7_68_gaf_F <- readRDS("../../Pf3D7_genomes_gtfs_indexs/Pf3D7_63_gaf_F.rds")
Pf3D7_68_gaf_P <- readRDS("../../Pf3D7_genomes_gtfs_indexs/Pf3D7_63_gaf_P.rds")
Pf3D7_68_gaf_C <- readRDS("../../Pf3D7_genomes_gtfs_indexs/Pf3D7_63_gaf_C.rds")
Pf3D7_68_gaf <- readRDS("../../Pf3D7_genomes_gtfs_indexs/Pf3D7_63_gaf.rds")
...
```{r, message=FALSE}
## GO
strn_DE_go_std_pdb <- list(de_gns, DE_unvrs_gns_afm) %>%
...
##Print the DE GO tables using and arrange in the order msc14 female and then msc3 male
# DE_go_gt_lst <- map(DE_go_std_pdb[c(1,2)],
strn_DE_go_gt_lst_slim2 <- map(strn_DE_go_std_pdb_p,
                 ~.x %>% 
                   group_by(Ontology) %>% 
                   distinct(GeneRatio, .keep_all = T) %>%
                   arrange(`p.adjust`) %>% 
                   slice_head(., n=3) %>% 
                   ungroup()  %>% 
                   dplyr::select(c('Ontology', 'Description', 'GeneRatio', 'P')) %>% rename(c('Gene\nRatio'= GeneRatio)) %>%  
...
# fem_gns_pos <- afm_gns_don[["f"]] %>% 
fem_gns_pos <- afm_gns_don[["a"]] %>% 
...
pf_chr_size <- read_table("/lustre/scratch126/tol/teams/lawniczak/users/jr35/Pf3D7_genomes_gtfs_indexs/Pf_Pdb68_Genome.size", col_names = F) 
...
background_genes <- all_pf_gns %>%
  filter(gene_id %in% DE_unvrs_gns_afm[["f"]]) %>% mutate(
    seqnames = chr,
    start = start_pos,
    end = end_pos,
    gene_id = gene_id,
