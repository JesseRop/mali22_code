...
sample_name_ed_cr_slct <- sample_name_ed_cr[str_detect(sample_name_ed_cr, "CbendrCr")]
```
...
all_pref_pred <- list.files("data/processed/Pf", pattern = "bp.*_(CbendrCr)_pred_sR.RDS$", full.names = T, recursive = T, include.dirs = T) %>%
...
```{r, message=F, warning=F, eval=T}
## Get mdata file list
# all_seu_list_files <- Sys.glob("data/processed/Pf/*/seu_obj/bp_CbendrCr3.RDS")
all_seu_list_files <- Sys.glob("data/processed/Pf/*/seu_obj/dense_CbendrCr3.RDS")
all_seu_list_files
```

```{r, message=F, warning=F, eval=T}
# sample_name_ed_cr_slct3 = paste0(sample_name_ed_cr_slct, "3")
```

```{r, message=F, warning=F, eval=T}
## read filtered barcodes
all_seu <- map(all_seu_list_files, ~.x %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/|/seu_obj/dense|3.RDS')))) %>% 
  unlist(., recursive = F) %>% 
  .[sample_name_ed_cr_slct] %>%
  .[!is.na(.)] %>%
  map(readRDS)
...
# --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/MSC*/seu_obj/bp_CbendrCr_prelim_stg3.RDS" \
# : "${HVG:=500}"
# : "${CLUSTER_RESOLN:=0.5}"
