...
# source("mali22_code_lnk/local_rmd_nbooks/m2_pbulk_gtypes_headers/m2_pbulk_gtypes_resc_ppQC_header.R")
source("mali22_code_lnk/pseudobulk_gtyping_preQC/m2_pbulk_gtypes_headers/m2_pbulk_gtypes_resc_ppQC_header3.R")
# source("mali22_code_lnk/local_rmd_nbooks/m2_pbulk_gtypes_preQC_m55_m60_header.R")
...
seqVCF2GDS("data/processed/Pf/MSC14/pbulk_gtypes_ppQC_resc/stage_afm_strain_k8_hsat/p1/fb_biallelic.vcf", "data/processed/Pf/MSC14/pbulk_gtypes_ppQC_resc/stage_afm_strain_k8_hsat/p1/fb_biallelic.gds")
...
pmap(list(sample_name_k, optimum_k), ~{
  ...
  map(grps, ~paste(paste0("data/processed/",species, "/", sm ,"/", gtyp_step, "/", .x,"_k", gp,"_", algn_nm,"/p1/fb_biallelic.vcf"), paste0("data/processed/",species, "/", sm ,"/", gtyp_step, "/", .x,"_k", gp,"_", algn_nm,"/p1/fb_biallelic.gds")))
...
pmap(list(sample_name_k, optimum_k), ~{
  ...
  map(grps,~print(paste0("/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/",species, "/", sm ,"/", gtyp_step, "/", .x,"_k", gp,"_", algn_nm,"/p1/fb_biallelic.vcf")))
  
  map(grps,~print(file.exists(paste0("/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/",species, "/", sm ,"/", gtyp_step, "/", .x,"_k", gp,"_", algn_nm,"/p1/fb_biallelic.vcf"))))
...
pf7_hlght <- function(m14_gt ) {
  pf7_h <-  m14_gt %>%
    mutate(across(filt_reason1, ~case_when(. == "Pass_1" & (filter != 'PASS' | is.na(filter)) ~ 'Absent Pf7_Pass', ## absent in pf 7 means also non - PASS
                                           # . == "Pass_1" & (filter != 'PASS' | is.na(filter)) ~ 'Absent Pf7_Pass & ULI', ## absent in pf 7 means also non - PASS 
                                           . == "Pass_1" ~ 'Pass',
                                           T ~ as.character(.)))
           ) 
  
  pf7_h %>% dplyr::count(filt_reason1, 
                         # ULI_snp, 
                         filter)%>% print()
...
broadqc_lvls <- c('Pass', 'Absent Pf7_Pass', 'Mt', 'Low quality')
...
cln_plt_snp_filt_categ <- function(annot_snp_qc, xlb = 'Strain', ttl, group2exclude = "nothing", y_var = "n", y_lab = 'Number of SNPs'){
...
annot_snp_qc %>% 
  ...
  mutate(across(groups, ~str_remove_all(., "MSC[0-9]+_"))) %>% 
    ...
    filter(!(filt_reason %in% group2exclude)) %>%
  # ggplot(aes(x=filt_reason,fill=filt_reason, y = !!rlang::sym(y_var)/1000)) + 
  ggplot(aes(x=filt_reason,fill=filt_reason, y = !!rlang::sym(y_var))) + 
  geom_col(position = position_dodge(width = 1)) + 
  ...
  facet_wrap(.~groups, nrow = 1,scales = "free_x", drop = T) +
  labs(x = xlb, y = y_lab, fill = 'SNP category', title = ttl) +
  theme_classic()+
  theme(axis.text.y = element_text(size = 14),
        axis.title.x = element_blank(),
        strip.text.x.top = element_text(size = 15, face='bold'),
...
```{r, fig.height=6, fig.width=30}
ss_afm_pf7M_qc[[1]] %>% filter(filt_reason %in% c("Clean", "Non distinct", "Noisy")) glimpse()
```

```{r, fig.height=6, fig.width=20}
##By retaining only SNPs with absolute difference between alt and ref >10 we also factor in depth per allele per cluster
##!!!CHEATING - SORT OUT
ss_afm_k6r_qc_plt_flt <- imap(ss_afm_pf7M_qc, ~{
  cln_plt_snp_filt_categ(annot_snp_qc = .x %>% 
                           mutate(across(groups, ~str_remove(., "MSC[0-9]+[|C|_SLOC|_MACS]_"))), 
                         ttl = .y,
                         group2exclude = c("Low UMI support", "Clean", "Ref inverted", "Alt inverted"),
                         y_var = "perc",
                         y_lab = "Percentage")
  })
ss_afm_k6r_qc_plt_flt
