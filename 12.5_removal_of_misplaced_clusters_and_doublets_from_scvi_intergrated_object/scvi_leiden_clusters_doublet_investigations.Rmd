...
```{r}
all_jcC_seu_scvi_stg_noM55_afm <- SplitObject(all_jcC_seu_scvi_noM55, split.by= "stage_afm")
```


```{r}
all_jcC_seu_scvi_stg_noM55_afm <- c(list(all_jcC_seu_scvi_noM55) %>% set_names("all"), 
...
```{r}
DimPlot(all_jcC_seu_scvi_noM55, group.by = "leiden", reduction = "scvi_umap", label = T)
...
saveRDS(all_jcC_seu_scvi_stg_noM55_afm_ActivAsexDbts, "data/processed/Pf/m2_all/scvi2seu/all_jcC_seu_scvi_stg_noM55_afm_ActivAsexDbts.RDS")
saveRDS(all_jcC_seu_scvi_stg_noM55_afm_ActivAsexDbts[[1]], "data/processed/Pf/m2_all/scvi2seu/all_jcC_seu_scvi_stg_noM55_ActivAsexDbts.RDS")

```

```{r}
saveRDS(tbl_umap_gn_asx_hires_stages, "data/processed/Pf/m2_all/scvi2seu/tbl_umap_gn_asx_hires_stages.RDS")

```


```{r, warning=FALSE, message=FALSE}
# top_de_tbl_rev <- map(seu_scviLeiden_markers_anot, ~.x %>% 
#   # filter(gene %in% gns) %>% 
#   arrange(desc(avg_log2FC)) %>% 
#     filter(p_val_adj< 0.05 & avg_log2FC<0 & pct.1 <0.05) %>%
#     group_by(cluster) %>% 
#     slice_min(order_by = p_val_adj, n = 40) %>% 
...
imap(top10gns[["Female"]], ~FeaturePlot(all_jcC_seu_scvi_noM55, features = .x[1:2], reduction = "scvi_umap", raster = T, order = T))
...
imap(top10gns_strict[["Female"]], ~FeaturePlot(all_jcC_seu_scvi_noM55, features = .x, reduction = "scvi_umap", raster = T, order = T, ncol = 5))
...
```{r}
# kahrp
map(all_jcC_seu_scvi_stg_noM55_afm_ActivAsexDbts, ~FeaturePlot(.x, features = "PF3D7-0202000", reduction = "scvi_umap", pt.size = 0.01, raster = F, order = T, label = T))
```


```{r}
#cdpk1
FeaturePlot(all_jcC_seu_scvi_noM55, features = "PF3D7-0217500", reduction = "scvi_umap", label = T, raster = F, order = T)
...
```{r, warning=FALSE, message=FALSE}
## Gamete top marker - PF3D7_0825800 https://www.nature.com/articles/s41586-025-09653-0
map(all_jcC_seu_scvi_stg_noM55_afm_ActivAsexDbts, ~FeaturePlot(.x, features = c("PF3D7-0825800"), order =T, ncol = 1))

```


```{r, warning=FALSE, message=FALSE, fig.width=16, fig.height=6}
map(all_jcC_seu_scvi_stg_noM55_afm_ActivAsexDbts, ~FeaturePlot(.x, features = c("PF3D7-0829900", "PF3D7-1229200", "PF3D7-1222600", "PF3D7-1102500", "PF3D7-0818900"), order =T, ncol = 3))
...
map(all_jcC_seu_scvi_stg_noM55_afm_ActivAsexDbts, ~FeaturePlot(.x, features = cold_activated_RNAs, order =T, ncol = 3))
```
