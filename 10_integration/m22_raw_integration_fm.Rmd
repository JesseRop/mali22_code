...
m2_asex_raw_mrg <- list.files("data/processed/Pf/m2_all/", pattern = "all_msc_qcd_anotd_asex_jc.*mrg3.RDS$", full.names = T, recursive = T, include.dirs = T) %>%
  set_names(nm = (str_remove_all(.,'data/processed/Pf/m2_all/|/all_msc_qcd_anotd_asex_|_mrg3.RDS'))) %>% 
  # .[sample_name_all] %>%
  .[mrg_names] %>%
  map(readRDS)
```
