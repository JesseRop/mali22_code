...
```
########################## START - Add metadata for asexual like female cells ##########################

```{r}
# generated in 'scvi_leiden_clusters_seuratDE.Rmd' using these asexual markers c("PF3D7-0831700", "PF3D7-0501300", "PF3D7-0202000", "PF3D7-0113000", "PF3D7-1372200", "PF3D7-1401400", "PF3D7-1001500") - go to that notebook for more details
tbl_umap_gn_asx_hires_stages <- readRDS("data/processed/Pf/m2_all/scvi2seu/tbl_umap_gn_asx_hires_stages.RDS") %>% 
  select(c("bcode", "stage_fld", "PF3D7-0831700", "PF3D7-0501300", "PF3D7-0202000", "PF3D7-0113000", "PF3D7-1372200", "PF3D7-1401400", "PF3D7-1001500", "mean_asx_gns", "sum_asx_gns", "fem_asx_dblt_mean", "fem_asx_dblt_sum", "stage_scvi_hires_fem")) %>%
  mutate(fem_dbt_status = case_when(stage_scvi_hires_fem == "fem8_asx_dblt" ~ "likely_asexual_doublet",
                   T ~ "not_asex_dbt"))
...
```{r}
tbl_umap_gn_asx_hires_stages %>% count(stage_scvi_hires_fem, fem_dbt_status)
```


```{r}
all_jcC_seu_scvi_noM55 <- AddMetaData(all_jcC_seu_scvi_noM55, tbl_umap_gn_asx_hires_stages %>% select(bcode, fem_dbt_status) %>% column_to_rownames('bcode'))
                                  
```

```{r}
DimPlot(all_jcC_seu_scvi_noM55, reduction = "scvi_umap", group.by = "fem_dbt_status", label = T)
```

```{r}
## KAHRP
FeaturePlot(all_jcC_seu_scvi_noM55, reduction = "scvi_umap", features = "PF3D7-0202000", label = T, slot = "counts", order = T)
```
########################## END - Add metadata for asexual like female cells ##########################
...
all_seu_scvi_fem_femLE <- all_jcC_seu_scvi_noM55[, all_jcC_seu_scvi_noM55$stage_fld %in% c("Female", "Female (LE)")]
all_seu_scvi_male_maleLE <- all_jcC_seu_scvi_noM55[, all_jcC_seu_scvi_noM55$stage_fld %in% c("Male", "Male (LE)")]

```

```{r}
all_seu_scvi_fm <- list(all_seu_scvi_fem, all_seu_scvi_femLE, all_seu_scvi_male, all_seu_scvi_maleLE, all_seu_scvi_fem_femLE, all_seu_scvi_male_maleLE) %>% set_names(c("f", "fLE", "m", "mLE", "f_fLE", "m_mLE"))
...
```{r}
clstrs2keep <- list(c(2, 4, 8, 9, 10, 14, 15), c(3), c(0, 6, 11), c(11), c(2, 4, 8, 9, 10, 14, 15, 3), c(0, 6, 11))
```
...
donor_counts_in_leiden0.4 <- map(donor_counts_in_leiden_clusters, ~.x %>% filter(don_prop >= 0.4) %>% pull(leiden) %>% unique())
donor_counts_in_leiden0.4
...
lists <- list(donor_counts_in_leiden_all, donor_counts_in_leiden0.7, donor_counts_in_leiden0.5, donor_counts_in_leiden0.4)
...
names(dedup_lists) <- c("all", "w.7", "w.5", "w.4")
...
```{r}
imap(fm_leiden_w_donors, ~DimPlot(.x, reduction = "scvi_umap", group.by = "leiden", label = T) + labs(title = .y))
```

########################## START - Add a version of the female object with the asexual like cells removed ##########################

```{r}
## remove asex like females from objects
objs_to_rm_asex <- c("w.5.f", "w.5.f_fLE")

fm_leiden_w_donors_wout_asex_dbt <- map(fm_leiden_w_donors[objs_to_rm_asex], ~subset(.x, subset = fem_dbt_status != "likely_asexual_doublet")) %>% set_names(paste0(objs_to_rm_asex, "_no_asx_dbt"))
...
fm_leiden_w_donors[paste0(objs_to_rm_asex, "_no_asx_dbt")] <- fm_leiden_w_donors_wout_asex_dbt

```

```{r}
imap(fm_leiden_w_donors, ~DimPlot(.x, reduction = "scvi_umap", group.by = "fem_dbt_status", label = T) + labs(title = .y))

```


########################## END - Add a version of the female object with the asexual like cells removed ##########################

Asses outlier cells below

```{r}
## assess cells above 99% quantile for UMI count
map(fm_leiden_w_donors, ~VlnPlot(.x, features = "nCount_RNA", group.by = "leiden", split.by = "stage_fld") + geom_hline(yintercept = quantile(.x$nCount_RNA, probs = c(0.01, 0.99))))
```
...
# --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/integration/fm_leiden_w_donors_*.{f,f_fLE,f_no_asx_dbt,f_fLE_no_asx_dbt}.RDS" \
# --hvg 900 --cluster_resoln 0.1 --cl_nm "seurat_clusters" --integrtd_pc30 "no" \
...
# --input_seu "/lustre/scratch126/tol/teams/lawniczak/users/jr35/phd/Mali2/data/processed/Pf/m2_all/integration/fm_leiden_w_donors_*.{m,m_mLE}.RDS" \
...
## Get file list
fm_leiden_w_donors_norm_harmony_list_files <- Sys.glob("data/processed/Pf/m2_all/integration/fm_leiden_w_donors_*_norm_harmony_PC_man.RDS")

fm_leiden_w_donors_norm_harmony_list_files
```

```{r, message=F, warning=F, eval=T}
## read filtered barcodes
fm_leiden_w_donors_norm_harmony <- map(fm_leiden_w_donors_norm_harmony_list_files, ~.x %>% 
  set_names(nm = (str_remove_all(.,'data/processed/Pf/m2_all/integration/fm_leiden_w_donors_|_norm_harmony_PC_man.RDS')))) %>% 
  unlist(., recursive = F) %>% 
  # .[sample_name] %>%
  map(readRDS)
...
# saveRDS(fm_leiden_w_donors_norm_harmony_umap_mdata, "data/processed/Pf/m2_all/integration_figs/fm_leiden_w_donors_norm_harmony_umap_mdata.RDS")
...
```{r, warning=FALSE, message=FALSE}
imap(fm_leiden_w_donors_norm_harmony, ~DimPlot(.x, group.by = "leiden", reduction = "umap.harmony_PC_man", label = T)+ labs(title = .y))
...
plot_ly(data = data.frame(fm_leiden_w_donors_norm_harmony[["w.5.f_fLE_no_asx_dbt"]]@reductions[["umap.harmony_PC_man"]]@cell.embeddings),
        x = ~umapharmonyPCman_1, y = ~umapharmonyPCman_2, z = ~umapharmonyPCman_3,
        color= fm_leiden_w_donors_norm_harmony[["w.5.f_fLE_no_asx_dbt"]]@meta.data$stage_fld,
        # colors = c('blue', 'red'),
        opacity = 0.8,
        size = 0.1, 
        type = 'scatter3d'
...
plot_ly(data = data.frame(fm_leiden_w_donors_norm_harmony[["w.5.f_fLE"]]@reductions[["umap.harmony_PC_man"]]@cell.embeddings),
        x = ~umapharmonyPCman_1, y = ~umapharmonyPCman_2, z = ~umapharmonyPCman_3,
        color= base::as.factor(fm_leiden_w_donors_norm_harmony[["w.5.f_fLE"]]@meta.data$leiden),
        # colors = c('blue', 'red'),
        opacity = 0.8,
        size = 0.1, 
        type = 'scatter3d'
...
```{r, warning=FALSE, message=FALSE}
imap(fm_leiden_w_donors_norm_harmony, ~DimPlot(.x, label = T, reduction = "integrated.harmony_PC_man", group.by = "leiden", dims = c(1,2)) + labs(title = .y))

```

```{r, warning=FALSE, message=FALSE}
imap(fm_leiden_w_donors_norm_harmony, ~DimPlot(.x, label = T, reduction = "integrated.harmony_PC_man") + labs(title = .y))
imap(fm_leiden_w_donors_norm_harmony, ~DimPlot(.x, label = T, reduction = "umap.harmony_PC_man")+ labs(title = .y))
imap(fm_leiden_w_donors_norm_harmony, ~DimPlot(.x, group.by = "donor", reduction = "umap.harmony_PC_man")+ labs(title = .y))
imap(fm_leiden_w_donors_norm_harmony, ~DimPlot(.x, group.by = "leiden", reduction = "umap.harmony_PC_man")+ labs(title = .y))
imap(fm_leiden_w_donors_norm_harmony, ~FeaturePlot(.x, features = "nFeature_RNA", reduction = "umap.harmony_PC_man")+ labs(title = .y))
imap(fm_leiden_w_donors_norm_harmony, ~FeaturePlot(.x, features = "nCount_RNA", reduction = "umap.harmony_PC_man")+ labs(title = .y))
...
map(fm_leiden_w_donors_norm_harmony, ~FeaturePlot(.x, features = c("PF3D7-0202000", "PF3D7-0113600", "PF3D7-1102500"), reduction = "umap.harmony_PC_man", ncol = 3))
...
```{r, warning=FALSE, message=FALSE}
imap(fm_leiden_w_donors_norm_harmony_jnd, ~saveRDS(., paste0("data/processed/Pf/m2_all/integration/fm_leiden_w_donors_", .y,"_norm_harmony_jnd.RDS")))

```


```{r, warning=FALSE, message=FALSE}
# Combined metadata and umap embeddings for figure generation
fm_leiden_w_donors_norm_harmony_jnd_umap_mdata <- map(fm_leiden_w_donors_norm_harmony_jnd_all, ~{
  .x@meta.data %>% 
    rownames_to_column("bcode") %>%
      left_join(., .x@reductions$umap.harmony_PC_man@cell.embeddings %>% data.frame() %>% rownames_to_column("bcode"), by = "bcode") 
})
...
```{r, warning=FALSE, message=FALSE}
fm_leiden_w_donors_norm_harmony_jnd_umap_mdata <- fm_leiden_w_donors_norm_harmony_jnd_umap_mdata %>% 
  set_names(str_replace_all(names(fm_leiden_w_donors_norm_harmony_jnd_umap_mdata), c("w\\." = "w_pt", "\\." = "_")))
```

```{r}
# Save for figure generation
imap(fm_leiden_w_donors_norm_harmony_jnd_umap_mdata, ~saveRDS(.x, paste0("data/processed/Pf/m2_all/integration/fm_leiden_w_donors_norm_harmony_jnd_", .y, "_umap_mdata.RDS")))
