...
```{r}
m2_all_raw_mrg <- readRDS("data/processed/Pf/m2_all/scvi2seu/all_jcC_seu_scvino_M55.RDS")

```

```{r}
DimPlot(m2_all_raw_mrg, reduction = "scvi_umap", group.by = "stage_fld", cols = sp_col_new)

```

```{r}
DimPlot(m2_all_raw_mrg, reduction = "scvi_umap", group.by = "stage_fld", cols = sp_col_new, dims = c(1,2))
...
VlnPlot(m2_all_raw_mrg, features = "nFeature_RNA", group.by = "stage_fld", pt.size = 0) + geom_hline(yintercept = c(200, 400, 500, 1000))
```

```{r, warning=FALSE, message=FALSE}
VlnPlot(m2_all_raw_mrg, features = "nFeature_RNA", group.by = "leiden", pt.size = 0) + geom_hline(yintercept = c(200, 400, 500, 1000))
```

```{r}
all_sct_intgrtd_fem <- m2_all_raw_mrg[, m2_all_raw_mrg$stage_fld  == "Female"]
all_sct_intgrtd_femLE <- m2_all_raw_mrg[, m2_all_raw_mrg$stage_fld  == "Female (LE)"]
all_sct_intgrtd_male <- m2_all_raw_mrg[, m2_all_raw_mrg$stage_fld  == "Male"]
all_sct_intgrtd_maleLE <- m2_all_raw_mrg[, m2_all_raw_mrg$stage_fld  == "Male (LE)"]

```

```{r}
all_sct_intgrtd <- list(all_sct_intgrtd_fem, all_sct_intgrtd_femLE, all_sct_intgrtd_male, all_sct_intgrtd_maleLE) %>% set_names(c("fem", "femLE", "male", "maleLE"))

```

```{r}
# names(all_sct_intgrtd)
...
```{r}
pmap(list(all_sct_intgrtd, c(5,5), c(-1,-0.5)),  ~DimPlot(..1, reduction = "scvi_umap", group.by = "leiden", label = F, pt.size = 0.4) + geom_vline(xintercept =  ..2) + geom_hline(yintercept =  ..3))
```

```{r}
all_sct_intgrtd_fem <- pmap(list(all_sct_intgrtd, c(5,5), c(-1,-10), c(15,-0.5)),  ~..1[, ..1@reductions[["scvi_umap"]]@cell.embeddings[, "umap_1"] > ..2 & ..1@reductions[["scvi_umap"]]@cell.embeddings[, "umap_2"] > ..3 & ..1@reductions[["scvi_umap"]]@cell.embeddings[, "umap_2"] < ..4]
)
```

```{r}
map(all_sct_intgrtd_fem, ~DimPlot(.x, reduction = "scvi_umap"))
```

```{r}
map(all_sct_intgrtd_fem, ~DimPlot(.x, reduction = "scvi_umap", group.by = "leiden", label = T))
```

```{r, warning=FALSE, message=FALSE}
for (nm in names(all_sct_intgrtd_fem)) {
  DefaultAssay(all_sct_intgrtd_fem[[nm]]) <- "RNA"
  all_sct_intgrtd_fem[[nm]][["RNA"]]$data <- NULL
  all_sct_intgrtd_fem[[nm]][["RNA"]]$scale.data <- NULL
  all_sct_intgrtd_fem[[nm]][["SCT"]] <- NULL
}
```

```{r, warning=FALSE, message=FALSE}
all_sct_intgrtd_fem <- map(all_sct_intgrtd_fem, ~split(.x, f = .x$orig.ident))
...
don_50 <- map(all_sct_intgrtd_fem, ~table(.x$orig.ident) %>% data.frame() %>% filter(Freq >= 100) %>% pull(Var1) %>% as.character)
```

```{r}
## Retain only donors with more than 50 cells
all_sct_intgrtd_fem <- map2(all_sct_intgrtd_fem, don_50, ~subset(.x, subset = orig.ident %in% .y))
```
