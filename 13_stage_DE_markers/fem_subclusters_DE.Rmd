...
avg_expression_clustrs_df_data %>%
  ggplot(aes(x = `g2_w.5.f`, y = `g2_all.fLE`)) +
...
```{r, message=FALSE, warning=FALSE, eval=T, fig.width=6, fig.height=3.2}
avg_expression_clustrs_df_data %>%
  filter(g2_w.5.f < 20 & g2_all.fLE < 20 ) %>%
  ggplot(aes(x = `g2_w.5.f`, y = `g2_all.fLE`)) +
  geom_point(size = 0.01) +
  geom_abline()
...
```{r}
## Check how well the original leiden clusters correlate with the newly generated harmony_clusters_coarse_bulkDE clusters
map(fm_leiden_w_donors_norm_harmony_jnd_cln_plots_umap_mdata, ~.x %>% count(leiden, harmony_clusters_coarse_bulkDE) %>% group_by(leiden) %>% mutate(prop = round(n/sum(n), 2)))
...
## Since the cluster assignment does not change largely for the females and in consistency with the original leiden annotations keep the leiden annotation. New 'harmony_clusters_coarse_bulkDE' commented out
...
activ_fem <- pmap(list(fm_leiden_w_donors_norm_harmony_jnd_cln_plots_umap_mdata, c("leiden", "harmony_clusters_coarse_bulkDE"), c("8", "2")), ~..1 %>% filter(!!rlang::sym(..2) == ..3) %>% pull(bcode)) 

names(activ_fem) <- paste0("ActivFem_", names(activ_fem))

fem <- pmap(list(fm_leiden_w_donors_norm_harmony_jnd_cln_plots_umap_mdata, c("leiden", "harmony_clusters_coarse_bulkDE"), c("8", "2")), ~..1 %>% filter(!!rlang::sym(..2) != ..3) %>% pull(bcode)) 
...
seu_scvi_fem_sub_clusters <- all_jcC_seu_scvi_noM55_afm_cln[[1]]@meta.data %>%
  rownames_to_column("bcode") %>%
  mutate(fem_subclusters = case_when(bcode %in% activ_fem[["ActivFem_w.5.f"]] ~ "ActivFem",
                                     bcode %in% activ_fem[["ActivFem_all.fLE"]] ~ "ActivFemLE",
                                     bcode %in% fem[["Fem_w.5.f"]] ~ "Female",
                                     bcode %in% fem[["Fem_all.fLE"]] ~ "Female (LE)",
                                     T ~ "Rest"),
         fem_hires_subclusters = case_when(fem_subclusters == "Female" & leiden == "2" ~ "Female2",
                                           fem_subclusters == "Female" & leiden == "4" ~ "Female4",
                                           T ~ fem_subclusters)) %>%
  select(bcode, fem_subclusters, fem_hires_subclusters) %>% 
...
seu_scvi_fem_sub_clusters <- list(seu_scvi_fem_sub_clusters) %>% set_names(c("f"))

```

```{r}
map(seu_scvi_fem_sub_clusters, ~DimPlot(.x, group.by = "fem_hires_subclusters") )
...
```{r, warning=FALSE, message=FALSE}
# Save for plotting
imap(seu_scvi_fem_sub_clusters, ~saveRDS(.x, paste0(save_dir_stage_DE_markers, 'seu_scvi_fem_sub_clusters_', .y,'.RDS')))

```

```{r}
# Get UMAPs and metadata for plotting
seu_scvi_fem_sub_clusters_umap_mdata <- map(seu_scvi_fem_sub_clusters, ~{
  .x@meta.data %>% 
    rownames_to_column("bcode") %>%
...
```{r, warning=FALSE, message=FALSE}
# Save for plotting
imap(seu_scvi_fem_sub_clusters_umap_mdata, ~saveRDS(.x, paste0(save_dir_stage_DE_markers, 'seu_scvi_fem_sub_clusters_umap_mdata_', .y,'.RDS')))
